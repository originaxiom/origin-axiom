"""B350 lock -- gate A extension: the cyclic-cover (abelian) torsion class is a Galois orbit.
...
_PATH = (pathlib.Path(__file__).resolve().parents[1] / "frontier"
         / "B350_cyclic_cover_torsion_galois" / "cyclic_cover_torsion.py")
_spec = importlib.util.spec_from_file_location("b350", _PATH)
b350 = importlib.util.module_from_spec(_spec); _spec.loader.exec_module(b350)


def test_torsion_orders_are_the_P8_C5_lucas_ladder():
    orders = b350.torsion_orders(8)
...
def test_resultant_identity_exact():
    for n, (prod, det) in b350.resultant_identity(6).items():
        assert prod == det, f"n={n}"


def test_factor_multiset_is_a_symmetrizable_galois_orbit():
    for n in (3, 4, 5, 6):
        perm_ok, esf, esf_int = b350.factor_orbit_is_galois_closed(n)
...
def test_deck_action_fixed_point_free_uniformly():
    ...
    unit, results = b350.deck_action_fixed_point_free(5)
...
def test_generic_tier_honesty():
    ...
    assert b350.generic_tier_note() == -1
