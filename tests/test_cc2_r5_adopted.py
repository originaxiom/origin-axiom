...
# L184 lazy-fy (B1178/R50-5): the 300-line module-level lock script executed ~36 s AT
# COLLECTION (the E50 shape). The body now runs once, at first test execution, via the
# cached runner; the exact_engine import is deferred with it. Outcomes unchanged.
import functools
...
def test_r5_adopted_locks_all_pass():
    PASS = _run()  # the lock script runs once here (was import-time pre-L184)
