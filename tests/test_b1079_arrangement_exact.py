"""B1079 — the rung arrangement exact over ℚ, and the 64 Levi subsystems.

Reads `results.json` (computed) and never prose. The cross-arc assertions are the point:
B1078 reached the same arrangement from the charges, mod three primes; this one reaches it
...
def r():
    with open(os.path.join(F, "B1079_arrangement_exact", "results.json")) as fh:
        return json.load(fh)


@pytest.fixture(scope="module")
def prev():
    with open(os.path.join(F, "B1078_rung_spectrum_attained", "results.json")) as fh:
        return json.load(fh)
...
def test_two_independent_routes_agree(r, prev):
    """B1078: charges → ad-matrices → charpoly orbits → three primes.
    B1079: E₆ roots → restrict to (A₂)-perp → over ℚ. Same lattice, same spectrum."""
...
def test_the_weight_profile_matches_the_charge_side_computation(r, prev):
    """12×1 + 18×3 = 66 = 72 − 6. B1078 got this from the charges; here it falls out of
