...
**Evidence** points to the test that locks the claim. Every `proven` claim is locked
...
date (B1–B532)** — **B69–B532 produced zero promotions** to this ledger (the SL(n) trace-map tower,
...
| P31 | The SL(n) figure-eight trace-map Jacobian is a GL(2,ℤ) representation with class-function universality for all n; the constructive module isomorphism `ρ_n = ⊕_d Sym^d(M_m)^{μ_d}` exact over ℚ[m] at n = 3, 4 (the explicit catalog for n ≥ 5 remains open). | `frontier/B103_tower_equivariance` · `tests/test_b103_tower_equivariance.py` ✓ | *(stamp 2026-08-19: still CURRENT as of B1082 — the stale-absence sweep verified this absence/openness against the full corpus.)*
...
| P36 | The principal-image sealing lemma (all n, hand proof): the `Sym^{n−1}` image of an SL(2,K) representation is a trace-map fixed point with all word-traces in K (the converse remains open). | `frontier/B138_s031_principal_lemma` · `tests/test_b138_s031_principal_lemma.py` ✓ | *(stamp 2026-08-19: still CURRENT as of B1082 — the stale-absence sweep verified this absence/openness against the full corpus.)*
...
| P43 | Golden is the unique metallic mean in the Jones window: λ_m < 2 ⟺ m = 1 (φ = 2cos(π/5)) (elementary exact). **[CORRECTED 2026-07-15, audit 5.4: the former "unique anyon-realizable member" reading was WRONG — unitary fusion categories admit simple objects with d ≥ 2 (Jones index ≥ 4 is a continuum), so d < 2 is not an anyon-realizability criterion. The supportable categorical statement is CONDITIONAL on the rank-2 fusion rule X² = 1 + mX (Ostrik): under that hypothesis only m ∈ {0,1} is unitary. The exact inequality stands; the anyon language is demoted.]** | `frontier/B218_metallic_anyon_selection` · `tests/test_b218_metallic_anyon_selection.py` ✓ |
...
| P46 | M(4,5) is the unique unitary minimal model (c < 1) that is also N=1 superconformal (literature-established + exact rational arithmetic). **[SPLIT 2026-07-15, audit 5.5: the metallic-family conclusion ("golden is the unique metallic member whose chain is superconformal") is CONDITIONAL on the dictionary m → k = m² + 2, which is chosen/motivated, not derived — no fusion data or Hamiltonian is constructed from the metallic object. Tiers: (i) the M(4,5) fact [literature]; (ii) the central-charge identities [exact]; (iii) the metallic dictionary [CONDITIONAL].]** | `frontier/B224_golden_unique_susy_metallic` · `tests/test_b224_golden_unique_susy_metallic.py` ✓ |
...
| P51 | ρ_prin's deformation space in the figure-eight's E₆ character variety: UNOBSTRUCTED TO ORDER 3 — the full polarized quadratic form vanishes EXACTLY on all six directions and all fifteen cross-pairs (B575's exact computation over ℚ(√−3), recomputed at every l51 load; upgrading B273's sampled mod-p route), plus the banked Massey/cubic vanishing at the computed instances. **[DEMOTED 2026-07-15, audit 5.1: the former "smooth point / exists unconditionally / all orders" language over-read a mod-p Schwartz–Zippel sample (6+1 evaluations cannot determine 21 quadratic or 56 cubic coefficients; two primes do not prove a ℚ-identity). Smoothness beyond order 3 is [CONJECTURE]. The existence of ρ_prin itself as a point is trivial and unaffected.]** | `frontier/B575_bridge_obstruction` (exact, order ≤ 3) + `frontier/B273_e6_obstruction`/`B274` (historical, sampled) ✓ |
...
| P62 | The seam is a twist invariant: the theta and canonical level-15 lifts are inequivalent (exhaustive natural-family intertwiner search empty); Par commutes with the entire canonical image (hence the B358 seam-null is a commutant corollary) and not with the theta image, where Par−J = the Weyl step (P57); the twist is exactly the half-characteristic diagonal cocycle ζ₁₅^{−j(j+1)/2}. seam ≠ 0 ⟺ the quantization carries the twist. | `frontier/B381_twist_isolation` · `tests/test_b381_twist.py` ✓ |
| P63 | The existence law of the value sector (v2, derived and predictive): the level-N theta model tensor-factorizes by CRT; the sector = (local line)⊗(local doublet); doublets exist ONLY at odd prime powers (3-side: all odd powers, 90°; 5-side: by QR class of the multiplier mod 5, 36°/108°), lines on the inert 3-side always and on the ramified 5-side at even powers only. Re-derives all six banked rungs including the 225-death, and passed a registered three-level acceptance duel 3/3 (375: 108°/ord 500; 405: 36°/ord 540; 675: 90°/ord 900) at three primes each — nine sub-verdicts, predictions committed before computation. | `frontier/B377_existence_law_derivation` · `tests/test_b377_existence_law.py` · `tests/test_b377_acceptance.py` ✓ |
| P64 | The shifted trace formula of the theta model (level 15) and the origin of the 1/12: `tr(U_γ·XᵃZᵇ) = tr(U_γ)·ζ₁₅^{½ω(v,(γ−I)⁻¹v) − ½ab − ½ω(v,(1,1))}` on the domain det(γ−I) ∈ (ℤ/15)ˣ (all 225 shifts per word; the two non-invertible sample words fail as pure phases — the domain self-confirms); the twist enters as exactly the half-characteristic term −½ω(v,(1,1)) (canonical model: same quadratic and ordering, linear part ≡ 0 — registered cross-check); the whole Par-table factorizes through the formula (142/142 domain cells); the slot constant reassembles exactly, and its det-class split is `−1/12 = −1/16 (generic) − 1/48 (5-boundary)` with classes 3, 15 silent — the mechanical-240 reading refuted; magnitudes obey `|χ|² = #Fix(γ′)` (unit on generic cells). **Strengthened 2026-07-05 (B396): the factorization is UNIVERSAL — verified on the domain of all six banked pairs, 661/661 cells, zero mismatches (`tests/test_b396_allpairs.py`).** | `frontier/B382_why_one_twelfth` · `tests/test_b382_trace_formula.py` ✓ |
| P65 | The row-16 reality theorem (level 15): `t(16,b) ∈ ℚ(√5)` for all b — the √−3 and √−15 components vanish across the entire row (finite exact verification); mechanism: the anti-table's ζ₅-spectrum has its zero at exactly the 16-exponent window ζ₅^{−4j}, while the row-4 window (ζ₅^{−j}) sees the same nonzero class vector (row 4 bright at b ∈ {4,8}: (0,−1/40,±1/48,∓1/48)). Together with the mirror law (P61), row-16 silence is fully mechanized: reality + mirror. | `frontier/B383_row16_reality` · `tests/test_b383_row16.py` ✓ |
| P66 | The CRT-local closed form of the 1/12 (level 15): the Par-graded trace tensor-factorizes exactly, `C[j,l] = C₃[j,l]·C₅[j,l]` on all 240 cells at the banked multiplier convention (u₃,u₅)=(2,2) (the (1,1) convention fails at 146/240 — the multiplier is load-bearing); the windowed class partials assemble as two-branch products of local sums — the 3-side ℤ₄×ℤ₄ table times 5-side parity-branch constants — reproducing (0,0,−1/16,−1/16) and (0,0,−1/48,−1/48) exactly, hence 1/12 = 1/16 + 1/48 end-to-end from local data; the golden-boundary class couples equal parities only (its off-parity branches vanish identically). | `frontier/B386_crt_closed_form` · `tests/test_b386_tensor.py` ✓ |
| P67 | Brightness is a local property (level 15): for every banked pair the Par-table tensor-factorizes `C = C₃·C₅` at (u₃,u₅) = (2,2) (12/12, extending P66's single-pair identity), the s-classification (√−15 channel) is reproduced from the two LOCAL theta models alone via spectral convolution (12/12: all seven bright pairs' s-cells nonzero, all five dark pairs' zero), and the criterion is predictive out-of-sample: pair (2,5), never previously computed, was predicted DARK from local data alone (registered before computation) and verified DARK by the full global table. The bright/dark riddle is thereby localized to how the two local spectra pair. | `frontier/B390_criterion_tensor` · `tests/test_b390_criterion.py` ✓ |
| P68 | The root-of-unity law of the generic Par-table (level 15): every det-class-1 cell is a root of unity in μ₆₀ (142/142), and the orders are character-gated — `3 | ord(C) ⟺ χ₋₃(det(γ′−I)) = −1` (the Eisenstein gate, now derived: χ₋₃ = +1 forces C ∈ μ₂₀, hence the cell-wise ℚ(√5)-reality of B401), while χ₅(det) = −1 forces `5 | ord(C)` one-directionally. The generic table is pure phase with class-field-gated torsion. | `frontier/B404_gate_derivation` · `tests/test_b404_gate.py` ✓ |
| P69 | **The First Measurement Theorem** (two-seat): in the B854 Chevalley 𝔢₆ the four 2T-invariant vectors span an abelian u(1)⁴; the ⟨x₈,x₁₆⟩-plane stratifies centralizer dimensions as generic-in-torus 12 / generic-in-plane 30 / **exactly three lines at 46**, the lines being the S₃-Galois orbit of the irreducible cubic μ (pencil identities det₃₆ = c·μ¹², det₁₂ = c·μ⁴, exact /ℚ); z(plane) = so(8)⊕u(1)²; each line's centralizer = so(10)⊕u(1); the tiling 𝔢₆ = (so(8)⊕u(1)²) ⊕ V₁⊕V₂⊕V₃ (dim Vᵢ = 16, Vᵢ = ad(x_j)(Kᵢ), j-independent) satisfies [Vᵢ,Vᵢ] ⊆ core and [Vᵢ,Vⱼ] = V_k (weight-line lemma a priori; sum-freeness certified at two primes × all roots); each 𝔢₆/Kᵢ ≅ Vⱼ⊕V_k = 16⊕16̄ of so(10)⊕u(1); module-level the decomposition is the magic square M(𝕆,ℂ) (sector Hom-matrix diag(4,4,4) with vanishing off-diagonal, certified). | `frontier/B877_fmt_review` · `tests/test_b877_fmt_review.py` ✓ · `tests/test_b866_charge_cubic.py` ✓ · `tests/test_b872_coset.py` ✓ · `tests/test_b875_tiling.py` ✓ · `tests/test_b880_signature.py` ✓ |
| P70 | **The magic-square isomorphism theorem**: the algebra L(𝕆_split, ℂ′_split) — constructed over ℚ from Zorn vector-matrix octonions with tri(𝕆) the exact 28-dim triality nullspace, cross products the UNIQUE equivariant bilinears ((1,2)→3: xy; (2,3)→1: y·x̄; (3,1)→2: ȳ·x; ℂ′-side zw, z̄w, zw̄ — multiplicity-one certified over all 28 triples × 64 pairs), same-summand duals the Killing-duals (equivariant by construction), and the nine remaining scalars fixed by Jacobi to λ ≡ 1, μ ≡ −24, ν ≡ −12 — satisfies the Jacobi identity on ALL 76,076 unordered basis triples exactly; its adjoint splits over its rational torus into 72 one-dimensional rational root spaces + a 6-dim Cartan with Cartan matrix E₆; and the Chevalley-generator correspondence extends to an explicit linear map φ verified as a Lie-algebra isomorphism on ALL 3,003 basis pairs (det φ = −2/3) onto the B854 Chevalley 𝔢₆ — the banked tiling (so(8)⊕u(1)² core, three 16-dim tiles, the cyclic law) IS the (𝕆,ℂ) magic-square algebra by explicit structure-constants isomorphism. | `frontier/B904_barton_sudbery` · `tests/test_b904_bs.py` ✓ · `tests/test_b880_signature.py` ✓ |
