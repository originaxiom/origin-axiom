...
| S032 | the **no-forced-choice forward program** (tier MATH, **not a physics bridge**) — (A) theorem-version: prove *no* invariant whatsoever (not just trace-ring) is discretely-multivalued-and-unsymmetric; (B) the two-seed question: does gluing `M_{m₁},M_{m₂}` along their cusp tori create an *internal* discrete fork neither had alone (is the minimal multiplicity for an internal forced choice **two**?). Detector: 0-dim part of the gluing character variety via κ-elimination + Jacobian rank. Unrun | OPEN (MATH) | **B130**/V119, K013, S031, S029 |
