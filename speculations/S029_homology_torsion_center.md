...
*why* (one contingently, one structurally — both confirming the firewall, neither a crossing). The once-named
falsifier (the genus ladder, B139-G) is now **closed (B140)**: the genus-1 `M→Mᵀ` is a genus-1 *mechanism* but the
conclusion is the standard orientation-reversal theorem (genus-independent), so the chirality block survives every
genus; the residual caveat is the general "no *cleverer* invariant" hedge. Cross-ref `S033` (Gate-1),
`../philosophy/P009` (the chirality articulation + caveat), `../docs/OPEN_LEADS.md` (B139-G ANSWERED),
`../REPRODUCIBILITY.md` MB9.
