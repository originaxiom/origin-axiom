...
Seat reactivated; read main to B1200, which unifies three faces under `Φ₃` with linking map `u ↦ u²` = the Galois generator `c`. **Paper IV's orbit theorem runs on `G = ℤ/2` generated by the mirror. Those are the same involution**, verified exactly in five links (12/12, two live controls): `t² = t−1` ⟹ `t = ζ₆`, `ω = t²` primitive cube root; `Φ₃(ω) = 0` **from the holonomy**; `u ↦ u²` swaps the roots; the swap **is** complex conjugation on `ℚ(√−3)`; and conjugating the holonomy (`t ↦ 1−t`, same relator) **is orientation reversal**.
