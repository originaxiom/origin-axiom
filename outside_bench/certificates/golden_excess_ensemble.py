...
# ---- E4: THE CORRECTIONS THE PREREGISTRATION FAILED TO REQUIRE
Nn = np.array([r[2] for r in rows], float)
Aa = np.array([r[3] for r in rows], float)
isg = np.array([r[0].startswith('golden') for r in rows])
rcorr = float(np.corrcoef(Nn, Aa)[0, 1])
b1, b0 = np.polyfit(Nn, Aa, 1)
res = Aa - (b0 + b1 * Nn)
gmax_res = res[isg].max()
p_res = float((res[~isg] >= gmax_res).mean())
p_raw = float((oa >= ga.max()).mean())
p_sidak = 1 - (1 - p_raw) ** int(isg.sum())
imax = int(np.argmax(Aa))

print("\nE4 — *** THE PREREGISTRATION WAS UNDER-SPECIFIED, AND THIS CELL SAYS SO")
print("     BEFORE REPORTING ITS OWN HEADLINE. ***  The sealed criterion was")
print("     'above the 90th percentile of the ensemble'.  It required NEITHER")
print("     a LOOK-ELSEWHERE correction (the golden slope contributes several")
print("     windows and the MAXIMUM was quoted) NOR a CONTROL FOR WINDOW SIZE.")
print("     Both are needed, and both cut against the headline:")
print(f"     (a) N-CONFOUND: corr(asymmetry, N) = {rcorr:+.3f} — the asymmetry")
print(f"         GROWS with window size (fit: asym ~ {b0:.2f} + {b1:.5f}*N), and")
print(f"         golden's large values sit at its LARGEST windows.")
print(f"         N-adjusted, golden's best residual {gmax_res:+.2f} gives")
print(f"         P(other >= it) = {p_res:.3f} — still only the 1/15 resolution floor.")
print(f"     (b) LOOK-ELSEWHERE: golden contributed {int(isg.sum())} windows and the")
print(f"         maximum was quoted.  Raw P = {p_raw:.3f}; Sidak-corrected over")
print(f"         {int(isg.sum())} draws = {p_sidak:.3f}.  NOT SIGNIFICANT.")
print(f"     (c) THE SINGLE MOST DAMAGING FACT: the ensemble maximum is")
print(f"         {Aa[imax]:.0f}, from '{rows[imax][0]}' at N={rows[imax][2]} — a")
print(f"         NON-METALLIC slope, at a SMALLER window than golden's best")
print(f"         ({Aa[isg].max():.0f}).  A slope with no golden structure at all")
print("         produces a LARGER asymmetry.")

print("\nE4b — THE VERDICT AS THE SEAL WROTE IT (reported, then corrected):")
...
print("\nE4c — *** THE CORRECTED VERDICT: G-NOISE. ***")
print("     The sealed criterion fires G-EXCESS, but it is under-specified, and")
print("     under BOTH corrections the excess evaporates: Sidak-corrected")
print(f"     P = {p_sidak:.3f}, the N-trend explains the ordering, and a")
print("     non-metallic slope beats golden outright.  THE LEAD CLOSES")
print("     NEGATIVELY.  memo 144 registered this as a lead and DECLINED to")
print("     claim it on four controls; that caution is VINDICATED, and the")
print("     failure recorded here is in the SEAL, not in the earlier memo.")
