...
def build_basis(dps=90):
    ...
    # === THE INTERVENTION (outside bench, sealed prereg VOL_BASIS_PREREG.md) ===
    # The object's own canonical regulator, absent from the original basis.
    # Vol(m004) computed from the dilogarithm, not quoted:
    from mpmath import polylog, exp as _exp, im as _im
    _VOL = 6 * (_im(polylog(2, _exp(2j * pi / 3))) / 2)
    b['vol'] = _VOL
    b['vol_pinorm'] = _VOL / pi
    b['vol_over_zetaK2'] = _VOL / R['zetaK'][2]

    forms = {
        'FULL': list(b.keys()),
        'E6_only': [k for k in b if k.startswith(('Lm3_', 'zetaK_', 'vol')) or k in ('pi', 'sqrt3', 'zeta3')],
