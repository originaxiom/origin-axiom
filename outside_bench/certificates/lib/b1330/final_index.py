...
GERMS=[(2,),(4,),(2,2),(2,4),(4,4),(2,2,2),(2,4,6),(2,2,2,2)]   # even only: valid for a -I (projective) lift
for NM in sys.argv[1:] or ['s958','t12833','t12835']:
    got=get_rho(NM)
    ...
    for r in rels:
        R=wev(r,rho,2)
        good = R==eye(2) or R==scal((F(-1),F(0),F(0),F(0)),eye(2))
        ok&=good
        print(f"  relator {r!r} -> {'I' if R==eye(2) else ('-I' if good else '** NOT +-I **')}")
    print("  RELATORS VALIDATED:",ok, flush=True)
    if not ok: continue
    chars=order3_chars(gens,rels,mu,lam)
    print("  order-3 cusp-trivial characters:",len(chars), flush=True)
    ...
    print(f"  EXACT in-domain sectors: {tot}")
    print(f"  (t0,I): {dict(sorted(res.items()))}")
    print(f"  t0>=2: {sum(v for (t,i),v in res.items() if t>=2)}")
    print(f"  NON-ZERO INDEX: {len(nonzero)}", flush=True)
    for x in nonzero[:6]: print("     Sym",x[0],"psi",x[1],"t0=",x[2]['t0'],"r1=",x[2]['r1'],"I=",x[2]['I'])
