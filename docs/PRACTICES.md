...
| A **generated surface** must equal what its generator produces. `docs/RECURRENCE_ATLAS.md` is derived from every arc's prose, has its OWN generator (`scripts/atlas/render.py`, not `scripts/views/generate.py`), and had **no freshness check at all** — so it went stale twice in one session and was visible only because `tests/test_atlas.py` silently **writes** it during the suite (B1054 / R1-15) | **GATED** | `atlas-generated` |
