...
18. **Full `pytest`** — green *before* commit, and **do not touch the tree while it runs** (a
    ...
    - **THE LAST KNOWN-GREEN FULL RUN, DATED AND PINNED TO ITS COMMIT — because B1041's finding
      was that nobody knew:** `cd52d44` (the 29th gate's two repairs, 2026-08-14), **4006 passed /
      120 skipped / 0 failed, exit code 0, 54:47**, run against a **committed** tree with nothing
      edited under it — **exit code read directly, never through a pipe**.
      **IT CERTIFIES THE INSTRUMENTS RATHER THAN THEIR CACHES** — the second such run on this
      branch; `6be907e` was the first. It includes `tests/test_instrument_freshness.py`, which re-runs every arc
      ...
      **⚠ AND THAT CLEAN-TREE CLAIM WAS DATE-DEPENDENT WHEN IT WAS FIRST WRITTEN AT `6be907e`.**
      `render.py` stamps `> Last generated: {today}`, so rewriting the atlas was a no-op only on
      the day it had been rendered — the same run one day later dirtied the tree, and the 29th
      gate itself went red on the calendar rather than on staleness. Both were repaired at
      `cd52d44` (the stamp excluded from the comparison; the gate and `tests/test_atlas.py` now
      restore the bytes they found), and **this pin is the first run for which the clean tree is a
      property of the code rather than of the date.**
      *(Previous: `6be907e`, 4006 passed, 53:07 — the first run certifying instruments rather than
      caches, and the run that produced the date-dependent claim corrected above. Before that
      `be87a51`, 3996 passed, 49:23 — green, but green over caches. Before that
