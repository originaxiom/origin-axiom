...
What the window adds to the specification, read from the records: **B1303** — the tree-level vacuum of the ninth closing is SM × U(1)_Z′ (rank 5) on a supersymmetric E₆SSM-type model, with the Z′ regime a FORK on which family carries the expectation value (P9); the paper now states the bounds as constraints on a chiral model suggested by the closing, not on the closing (its spectrum is vector-like), with the normalisation Tr Q′²/Tr Y² = 6210/9. **B1340** (paper-verification lane; item 2 applied to the paper, item 3 held for verification on main) — on the record's vector-like spectrum every SM-commuting direction is anomaly-free, so "anomaly-free by the conjugate copies" carries zero bits; over a chiral spectrum the family part of any anomaly-free U(1)′ is forced to (0, t, −t), which the record's (−10, 5, 5) is not. **B1409** — dim H¹ = k(n − 1) on m004's own covers at n = 2, 3, 4; 20 real parameters at the ceiling, all boundary data, zero interior moduli. **B1355** (the SM seat; its locks re-run green on main 2026-09-15, 21 passed) — the closing's local model is the G₂ cone over CP³/2T, where Witten's inflow forces chiral E₆ matter, one 27 per apex (geometry proved, count cited): the first local model of the bit the object withholds. **The paper's freedom ledger** now carries seven non-continuous rows: the seventh is the electromagnetic identification Q = T₃ + Y that anchors hypercharge, graded unpriced by B919. Zero of the nineteen parameters are supplied; the count is unchanged.

## Currency note — 2026-09-16, the SM seat's closing arcs (B1415)
