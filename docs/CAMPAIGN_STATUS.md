...
**LATEST — B997 (2026-08-09): the first grammar-level UNIQUENESS, with a proof.** |SL(2,ℤ/N)| is a McKay order for **exactly N ∈ {3,4,5}** (2T→E₆, 2O→E₇, 2I→E₈) — proved, since |SL(2,ℤ/N)| ≥ (6/π²)N³ > 120 for N ≥ 6. The metallic conductor is m²+4. **They meet at exactly one m: the golden.** m²+4 = 3 has no integer solution; m²+4 = 5 ⟺ m = 1. **So the golden is the unique metallic grammar whose own-conductor shadow IS a McKay group**, supplying the uniqueness B666 asserted but never computed. **It survives what killed B995 (rarity implies separation) and B996 (genericity at a fixed modulus) because there is no base rate to attack — a uniqueness over an infinite family.** **Limits: the end reached is E₈ not E₆; the conductor identification is inferred from two points; Tier 1, none of the 19 numbers.** Reached by following the owner's redirection from manifolds to **grammars**.
