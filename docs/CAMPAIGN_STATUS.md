...
**LATEST — B1003 (2026-08-09): THE AXIOM CHAIN COSTS EXACTLY TWO AXIOMS, and both alternatives are named objects.** Phase 1 item 2. **Correcting B998, which is this seat's:** the F2/F8 COMPUTATIONS existed all along — B749 has all seven forks — **the gap was the LOCKS**, now written. **F2 = ROBUST:** the periodic family has **pseudo-Anosov count 0**, so allowing periodicity *loses the hyperbolic carrier* — **A2 selects the destination.** **F8 = GEOMETRY-NECESSARY:** four pre-registered witnesses fail exactly, **x²+3 irreducible over ℚ(√5)** — the combinatorial carrier sees **only hearing**, and **ℚ(√−3) is bought at geometrization and nowhere earlier**, so the being/hearing split is **forced by the step into geometry**. **Five ROBUST, two FRAGILE — F5 orientation and F6 the puncture** — and per P019 a FRAGILE fork *prices* the chain rather than breaking it. **Phase 1: 3 of 4 done; only the relay dispositions remain.**
