...
**LATEST (2026-08-07): we went back and audited our own bragging, and it did not fully survive.** Two days ago a seal invented a rule — no claiming to be first until the literature sweep reaches a named standard — and the arc that invented it obeyed it. But an older arc, banked the same week, had already claimed a first on a sweep nobody had recorded the depth of. A gate that only binds future work is not a gate, so the same adversarial panel was pointed backwards. It found prior art: not enough to kill the result, but enough to kill the sentence. The precedent we had quoted for ourselves was wrong — the published record reaches thirteen decimal places, not the ten we asserted, so our margin is twelve places rather than fifteen. Our phrase "on any hyperbolic 3-manifold" was wrong in a second, independent way. And an escape route existed: all the high-precision prior work is on orbifolds, so a strict reading of "manifold" would have saved us on a technicality. **We declined it.** What is banked now is a comparative fact instead of a boast — twenty-five decimal places against a published thirteen, on a cusp form, not inherited from the parent group — with the non-inheritance verified here from Humbert's volume formula rather than taken on the panel's word, and with its own limit stated: that check is a screen, not a proof. The mathematics never moved. Only the sentence around it did.
