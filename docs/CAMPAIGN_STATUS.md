...
choice of face; the two-27-VEV route is B955/B962/B964's (orbit K, B969); the triplet's zero Yukawa is located:
the 4d family tensor on H¹(Q; 3_ρ) is symmetric and on m004 the triplet is **one class**. The count's only
surviving route (h¹ = 3) is also the Yukawa's; both need a closing with an H³ (B1043's door). E68 typed. Price
unchanged (14 rows / 11 sources).
...
(residual 10⁻⁶³); **the transport computed** — every surviving gauge algebra named (0, 0, 0, u(1)⁴), the 27
has no slot on Q in either transport, **the SM algebra is not a centralizer in e₆**, and the object's one
unitary flat connection is a 4d N=1 U(1)⁴ vector-like theory of 24 chirals. JOIN 1: refuted for every h¹
reading, blocked by a named object (three E₇ points on a conical G₂ geometry, permuted by 2T/Q₈). The chain
assembled: `docs/THE_ASSEMBLY_2026-09-06.md`. Price unchanged at 14.
