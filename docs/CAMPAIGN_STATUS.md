...
**LATEST — B993/B994 (2026-08-09): the cornerstone is GENERIC, the cascade's endpoint is NOT the rule's doing.** Verified here: **32.8% of one-cusped census manifolds carry m004's own surjection count onto 2T**, and **Reid-uniqueness is consumed at zero steps** — so the derivation is a theorem about the **class** ℚ(√−3) plus a theorem about E₆, joined by a generic fact. **Object-specificity, if it exists, lives at the GROUP level** (B803's own addendum). Separately, the never-asked rule-variation test came back **favourable**: **all six** registerable-respecting chains end at the SM, so the **assumed** half of B861's rule carries no load — **but registerability IS chirality, which the object provably does not supply**. Honest statement: *given E₆ and given chirality, six paths land on the SM* — **REPRODUCED, Tier 1, none of the 19 numbers**. Also written: **`docs/WHAT_WOULD_COUNT.md`**, the programme's own two-outcome criterion — five tiers, three grades, four falsifiers, and the twelve distinct things called "the wall".
