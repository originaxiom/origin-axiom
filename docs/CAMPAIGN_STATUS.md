...
**LATEST — B1002 (2026-08-09): Phase 1 item 1 discharged, and it STRENGTHENS B997.** The owed conductor check is answered from a **banked law**, not from matching values — and the corpus uses *conductor* for **two different quantities in adjacent laws** (B675's cusp-order conductor: golden 4, silver 2; B666's shadow modulus: golden 5, silver 8). **B997 used the shadow, and m²+4 reproduces it exactly.** **B675 supplies the mechanism:** the reduction is an **isomorphism ⟺ conductor coprime to shadow** — golden gcd(4,5)=1 → iso, silver gcd(2,8)=2 → ramified, **which is why 2O sits one quotient down.** So the golden's shadow is the McKay group for **two independent reasons**. B666 stated the fact, B675 the mechanism, B997 the uniqueness — **none cited the other two.** Terminology collision registered.
