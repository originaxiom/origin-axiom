...
**LATEST — B995 + L146 (2026-08-09): the census returned A and its own control voided the instrument.** Sealed before compute with a declared prior of B; two invariants passed, and the post-hoc control showed **rarity implies separation** against a five-member set (P ≈ 99.5% and 79.9% by chance) and that **19.3% of manifolds are unique** in their Alexander polynomial — so uniqueness is a 1-in-5 event, not 1-in-1000. **Sealed A and adjudicated non-discriminating are both recorded**; the instrument failed, the question is untouched. **The owner's question supplied the fix:** there is a common base — **Σ₁,₁ with SL(2,ℤ)** — carrying the four letters, the object, B277's 4d lift, B150's identity of the trace-map action with class-S S-duality, and the twelfth face; **E₆ is not on it**, arriving from the class-level trace field. **Two bases, treated as one** — and since B993 placed object-specificity at the **group** level, that base **is** the open address. **L146:** is m004 distinguished among once-punctured-torus bundles?
