...
| T-θTANGENT | the manifold's amphichiral involution IS θ on the tangent, canonically: the sl₂-commutant of Aut(𝔢₆) is {1,θ}, θ = the E₆→F₄ folding (Out E₆ = ℤ/2). Gate B settled fact. | B521 (audit B501) | `test_b521.py::test_gateB_theta_is_out_e6` | DERIVABLE — Out(E₆)=ℤ/2 classical (Dynkin diagram automorphism); the geometric identification is the program's |
