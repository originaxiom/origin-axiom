...
| T-FORCEDNESS | The forcedness census (B1123, checker-gated): **39 of 43 chain-links are FORCED**; the stretch C6→C17 (the knot → the algebra) is **axiom-free**; the four axioms are C3/C4/C5 (the genesis frame) + C18 (the observer's closings, after the algebra). E₆ is not chosen — it is **what level 15 = 3·5 factors into** (CRT + McKay: 2T→E₆, 2I→E₈). Regenerated from `docs/THEOREM_LEDGER.md` by `scripts/checks/forcedness_census.py`, which FAILS non-vacuously on drift | B1123 | `tests/test_b1123_forcedness.py` | **NEEDS-LIT**: forcedness as a distributed-then-consolidated invariant; the doorway as three theorems, not a selection |
