...
> Last generated: 2026-07-03 from 351 frontier probes.
...
THE RECURRENCE ATLAS -- context card
  corpus: 351 frontier probes; status {'banked': 238, 'dead': 57, 'open': 42, 'dormant': 14}
  the ONE conserved first integral: kappa (recurs 98x, 27%) -- genuine unity, MUST recur
  top recurring motifs: firewall(232), golden(213), trace_map(202), metallic(192), figure_eight(186), eisenstein(178)
  recurrence is: structural-invariant 1290 mentions | conserved-integral 98 | TOOL 202
  the honest split: the trace-map TOOL is in 202 probes (57%) = method/selection-effect, NOT unity; only kappa is a forced first integral
...
|---|---|---|---|---|---|---|
| firewall | 232 | 66% | structure | structural | meta | the firewall / structural theorem / form-not-values |
| golden | 213 | 60% | arithmetic | structural | arithmetic | the golden end: Q(sqrt5), phi, E8, 2I |
| trace_map | 202 | 57% | dynamics | tool | dynamics | the trace map / Dehn-twist words / monodromy / substitution -- the METHOD |
| metallic | 192 | 54% | structure | structural | arithmetic | the metallic family lambda_m tower (golden/silver/bronze) |
| figure_eight | 186 | 52% | object | no | topology | the simplest hyperbolic knot; the carrier object |
| eisenstein | 178 | 50% | arithmetic | structural | arithmetic | the Eisenstein end: Q(sqrt-3), omega, E6, 2T |
| amphichiral_cp | 102 | 29% | symmetry | structural | topology | amphichirality / the CP sign +-pi/6 / CS=0 |
| kappa | 98 | 27% | invariant | first-integral | dynamics | the conserved commutator trace kappa = tr[a,b] = the Suto invariant |
| torsion | 96 | 27% | arithmetic | structural | arithmetic | the (Z/4)^2 congruence torsion / Alexander polynomial |
| wrt_quantum | 87 | 24% | quantum | no | quantum | the WRT / colored-Jones / modular quantum invariants |
| dickson_tower | 78 | 22% | structure | structural | representation | the Dickson tower rho_n / degree=rank / the det=-1 parity |
| z3_generation | 68 | 19% | symmetry | structural | arithmetic | the generation Z/3 (deck / commensurator / omega-circulant) |
| symplectic | 65 | 18% | structure | structural | geometry | the Goldman symplectic / Neumann-Zagier pairing |
| lorentzian | 59 | 16% | physics-bridge | no | physics | the Lorentzian / signature / spacetime bridge |
...
- **Genuine unity:** the one conserved **first integral** `κ = tr[a,b]` recurs in **98** probes (27%). A first integral is *conserved by the trace map ∀m* (K001/K007), so it **must** recur — this recurrence is forced, not chosen.
- **Structural invariants** (the two ends, ω, the Dickson parity, …): **1290** mentions — invariants of the object's transforms.
- **The hammer (selection effect):** the trace-map **tool** appears in **202** probes (57%). This recurrence is *because it is our method* — it is **not** evidence of unity. The atlas keeps this separate on purpose (verify-don't-trust).
...
|---|---|---|---|
| cancellation | 20 | golden | trace_map(15), golden(15), figure_eight(13), eisenstein(13) |
...
| observable | 5 | firewall | firewall(5), metallic(3), eisenstein(3), golden(3) |
