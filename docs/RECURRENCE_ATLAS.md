...
> Last generated: 2026-08-12 from 959 frontier probes.
...
THE RECURRENCE ATLAS -- context card
  corpus: 959 frontier probes; status {'open': 60, 'banked': 629, 'dead': 251, 'dormant': 19}
  the ONE conserved first integral: kappa (recurs 224x, 23%) -- genuine unity, MUST recur
  top recurring motifs: golden(547), firewall(541), eisenstein(506), figure_eight(404), metallic(396), trace_map(373)
  recurrence is: structural-invariant 3222 mentions | conserved-integral 224 | TOOL 373
  the honest split: the trace-map TOOL is in 373 probes (38%) = method/selection-effect, NOT unity; only kappa is a forced first integral
...
| firewall | 541 | 56% | structure | structural | meta | the firewall / structural theorem / form-not-values |
...
| trace_map | 373 | 38% | dynamics | tool | dynamics | the trace map / Dehn-twist words / monodromy / substitution -- the METHOD |
| amphichiral_cp | 322 | 33% | symmetry | structural | topology | amphichirality / the CP sign +-pi/6 / CS=0 |
...
| kappa | 224 | 23% | invariant | first-integral | dynamics | the conserved commutator trace kappa = tr[a,b] = the Suto invariant |
| z3_generation | 217 | 22% | symmetry | structural | arithmetic | the generation Z/3 (deck / commensurator / omega-circulant) |
...
- **Genuine unity:** the one conserved **first integral** `κ = tr[a,b]` recurs in **224** probes (23%). A first integral is *conserved by the trace map ∀m* (K001/K007), so it **must** recur — this recurrence is forced, not chosen.
- **Structural invariants** (the two ends, ω, the Dickson parity, …): **3222** mentions — invariants of the object's transforms.
- **The hammer (selection effect):** the trace-map **tool** appears in **373** probes (38%). This recurrence is *because it is our method* — it is **not** evidence of unity. The atlas keeps this separate on purpose (verify-don't-trust).
...
| measure | 90 | golden | golden(49), firewall(45), eisenstein(44), figure_eight(36) |
