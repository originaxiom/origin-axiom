...
> Last generated: 2026-08-19 from 991 frontier probes.
...
THE RECURRENCE ATLAS -- context card
  corpus: 991 frontier probes; status {'open': 64, 'banked': 649, 'dead': 257, 'dormant': 21}
  the ONE conserved first integral: kappa (recurs 227x, 22%) -- genuine unity, MUST recur
  top recurring motifs: golden(562), firewall(549), eisenstein(522), figure_eight(412), metallic(400), trace_map(376)
  recurrence is: structural-invariant 3294 mentions | conserved-integral 227 | TOOL 376
  the honest split: the trace-map TOOL is in 376 probes (37%) = method/selection-effect, NOT unity; only kappa is a forced first integral
...
|---|---|---|---|---|---|---|
| golden | 562 | 56% | arithmetic | structural | arithmetic | the golden end: Q(sqrt5), phi, E8, 2I |
...
| figure_eight | 412 | 41% | object | no | topology | the simplest hyperbolic knot; the carrier object |
| metallic | 400 | 40% | structure | structural | arithmetic | the metallic family lambda_m tower (golden/silver/bronze) |
| trace_map | 376 | 37% | dynamics | tool | dynamics | the trace map / Dehn-twist words / monodromy / substitution -- the METHOD |
...
| kappa | 227 | 22% | invariant | first-integral | dynamics | the conserved commutator trace kappa = tr[a,b] = the Suto invariant |
| z3_generation | 225 | 22% | symmetry | structural | arithmetic | the generation Z/3 (deck / commensurator / omega-circulant) |
| lorentzian | 179 | 18% | physics-bridge | no | physics | the Lorentzian / signature / spacetime bridge |
...
- **Genuine unity:** the one conserved **first integral** `κ = tr[a,b]` recurs in **227** probes (22%). A first integral is *conserved by the trace map ∀m* (K001/K007), so it **must** recur — this recurrence is forced, not chosen.
- **Structural invariants** (the two ends, ω, the Dickson parity, …): **3294** mentions — invariants of the object's transforms.
- **The hammer (selection effect):** the trace-map **tool** appears in **376** probes (37%). This recurrence is *because it is our method* — it is **not** evidence of unity. The atlas keeps this separate on purpose (verify-don't-trust).
...
| spacetime_3p1 | 116 | golden | golden(69), eisenstein(67), figure_eight(66), trace_map(54) |
| observable | 37 | golden | golden(28), torsion(19), metallic(19), eisenstein(17) |
