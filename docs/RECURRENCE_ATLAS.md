...
> Last generated: 2026-08-11 from 960 frontier probes.
...
THE RECURRENCE ATLAS -- context card
  corpus: 960 frontier probes; status {'open': 62, 'banked': 630, 'dead': 248, 'dormant': 20}
  the ONE conserved first integral: kappa (recurs 229x, 23%) -- genuine unity, MUST recur
  top recurring motifs: golden(550), firewall(543), eisenstein(505), figure_eight(409), metallic(404), trace_map(385)
  recurrence is: structural-invariant 3241 mentions | conserved-integral 229 | TOOL 385
  the honest split: the trace-map TOOL is in 385 probes (40%) = method/selection-effect, NOT unity; only kappa is a forced first integral
...
|---|---|---|---|---|---|---|
| golden | 550 | 57% | arithmetic | structural | arithmetic | the golden end: Q(sqrt5), phi, E8, 2I |
...
| metallic | 404 | 42% | structure | structural | arithmetic | the metallic family lambda_m tower (golden/silver/bronze) |
| trace_map | 385 | 40% | dynamics | tool | dynamics | the trace map / Dehn-twist words / monodromy / substitution -- the METHOD |
...
| kappa | 229 | 23% | invariant | first-integral | dynamics | the conserved commutator trace kappa = tr[a,b] = the Suto invariant |
...
| dickson_tower | 111 | 11% | structure | structural | representation | the Dickson tower rho_n / degree=rank / the det=-1 parity |
...
- **Genuine unity:** the one conserved **first integral** `κ = tr[a,b]` recurs in **229** probes (23%). A first integral is *conserved by the trace map ∀m* (K001/K007), so it **must** recur — this recurrence is forced, not chosen.
- **Structural invariants** (the two ends, ω, the Dickson parity, …): **3241** mentions — invariants of the object's transforms.
- **The hammer (selection effect):** the trace-map **tool** appears in **385** probes (40%). This recurrence is *because it is our method* — it is **not** evidence of unity. The atlas keeps this separate on purpose (verify-don't-trust).
...
| measure | 93 | golden | golden(50), eisenstein(46), firewall(46), figure_eight(37) |
