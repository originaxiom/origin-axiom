...
> Last generated: 2026-09-12 from 1218 frontier probes.
...
THE RECURRENCE ATLAS -- context card
  corpus: 1218 frontier probes; status {'open': 70, 'banked': 799, 'dead': 325, 'dormant': 24}
  the ONE conserved first integral: kappa (recurs 271x, 22%) -- genuine unity, MUST recur
  top recurring motifs: eisenstein(683), golden(676), firewall(635), figure_eight(525), amphichiral_cp(456), metallic(449)
  recurrence is: structural-invariant 4822 mentions | conserved-integral 271 | TOOL 441
  the honest split: the trace-map TOOL is in 441 probes (36%) = method/selection-effect, NOT unity; only kappa is a forced first integral
...
|---|---|---|---|---|---|---|
| eisenstein | 683 | 56% | arithmetic | structural | arithmetic | the Eisenstein end: Q(sqrt-3), omega, E6, 2T |
| golden | 676 | 55% | arithmetic | structural | arithmetic | the golden end: Q(sqrt5), phi, E8, 2I |
| firewall | 635 | 52% | structure | structural | meta | the firewall / structural theorem / form-not-values |
| figure_eight | 525 | 43% | object | no | topology | the simplest hyperbolic knot; the carrier object |
| amphichiral_cp | 456 | 37% | symmetry | structural | topology | amphichirality / the CP sign +-pi/6 / CS=0 |
| metallic | 449 | 36% | structure | structural | arithmetic | the metallic family lambda_m tower (golden/silver/bronze) |
| trace_map | 441 | 36% | dynamics | tool | dynamics | the trace map / Dehn-twist words / monodromy / substitution -- the METHOD |
...
| closing | 378 | 31% | question | structural | topology | what closing the open object supplies and costs -- Dehn filling, the seam, the constitutive closure (B286/B287/B294) |
...
| measurement | 293 | 24% | question | no | dynamics | collapse, decoherence, the measurement postulate as a structural shape rather than an added axiom |
| wrt_quantum | 278 | 22% | quantum | no | quantum | the WRT / colored-Jones / modular quantum invariants |
| kappa | 271 | 22% | invariant | first-integral | dynamics | the conserved commutator trace kappa = tr[a,b] = the Suto invariant |
...
| monoid | 97 | 7% | question | structural | dynamics | End(F2) beyond the units: the four Hopfian-det strata, the non-invertible verbs the programme has never computed |
...
| markov_cubic | 79 | 6% | invariant | structural | topology | the trace-triple SURFACE the trace map acts on: the Markov/Fricke cubic x^2+y^2+z^2-xyz=c and SL(2,Z) triples (tr A, tr B, tr AB). Deliberately EXCLUDES the bare phrase 'character variety', which B824 measured at 13.8%% of the corpus -- this programme's subject matter, not a topic within it |
| action | 75 | 6% | question | structural | dynamics | is there a least-action principle, and at which step -- the variational structure the object supplies rather than one lifted onto it (B6's derived potential, B21's bracket, B1341's theorem that the half step is generated by no Lagrangian and the monodromy is) |
...
- **Genuine unity:** the one conserved **first integral** `κ = tr[a,b]` recurs in **271** probes (22%). A first integral is *conserved by the trace map ∀m* (K001/K007), so it **must** recur — this recurrence is forced, not chosen.
- **Structural invariants** (the two ends, ω, the Dickson parity, …): **4822** mentions — invariants of the object's transforms.
- **The hammer (selection effect):** the trace-map **tool** appears in **441** probes (36%). This recurrence is *because it is our method* — it is **not** evidence of unity. The atlas keeps this separate on purpose (verify-don't-trust).
