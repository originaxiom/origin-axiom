...
> Last generated: 2026-09-05 from 1158 frontier probes.
...
THE RECURRENCE ATLAS -- context card
  corpus: 1158 frontier probes; status {'open': 67, 'banked': 759, 'dead': 310, 'dormant': 22}
  the ONE conserved first integral: kappa (recurs 259x, 22%) -- genuine unity, MUST recur
  top recurring motifs: golden(649), eisenstein(637), firewall(625), figure_eight(489), metallic(431), trace_map(421)
  recurrence is: structural-invariant 4450 mentions | conserved-integral 259 | TOOL 421
  the honest split: the trace-map TOOL is in 421 probes (36%) = method/selection-effect, NOT unity; only kappa is a forced first integral
...
|---|---|---|---|---|---|---|
| golden | 649 | 56% | arithmetic | structural | arithmetic | the golden end: Q(sqrt5), phi, E8, 2I |
| eisenstein | 637 | 55% | arithmetic | structural | arithmetic | the Eisenstein end: Q(sqrt-3), omega, E6, 2T |
| firewall | 625 | 53% | structure | structural | meta | the firewall / structural theorem / form-not-values |
...
| trace_map | 421 | 36% | dynamics | tool | dynamics | the trace map / Dehn-twist words / monodromy / substitution -- the METHOD |
| amphichiral_cp | 421 | 36% | symmetry | structural | topology | amphichirality / the CP sign +-pi/6 / CS=0 |
| coupling | 362 | 31% | question | no | physics | the observer/object interface: what the coupling supplies that neither side has alone (the listener map, the pair, the relational bit) |
| torsion | 354 | 30% | arithmetic | structural | arithmetic | the (Z/4)^2 congruence torsion / Alexander polynomial |
| closing | 342 | 29% | question | structural | topology | what closing the open object supplies and costs -- Dehn filling, the seam, the constitutive closure (B286/B287/B294) |
...
| z3_generation | 267 | 23% | symmetry | structural | arithmetic | the generation Z/3 (deck / commensurator / omega-circulant) |
| kappa | 259 | 22% | invariant | first-integral | dynamics | the conserved commutator trace kappa = tr[a,b] = the Suto invariant |
...
- **Genuine unity:** the one conserved **first integral** `κ = tr[a,b]` recurs in **259** probes (22%). A first integral is *conserved by the trace map ∀m* (K001/K007), so it **must** recur — this recurrence is forced, not chosen.
- **Structural invariants** (the two ends, ω, the Dickson parity, …): **4450** mentions — invariants of the object's transforms.
- **The hammer (selection effect):** the trace-map **tool** appears in **421** probes (36%). This recurrence is *because it is our method* — it is **not** evidence of unity. The atlas keeps this separate on purpose (verify-don't-trust).
...
| measure | 114 | eisenstein | eisenstein(60), golden(59), firewall(57), measurement(49) |
