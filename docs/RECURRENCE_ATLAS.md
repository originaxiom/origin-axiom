...
> Last generated: 2026-07-31 from 757 frontier probes.
...
THE RECURRENCE ATLAS -- context card
  corpus: 757 frontier probes; status {'banked': 502, 'dead': 184, 'open': 53, 'dormant': 18}
  the ONE conserved first integral: kappa (recurs 196x, 25%) -- genuine unity, MUST recur
  top recurring motifs: golden(483), firewall(478), eisenstein(407), figure_eight(359), metallic(349), trace_map(340)
  recurrence is: structural-invariant 2779 mentions | conserved-integral 196 | TOOL 340
  the honest split: the trace-map TOOL is in 340 probes (44%) = method/selection-effect, NOT unity; only kappa is a forced first integral
...
|---|---|---|---|---|---|---|
| golden | 483 | 63% | arithmetic | structural | arithmetic | the golden end: Q(sqrt5), phi, E8, 2I |
...
| metallic | 349 | 46% | structure | structural | arithmetic | the metallic family lambda_m tower (golden/silver/bronze) |
| trace_map | 340 | 44% | dynamics | tool | dynamics | the trace map / Dehn-twist words / monodromy / substitution -- the METHOD |
...
| kappa | 196 | 25% | invariant | first-integral | dynamics | the conserved commutator trace kappa = tr[a,b] = the Suto invariant |
...
- **Genuine unity:** the one conserved **first integral** `κ = tr[a,b]` recurs in **196** probes (25%). A first integral is *conserved by the trace map ∀m* (K001/K007), so it **must** recur — this recurrence is forced, not chosen.
- **Structural invariants** (the two ends, ω, the Dickson parity, …): **2779** mentions — invariants of the object's transforms.
- **The hammer (selection effect):** the trace-map **tool** appears in **340** probes (44%). This recurrence is *because it is our method* — it is **not** evidence of unity. The atlas keeps this separate on purpose (verify-don't-trust).
