...
> Last generated: 2026-09-12 from 1219 frontier probes.
...
THE RECURRENCE ATLAS -- context card
  corpus: 1219 frontier probes; status {'open': 70, 'banked': 800, 'dead': 325, 'dormant': 24}
  the ONE conserved first integral: kappa (recurs 272x, 22%) -- genuine unity, MUST recur
  top recurring motifs: eisenstein(684), golden(677), firewall(635), figure_eight(526), amphichiral_cp(456), metallic(450)
  recurrence is: structural-invariant 4829 mentions | conserved-integral 272 | TOOL 442
  the honest split: the trace-map TOOL is in 442 probes (36%) = method/selection-effect, NOT unity; only kappa is a forced first integral
...
|---|---|---|---|---|---|---|
| eisenstein | 684 | 56% | arithmetic | structural | arithmetic | the Eisenstein end: Q(sqrt-3), omega, E6, 2T |
| golden | 677 | 55% | arithmetic | structural | arithmetic | the golden end: Q(sqrt5), phi, E8, 2I |
| firewall | 635 | 52% | structure | structural | meta | the firewall / structural theorem / form-not-values |
| figure_eight | 526 | 43% | object | no | topology | the simplest hyperbolic knot; the carrier object |
| amphichiral_cp | 456 | 37% | symmetry | structural | topology | amphichirality / the CP sign +-pi/6 / CS=0 |
| metallic | 450 | 36% | structure | structural | arithmetic | the metallic family lambda_m tower (golden/silver/bronze) |
| trace_map | 442 | 36% | dynamics | tool | dynamics | the trace map / Dehn-twist words / monodromy / substitution -- the METHOD |
...
| closing | 379 | 31% | question | structural | topology | what closing the open object supplies and costs -- Dehn filling, the seam, the constitutive closure (B286/B287/B294) |
| torsion | 374 | 30% | arithmetic | structural | arithmetic | the (Z/4)^2 congruence torsion / Alexander polynomial |
| z3_generation | 309 | 25% | symmetry | structural | arithmetic | the generation Z/3 (deck / commensurator / omega-circulant) |
...
| kappa | 272 | 22% | invariant | first-integral | dynamics | the conserved commutator trace kappa = tr[a,b] = the Suto invariant |
...
- **Genuine unity:** the one conserved **first integral** `κ = tr[a,b]` recurs in **272** probes (22%). A first integral is *conserved by the trace map ∀m* (K001/K007), so it **must** recur — this recurrence is forced, not chosen.
- **Structural invariants** (the two ends, ω, the Dickson parity, …): **4829** mentions — invariants of the object's transforms.
- **The hammer (selection effect):** the trace-map **tool** appears in **442** probes (36%). This recurrence is *because it is our method* — it is **not** evidence of unity. The atlas keeps this separate on purpose (verify-don't-trust).
