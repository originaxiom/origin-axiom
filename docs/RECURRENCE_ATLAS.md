...
> Last generated: 2026-09-12 from 1220 frontier probes.
...
THE RECURRENCE ATLAS -- context card
  corpus: 1220 frontier probes; status {'open': 70, 'banked': 800, 'dead': 326, 'dormant': 24}
  the ONE conserved first integral: kappa (recurs 273x, 22%) -- genuine unity, MUST recur
  top recurring motifs: eisenstein(685), golden(677), firewall(635), figure_eight(527), amphichiral_cp(456), metallic(451)
  recurrence is: structural-invariant 4834 mentions | conserved-integral 273 | TOOL 443
  the honest split: the trace-map TOOL is in 443 probes (36%) = method/selection-effect, NOT unity; only kappa is a forced first integral
...
|---|---|---|---|---|---|---|
| eisenstein | 685 | 56% | arithmetic | structural | arithmetic | the Eisenstein end: Q(sqrt-3), omega, E6, 2T |
...
| figure_eight | 527 | 43% | object | no | topology | the simplest hyperbolic knot; the carrier object |
| amphichiral_cp | 456 | 37% | symmetry | structural | topology | amphichirality / the CP sign +-pi/6 / CS=0 |
| metallic | 451 | 36% | structure | structural | arithmetic | the metallic family lambda_m tower (golden/silver/bronze) |
| trace_map | 443 | 36% | dynamics | tool | dynamics | the trace map / Dehn-twist words / monodromy / substitution -- the METHOD |
...
| z3_generation | 310 | 25% | symmetry | structural | arithmetic | the generation Z/3 (deck / commensurator / omega-circulant) |
...
| kappa | 273 | 22% | invariant | first-integral | dynamics | the conserved commutator trace kappa = tr[a,b] = the Suto invariant |
...
| monoid | 99 | 8% | question | structural | dynamics | End(F2) beyond the units: the four Hopfian-det strata, the non-invertible verbs the programme has never computed |
...
- **Genuine unity:** the one conserved **first integral** `κ = tr[a,b]` recurs in **273** probes (22%). A first integral is *conserved by the trace map ∀m* (K001/K007), so it **must** recur — this recurrence is forced, not chosen.
- **Structural invariants** (the two ends, ω, the Dickson parity, …): **4834** mentions — invariants of the object's transforms.
- **The hammer (selection effect):** the trace-map **tool** appears in **443** probes (36%). This recurrence is *because it is our method* — it is **not** evidence of unity. The atlas keeps this separate on purpose (verify-don't-trust).
