...
> Last generated: 2026-09-06 from 1173 frontier probes.
...
THE RECURRENCE ATLAS -- context card
  corpus: 1173 frontier probes; status {'open': 67, 'banked': 769, 'dead': 314, 'dormant': 23}
  the ONE conserved first integral: kappa (recurs 262x, 22%) -- genuine unity, MUST recur
  top recurring motifs: golden(654), eisenstein(649), firewall(626), figure_eight(494), metallic(433), amphichiral_cp(429)
  recurrence is: structural-invariant 4508 mentions | conserved-integral 262 | TOOL 424
  the honest split: the trace-map TOOL is in 424 probes (36%) = method/selection-effect, NOT unity; only kappa is a forced first integral
...
|---|---|---|---|---|---|---|
| golden | 654 | 55% | arithmetic | structural | arithmetic | the golden end: Q(sqrt5), phi, E8, 2I |
| eisenstein | 649 | 55% | arithmetic | structural | arithmetic | the Eisenstein end: Q(sqrt-3), omega, E6, 2T |
...
| trace_map | 424 | 36% | dynamics | tool | dynamics | the trace map / Dehn-twist words / monodromy / substitution -- the METHOD |
...
| z3_generation | 278 | 23% | symmetry | structural | arithmetic | the generation Z/3 (deck / commensurator / omega-circulant) |
| wrt_quantum | 275 | 23% | quantum | no | quantum | the WRT / colored-Jones / modular quantum invariants |
| kappa | 262 | 22% | invariant | first-integral | dynamics | the conserved commutator trace kappa = tr[a,b] = the Suto invariant |
...
- **Genuine unity:** the one conserved **first integral** `κ = tr[a,b]` recurs in **262** probes (22%). A first integral is *conserved by the trace map ∀m* (K001/K007), so it **must** recur — this recurrence is forced, not chosen.
- **Structural invariants** (the two ends, ω, the Dickson parity, …): **4508** mentions — invariants of the object's transforms.
- **The hammer (selection effect):** the trace-map **tool** appears in **424** probes (36%). This recurrence is *because it is our method* — it is **not** evidence of unity. The atlas keeps this separate on purpose (verify-don't-trust).
...
| measure | 116 | golden | golden(61), eisenstein(61), firewall(57), measurement(50) |
...
| spacetime_3p1 | 135 | eisenstein | eisenstein(80), golden(78), figure_eight(73), closing(64) |
