...
> Last generated: 2026-09-02 from 1144 frontier probes.
...
THE RECURRENCE ATLAS -- context card
  corpus: 1144 frontier probes; status {'open': 64, 'banked': 754, 'dead': 304, 'dormant': 22}
  the ONE conserved first integral: kappa (recurs 254x, 22%) -- genuine unity, MUST recur
  top recurring motifs: golden(642), eisenstein(626), firewall(619), figure_eight(480), metallic(428), amphichiral_cp(418)
  recurrence is: structural-invariant 3756 mentions | conserved-integral 254 | TOOL 414
  the honest split: the trace-map TOOL is in 414 probes (36%) = method/selection-effect, NOT unity; only kappa is a forced first integral
...
| figure_eight | 480 | 41% | object | no | topology | the simplest hyperbolic knot; the carrier object |
...
| trace_map | 414 | 36% | dynamics | tool | dynamics | the trace map / Dehn-twist words / monodromy / substitution -- the METHOD |
| torsion | 348 | 30% | arithmetic | structural | arithmetic | the (Z/4)^2 congruence torsion / Alexander polynomial |
| wrt_quantum | 270 | 23% | quantum | no | quantum | the WRT / colored-Jones / modular quantum invariants |
| z3_generation | 260 | 22% | symmetry | structural | arithmetic | the generation Z/3 (deck / commensurator / omega-circulant) |
...
- **Structural invariants** (the two ends, ω, the Dickson parity, …): **3756** mentions — invariants of the object's transforms.
- **The hammer (selection effect):** the trace-map **tool** appears in **414** probes (36%). This recurrence is *because it is our method* — it is **not** evidence of unity. The atlas keeps this separate on purpose (verify-don't-trust).
