...
| **X10** | **L138's scope** — it fired, but **a canonical orbit is not a canonical VEV**. **SHARPENED 2026-08-10 (B1017): this rung is now THE UNSOURCED FIFTH INPUT** — the recount (cc3's `tau_vs_singlet`, verified and adopted) retracted B963's compete-corollary, so the rank-closing VEV is a **fifth resource with no supplier**: B766's three torsor bits are all committed. Open within it: whether the VEV's *magnitude* folds into the ℝ₊ (shared) or is new | the only live route to a distinguished VEV — and now **the interface's single remaining un-derived slot** (B990's two routes still the only two) |
...
| # | rung | the mechanism |
|---|---|---|
| **X19** | rank reduction | centralizers contain a maximal torus ⟹ **measurement is rank-preserving** (B952); all routes closed (B959, B960). **Re-sorted 2026-08-10 (B1013): a SPECIFICATION, not a wall** — an input in every framework; and **B1017**: its closing is by VEVs, *not* by τ — the compete-corollary is retracted |
...
| **X33** | **THREE GENERATIONS — added 2026-08-11 (B1031). This rung was MISSING: the word "generation" appeared nowhere in this file, while 51 arcs speak to it.** By this ladder's own binding rule that made the honest answer *"not checked"* — the exact inversion of what the file exists to prevent | **THE POSITIVE, and it is real: three-ness is computed twice.** **B632** — `h¹(M; 27_ρ) = 3` **exactly** over ℚ(ω), a graded three-slot structure (spins 0, 4, 8) with a canonical invariant vev, **explicitly not blocked by B307**. **B897** (sealed, prereg committed before compute) — the 27 splits into **three 9-blocks**, `su(3)′` acting as a **native flavor symmetry** replicating fixed color×`su(2)′` types into triplets. **B335** sharpens what that three is: the generation ℤ/3 is a **deck transformation, hence an ISOMETRY**, so every real invariant — *any mass* — is **exactly degenerate** across the three sheets, and **a hierarchy is its breaking**. **THE MECHANISM THAT BOUNDS IT (why this is BOUNDED, not a HOLE): B307**, promoted to **P54** — a C₃ field is Galois ⟹ totally real, a hyperbolic invariant trace field always has a complex place ⟹ **no hyperbolic knot has a cyclic-cubic trace field**, so a symmetric three-generation triple is **arithmetically impossible in a single hyperbolic knot**. With **B298** (NEGATIVE: seven routes give multiplicity 1 or 2, never 3) that closes the single-object route **for all hyperbolic knots**; **B302** then locates the ℤ/3 in the **commensurator** `PGL(2,𝒪₋₃)` as a hidden symmetry — *locating it, not deriving it* — and **B414** kills the naive ℤ/3 reading on its privilege test. **THE GRADE, unchanged by this rung:** `THE_CLAIM` says **STRUCTURAL** — *"a named debt inside a closed proof"* — not DERIVED (which it reserves for the ℤ₆ form and hypercharge); `SM_SPECIFICATION_LEDGER` says ***"structural, count matches"*** with the orbit↔generation bijection marked **unverified**. **TWO THINGS A READER MUST NOT NET AGAINST EACH OTHER.** (i) **The circularity, in the record's own words:** the cascade principle is *maximal residual symmetry among **registerable** options*, and **registerable is DEFINED as "the 27's generation structure survives"** — B994: *"registerable = the generation stays chiral … the endpoint is forced by an input the object does not have."* Generation structure is therefore partly a **filter on** the cascade, not solely an output of it. (ii) **The typing wall (1′):** an alternating family tensor ⊗ the symmetric E₆ cubic = **zero coupling for identical families** — three slots exist; a Yukawa-type family tensor at that level does not. **TWO routes B307 leaves open — corrected 2026-08-11 by B1032; this rung's first draft said "the only one" and that was false.** B307 forbids a **trace-field** route in a single knot; it says nothing about either of these. **(a) The relational route** — B302's commensurator ℤ/3 → multiplicity. **No arc runs it end to end.** **(b) The across-breakings route, and it already has two SEALED cells:** **B885** computes the inter-breaking dictionary on the 27 (two structural laws, zeros at 10⁻²¹–10⁻²⁶); **B889** makes the vacuum→frame identification a **computed bijection** — the S₃ torsor of frames realized as three labeled lines in the 27; **B890** (sealed, and **banking AGAINST its disclosed prior of EQUAL**) finds the **foreign vacua DISTINCT in all three frames**, decisively above the sealed 10⁻⁶ band — *"the Galois unlabelability of algebraic labels does NOT translate into operational indistinguishability; the geometry breaks the S₃ symmetry operationally"*; **B891** (sealed) extends it to **matter** — *"a single observer registers three pairwise-distinguishable matter sectors"* on the one 27. **THE FENCE BOTH SEALED CELLS DECLARE, kept verbatim: registerable distinctness is not mechanism-hood** — the three 16s **overlap** on one 27 (not direct summands), and replication *with identical gauge behaviour* is what "generations" means. **So route (b) is the live one and it stops at a declared fence, not at a theorem.** **AND THE CONSOLIDATION STOPS EXACTLY WHERE THE SEALED RESULTS BEGIN:** the cluster's **foundation** is curated — B885's inter-breaking laws hold a `LAW_MAP` row and a `THEOREM_LEDGER` entry — while **everything built on it (B889's computed bijection and both sealed cells) is carried by no curated surface under any of six topic names**, reachable in the generated views only (B1032) |
