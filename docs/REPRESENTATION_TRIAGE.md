...
| B1332 | PROCESS | the isotropy question of B1331 is well-posed only because the domain forces `psi` cusp-trivial (`V` itself is not self-dual: an invariant form needs `psi^2 = 1`), which also hands over `t_0 = t_0* = t_1/2` for free; isotropy then verified exactly over `Q(zeta_12)` on three CHIRAL targets (s958, t12833, t12835), 18 sectors each, zero exceptions — **but six per manifold are vacuous**, a 1-dimensional subspace being isotropic for free under an alternating form. Repaired by the form-free version: `L_V u L_V` vanishes in every component of `H^2(T;W (x) W)` carrying an `SL2`-invariant functional (non-vacuous exactly where the form test was empty). The blanket claim `L_V u L_V = 0` is **false** — `Sym^2 (+) Sym^4` leaves a 2-dimensional survivor, entirely off-diagonal and so invisible to any duality. To surface with the chirality row when the proof lands. |
