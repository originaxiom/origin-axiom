...
| 4 dynamics | the audit seat's conditional 4d compact-E₆ theory (R4–R11) with declared inputs (§E); main has no derived 4d action. **But the object's own DISCRETE dynamics does have a least-action principle, and only at the double tick (B1341)**: the half-step trace map pulls the invariant area form back to −itself on *every* leaf, so by the discrete-Euler–Lagrange obstruction it is generated by no Lagrangian at all; the double step is, and the obstruction is the record's own genesis bit, and B448's half-monodromy matrix `C = [[1,1],[1,0]]` **is** `L·P` with `C² = L∘R` the figure-eight monodromy — so **the action exists at the monodromy and fails at its formal square root** | audit:R4–R11 (B1304), B448, B1341 | **the 4d action is still input, not derived** — one degree of freedom on a surface is not a field theory; what is no longer true is that the object supplies *no* variational structure |
