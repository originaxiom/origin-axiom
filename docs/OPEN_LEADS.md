...
**L204 status 2026-09-09 (B1352): CLOSED on every computed deformation of the object.** The fixed-vector locus along V₁₀ is computed (`frontier/B1352_the_fixed_vector_locus_along_v10`): from each genuine V₁₀ point the augmented Newton (damping 10⁻⁵⁰, where B1268's stage (b) never converged) reaches the locus in 15–20 quadratic steps, and the locus is the **Sp(8) family** — self-dual (defects 10⁻⁵³), cyclic 27, exactly three cusp-fixed weights (the Cartan directions of Λ²₀(8)), h¹(27) = h¹(27̄) = 3, N = 0 at 2000 bits. Exactly: V₁₀ is the only θ-odd direction tangent to the locus at first order (ℚ(ω)); every formal V₁₀ branch constructed loses the three cusp-fixed vectors at order 4 (order 2 generically; two primes), the same order at which the deformation stops being self-dual (the six trace defects scale as t⁴ between the genuine points at steps 0.02 and 0.04, five ratios 16.884). So there is no point with a cusp-fixed weight on a non-self-dual representation within reach: the disc-type ∂⁺ has no carrier on the object. What remains of L204 is formal and registered in B1352 §6 (whether a tuned formal branch keeps a fixed vector beyond order 4); the physical escape moves off the cusp to the isolated enhancement point (B1353).

**L204 status 2026-09-09 (B1354): the formal residue closed.** Formal branches tangent to V₁₀ that keep all three cusp-fixed vectors of ρ₀ do exist (`frontier/B1354_the_maximal_persistence`: the obstruction-class tower, Gröbner bases over two primes; the second-order class must drop V₄, V₈, V₁₆ and carry a definite V₆ component, then every order is affine and free; the climb reaches order 8 with Toeplitz kernel dimensions 3m) — and every such branch is formally self-dual through order 8, while the greedy branch (the Newton curve's model) leaves the self-dual locus at order 4 and loses the fixed vectors at the same order. Cusp-fixed weights and self-duality are one condition along V₁₀ at every order reached. L204 is closed on the object, formally and numerically; the bit is the closing's (O4, curved) or the observer's (O5).
