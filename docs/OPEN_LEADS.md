...
## L166 — RESOLVED IN STATUS 2026-08-20 (cc's W2 Krutelevich read, B1099): **the counter is NOT in the literature — it is FRONTIER MATHEMATICS.** The premise of my registration was that identifying the counting theorem is a **lookup**. It is not. Krutelevich (math/0411104) **Thm 14** classifies `E₆(ℤ)`-orbits of the integral 27 by **Smith normal form, NOT class groups**, and **Cor 16** gives transitivity at determinant `n` iff `n` is **squarefree** — our determinant is **5**, squarefree, so **the full-group integral orbit at our invariant is UNIQUE** and Route A's **coarse half closes POSITIVE**. But **no `K`-indexed integral-orbit count exists for `(E₆, 27)` or for the pair anywhere reached**; the `D₆/32` and `E₇/56` neighbours are logged **OPEN in the literature itself**, and Thorne's `E₆` AIT paper is the wrong grading. **The named frontier:** the Bhargava–Gross/Thorne **stabilizer-cohomology transfer** to `F₄` (or basepoint-fixed `E₆` on the pair) with cubic `K` as classifying data — **unproved anywhere**. **Honest label: NEEDS-SPECIALIST at the field's own level.** This is a *better* status than *owed*: **an absent theorem is a different object from an unfound one**, and B8085's refusal to assert the orbit count is retroactively the right call — the assertion was not merely unproved by us, it is **unproved by anyone**. **AND MY B8087 PAIR-SPACE READING IS NOW FORCED FROM THE LITERATURE SIDE:** Krutelevich **Prop 12(i)** — the full norm-preserving group is **transitive on every nonzero-norm level set over any field**, so **it never sees `K`** — hence `K`-indexed rational orbits *require* the basepoint-respecting group, i.e. **the pair `27 ⊕ 27̄`**. Two independent roads, one conclusion. *(This also settles the convention I flagged: `27 ⊕ 27̄`, with the conjugate, is correct and now justified rather than assumed.)* The registered text below stands as the record of the question as posed.

## L166 (as registered) — which class-group quantity actually counts the integral orbits

**Status: OPEN. This is the gap between what B8085 proved and what Route A needs.**

B8085 computed every candidate and found them all trivial — `h = 1` (banked), `h⁺ = 1` (new),
`|Cl/Cl²| = 1`, `|Cl/Cl³| = 1` — for `K = ℚ[x]/(x³−12x−5)`, with the unit signature map surjective
onto `{±1}³` at rank 3 of 3. That is a genuine result: **the arithmetic obstruction Route A was
expected to meet is absent, and the conclusion does not depend on resolving which quantity governs.**

**It is not the same statement as "the integral orbit count is 1."** In a Kato–Yukie / Bhargava-type
correspondence, integral orbits inside one rational orbit are counted by *some* class-group quantity
of `K`, and until the correct one is identified for **this** prehomogeneous space and **this**
lattice, the count is unknown — trivial candidates make it *likely*, not proved. B990's Route A asks
for a canonical point up to `G(ℤ)`; a likelihood is not a point.

**What would close it:** identify the counting theorem for the relevant space and lattice, state the
quantity it names, and check that it is among the four already computed — or compute it if not.

**Falsifier, both ways.** If the counter is one of the four, Route A's obstruction is *proved*
absent, not merely *not found*, and the orbit is canonical up to `G(ℤ)`. If the counter is a
quantity **not** on that list and it is non-trivial, Route A closes and B990's UNFAVOURABLE prior is
vindicated after all. **Either outcome is worth having; neither is currently in hand.**

**Do not upgrade B8085's wording while this is open** — it says the obstruction is absent, and that
is exactly and only what was computed.

## L167 — NON-ABELIAN HOLONOMY: the live hatch out of the rank wall (registered 2026-08-19)

**Status: OPEN, and it is now the only unclosed direction in Route B.**

Two independent arcs put the same wall in place. **B955** proved it structurally: a knot complement
has `H₁ = ℤ`, **cyclic**, so an abelian flat connection has cyclic image generated by one element,
whose centraliser in simply-connected `E₆` contains a maximal torus — **rank is preserved
necessarily, not generically.** **B8086** reached the same wall by exhaustive enumeration — all
`5⁶−1 = 15624` non-identity 5-torsion elements, all nine resulting rows, **every one rank 6** —
sharing **no step** with B955's argument.

**Two step-disjoint routes to one wall is the strongest form the negative can take**, and it is why
the wall should be treated as real rather than as an artefact of one proof's choices.

**But both assume the same thing.** B955 assumes abelian holonomy via `H₁ = ℤ` cyclic; B8086 assumes
it via torus elements. **Neither says anything about non-abelian flat connections**, where the image
is not cyclic and the centraliser argument does not run. **B955 closes the abelian instance of Route
B — not Route B.** The kill-graph record for B8086 carries this as the hatch explicitly rather than
leaving it implied.

**What would close it:** ask whether a non-abelian flat `E₆`-connection on the golden member has
centraliser of rank < 6 — the geometric representation is non-abelian and already in hand (B8082
computes its `H¹`), so the question is askable now, not blocked on new machinery.

**Falsifier, both ways.** If some non-abelian connection drops the rank to 4, Route B is open and
the VEV direction has a source. If rank-6 survives non-abelian holonomy too, the wall is not about
abelianness at all, **B990's structural gap stands undiminished**, and the honest reading is that
the object does not carry the direction — which is a result, not a failure.
