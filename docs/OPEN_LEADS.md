...
| **L16** | **Field-multiplicity gap-labeling rank** — for a quasicrystal built by a *combined substitution* of metallic seeds, is the gap-labeling / Fourier-module rank `1 + (#distinct quadratic fields ℚ(√squarefree(m²+4)))`? Rigorous pieces: single seed → rank 2; same-field seeds capped at 2 (a quadratic field is 2-dim); distinct fields → ℚ-independent `√d` (classical). **Sub-lead (number theory):** the Pell structure of which seeds share a field (`m²−d·a²=−4`; the ℚ(√5) seeds = odd-index Lucas `{1,4,11,29,…}`). | MATH | ★★ | **OPEN** — review-proposed, verified-in-part in-session. The rank formula is a **conjecture with one open, possibly-false step**: a product / cut-and-project coupling can add `√(dᵢdⱼ)` directions (golden×silver → √10), pushing rank *above* `1+k`. Distinct from B131/K014 (3-manifold cusp-gluing ≠ substitution coupling — do not conflate). Physical reading (diffraction Fourier-module rank) `PERMITTED`, firewall-honored (scale = the material's). Anchor `../speculations/S023`. **Complementary face (do not double-count):** the *mechanism* "rational-CFT data factor through finite congruence quotients (Ng–Schauenburg, cited in `NOVELTY_AUDIT.md` R3 / `K015`), so the seed's golden content is invisible there" is the *why* to L16/S023's *where* (the real-quadratic axis) — same boundary, two faces, not two results. |
| **L17** | **Symmetric-phase exclusion / quantized breaking** — is the commuting (symmetric) locus *empty* rather than unstable? Sketch: commuting parabolics ⟹ trace-2 products ⟹ zero word-entropy ⟹ no records, so the symmetric phase is excluded by structure and breaking is "quantized" (first rung `tr=3`, the seed). | MATH | ★ | **OPEN** — review/audit-proposed, **fragmented, NOT banked**. Fragments exist (K013 empty κ-elimination / no forced choice; B128 permits-not-forces; B127 the CS=0 metallic locus), but the commuting⟹trace-2⟹entropy chain and the "quantized at tr=3" framing are **not formalized**; the entropy step appears unbanked. Check against B128/K013 before any bank. |
