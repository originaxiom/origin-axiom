...
| **L14** | **Class-S coincidence** — does the SL(2,ℤ) trace-map / mapping-class action on the once-punctured-torus character variety **coincide** (is it conjugate to) the known S-duality / mapping-class action on the Coulomb branch of the class-S theory of the once-punctured torus? The B148 §3 invariant data (hyperbolic classes, eigenvalue `λ_m²`, fixed slopes in ℚ(√(m²+4))) is what such a comparison must reproduce. | MATH | ★★★ | **OPEN (reading/proof, not sandbox)** — the **one mathematically-tractable** physics-adjacent direction. Read Gaiotto–Moore–Neitzke (0907.3987), Cantat–Loray, Gaiotto class S. Benchmark to pursue: the metallic trace map IS a known duality action with independent meaning; benchmark that kills it: gauge group free input, substitution adds nothing. **Even a positive result is mathematics — does NOT cross the firewall.** (B148 §4.A.) |
