...
| **W-annulus** | the cusp torus's partition by the θ-odd Higgs field's sign is annular: χ(∂⁺M) = 0, so Pantev–Wijnholt's net chiral(R) = χ(M, ∂⁺M) = −χ(∂⁺M) vanishes on the object | B1277 addendum (the arcs and the corners) | the eight cusp symmetries force the partition; a disc would need a non-unitary holonomy at the golden value (L204) |
