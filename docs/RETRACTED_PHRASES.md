...
| 11 | `exists only for A_4 and 2T` *(the 27-dimensional **assembly** classification; and the reading "the 27-with-invariant-cubic condition leaves only A₄ and 2T")* | **B8080**, `papers/structure_paper` | **REFUTED — all six polyhedral candidates admit one.** Block-sum lemma: an irreducible carrying a non-zero invariant cubic has **zero radical** (the radical is `G`-stable, so `0` or `W`, and `W` forces `f=0`), hence any block-diagonal sum of such modules is an assembly. Witnesses: `9×3` (S₄, 2O), `3×4+3×5` (A₅, 2I), `27×`(order-3 linear character) (A₄, 2T). **The defect is multiplicity, not triviality** — the earlier repair excluded *trivial* summands, and the two groups it kept are kept by 27 copies of a non-trivial **linear** character, i.e. the refuted construction twisted. **Load-bearing:** 2O and 2I survive and **both are binary**, so `cor:onlybinary` cannot absorb it. **The entrance stands** — it is the arithmetic surjection `π₁(4₁) ↠ SL(2,𝔽₃) ≅ 2T` (48 surjections; none onto A₅ or 2I), never this theorem. |
...
- **B8070's rank descent** (retracted 2026-08-17) — the arc self-retracted the day it was
...
- **"hypercharge is the unique gaugeable U(1)"** (scope sharpened 2026-08-17 by B8070) — **not a
