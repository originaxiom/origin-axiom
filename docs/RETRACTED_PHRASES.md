...
## Currency addition 2026-08-20 (B1095 — the B1085/B1091 phrasing supersession)

- **~~"the bulk is hand-blind to ≤1 state" / "the free half is edge-observable, and
  ONLY edge-observable"~~** (B1085's banked phrasing, carried into B1091's card row) —
  superseded 2026-08-20 by B1095, in the STRONGER direction: at reversal-closed windows
  (even Fibonacci index) the two hands are EXACTLY ISOSPECTRAL (1.3×10⁻¹⁵ over 2584
  eigenvalues) — the hand is spectrally INVISIBLE, and lives in WHICH of the eleven
  shared boundary-capable states bind to the cut (5/6 complementary; the ±1 an
  odd-family parity remainder). The defensible forms: **"energies P-invariant (forced),
  localization P-equivariant (free)"** and **"spectrally invisible,
  localization-visible."** Both original texts corrected in place with dated addenda
...
| "the class restricts to **c**" / ε described as "**mirror-odd**" (of B1192's relational bit) | B1192, 2026-08-27; the eigenline *evidence* already retracted at B1216 | **REFUTED, not merely unsupported.** By B1248, `det X ≡ 2 − κ (mod squares)` with κ = tr[A,M] the Fricke–Vogt invariant, and κ is a function of the Fricke coordinates (tr A, tr M, tr AM) — **all three mirror-invariant** — so `κ(A,M) − κ(A⁻¹,M⁻¹) = 0` identically (Gröbner residue exactly 0). The class is **mirror-EVEN for every pair**, hence by B1161's free-orbit theorem constant across branches, hence **cannot be c**. What stands: ε = −1 is a well-defined, non-vacuous ℤ/2 datum of the pair. What falls: its identification with the orientation bit, and the word *mirror-odd*. Corrected at source (B1192 Addendum 2, per E53) and in RETRACTIONS. |
| "ε is the **second Hilbert-symbol slot** of the invariant quaternion algebra" | B1248 draft, 2026-09-05 (self-caught, same session) | **FALSE.** The Maclachlan–Reid algebra is `(A₀², B₁²) = (tr²A − 4, (2 − κ)/(tr²A − 4))`; ε and the second slot differ by exactly the **first** slot. Caught by computing **2T**: the Q8 pair gives `(−1,−1)`, the **Hurwitz quaternions** (the known answer), while the draft form gives split `(−1,+1)`. `det X₀` is unaffected and the arc's lock already encoded the correct formula — the *test* was right while the *prose* was false. Class **E60**. |
| "det X₀ = **squarefree**(2 − κ)" (B1248's integral refinement) | B1248 draft, 2026-09-05 (caught by a peer seat) | **FALSE.** Against the actual realizer module on 500 random noncommuting partners: squarefree scores **493/500**; the correct form `det X₀ = det((AM − MA)/g)`, `g` the entry-gcd, scores **500/500**. Counterexample `2 − κ = −121 = −(11²)`, `g = 1`: true `det X₀ = −121` (**torsor**) while squarefree gives `−1` (**the bit**) — they disagree about whether the pair carries the bit. Corrected criterion: **ε = −1 ⟺ 2 − κ = −g²**. Class **E62**. |
| "all 48 surjections give κ = 0, so **the McKay door** is not class-preserving" | B1249 draft, 2026-09-05 (self-caught by its own control) | **MISATTRIBUTED.** All **384 generating pairs** of 2T already have κ = 0, so the figure-eight relator **discriminates nothing** and the observation is about *generation in 2T*, not the door. What survives is weaker and general: 2T's κ-spectrum is {−2,0,2}, class −1 never occurs, so **2T cannot carry the bit however one maps into it**. Class **E61**. Locked so a later seat re-deriving the door claim reds the suite. |
...
**Currency (verified 2026-09-07, through B1296 — the read this registry was owed by the `doc-currency`
gate, which tripped at B1296: lag 31 > tolerance 30).** The B1250–B1296 window — the generation-count
line (B1250–B1257), the chirality-bit campaign (B1290–B1293), the seat harvest (B1294) and MASTERPLAN v3's
doors D4/D1 (B1295/B1296) — produced **five retractions, four self-caught in the arc that made the claim,
one caught by the next arc.** Three are *readings* (the phrase stays legitimate; the tail says which reading
falls, so the sweeper does not take the bare phrase). Two are *phrases* and are swept. The read also found
that **B1253's refuted headline still stood as a live section header on three surfaces** (CHANGELOG,
PROGRESS_LOG, CAMPAIGN_STATUS — the paragraph under each said "refuted", the header did not): struck at
source today on CHANGELOG and CAMPAIGN_STATUS; PROGRESS_LOG is append-only (GOVERNANCE §9), so its correction is
the dated B1296 entry that follows it. And one fence, so that B1296 is not over-applied: **B576's** *"θ-even deformations stay
F₄-stable and vector-like"* is a statement about **F₄-valued holonomy** (27 → 26 ⊕ 1, both real, so
h¹(27) = h¹(27̄)) and is **NOT retracted** — B1296's "θ-even ≠ vector-like" is about a **Higgs direction in
the θ-even Cartan** with gauge group its E₆-centralizer (which is not inside F₄); different frame, both true.

| retracted phrase / reading | retracted by | why it is wrong |
|---|---|---|
| "the generation count is forced at three" | B1253, 2026-09-05 (self-caught in the same session, after the headline had reached three surfaces); class **E63** | **REFUTED.** The 15 distinct 16-blocks with their rigid pairwise geometry (overlaps {8, 10}, 11 empty triples all (8,8,8)) are real and non-generic, but **all 15 lie in ONE W(E₆)-orbit**, so the three 16s of any triple are Weyl-conjugates — the same objection that closed OPEN_PROBLEMS §C at B324. The control tested *rigidity*; the question was *inequivalence*. The number 3 is a **maximal-independent-family size**, not a species count. Live headers struck 2026-09-07 on CHANGELOG and CAMPAIGN_STATUS (this read); the append-only PROGRESS_LOG carries the correction as its B1296 entry. |
| "the pattern behind twelve lost three-nesses" read as twelve verified instances of one cause | B1255 addendum 2026-09-06 (*the twelve were a number, not a thing*); class **E64** | **OVER-ATTRIBUTED.** "Twelve" appeared once in the arc — in its title — and was never enumerated there; the ℚ(√−3) ramification mechanism is verified for **B298**, plausible for B324/B1253 on conjugacy, **unverified for the other nine**. What stands: the mechanism where it was checked, and the arc's computations (μ13/HIER S₃, [C18, D₂] ≠ 0, 27 = 16 + 10 + 1, I-24 REFUTED). |
| "the object gives 1 abelian + 2 chiral; the SM needs 3 chiral" read as a chirality count — B1253's price line; the decomposition itself stands | B1256/B1257 addenda 2026-09-06 (*the sl₂ frame cannot carry chirality*); class **E65** | **VOID AS A CHIRALITY STATEMENT.** The decomposition 27 = Sym¹⁶ ⊕ Sym⁸ ⊕ Sym⁰ under the principal sl₂ is correct, but **every Sym^n of SL(2) is self-dual**, so for any ρ factoring through SL(2) the 27 ≅ 27̄ as π₁-modules and **net chirality is identically zero** — "2 chiral" counted summands, not chirality. B327's own sentence said so. What stands: the h¹ table, the orbit dimensions, the Brieskorn uniqueness, the 2T-blindness theorem; what falls: the sl₂ frame as a generation-count route. |
| "χ_orb = 0 for all 17 wallpaper quotients" as a closure of the cone-point case | B1291, 2026-09-06 (caught by an adversarial verification lane after the owner pushed back); class **E67** | **VACUOUS, not false.** `χ_orb(T²/G) = χ(T²)/|G| = 0` identically, so the check cannot fail on *any* wallpaper quotient; the attached control varied the base (S²(2,3,7), S²(2,3,5)) — a parameter the argument never ranges over. χ_orb is also excluded on integrality (`D²(n)` has index 1 for every n while χ_orb = 1/n). **The conclusion survived; the argument did not:** the corner case closes instead on genericity (χ(T²∖4pts) = −4 for 3994/4000 one-cusped manifolds, = −2·#tetrahedra) and on admissibility; the script is retained, labelled at source as the refuted step. |
| "every θ-even Cartan direction" (gives / is `2(R ⊕ R̄)`, vector-like — fc R70's headline, relayed onto main by B1294 §"the seats' results") | B1296, 2026-09-07 (pre-registered as an expectation and **refuted by the computation**) | **FALSE ON 4 OF 15 FACES.** On the F₄ chamber (the θ-even Cartan of E₆) the faces `w4` (A₂A₁A₂, 54 chiral), `w3+w5` (A₁A₂A₁, 54), `w2+w4` (A₂A₂, 18) and `w1+w6+w3+w5` (A₂, 6) carry a **chiral** 27-spectrum — and every one of them is **SU(3)-cubic-anomalous**; the other 11 faces are vector-like, anomaly-free, Witten 0. θ-evenness gives *dimension* symmetry (44/44), not vector-likeness. The **conclusion** R70 drew survives, strengthened: the chamber is *vector-like or anomalous, never chiral and clean* (`T-CHARGE-LOCUS-PARITY-LOCK`). Corrected at source (B1294 FINDINGS, CAMPAIGN_STATUS B1294 entry) 2026-09-07; the seat correction is drafted and HELD (owner: all sends hold). |
| 10 | `83 of 83` | **B1235** | False. The family is **38/112 amphichiral**. The closure was measured with `M.is_isometric_to(mirror)`, which is orientation-blind; by `symmetry_group().is_amphicheiral()` 74 of the 112 are chiral, `o10_150700` among them. Registered 2026-09-10 (B1326) after the phrase survived a week in THE PAPER unregistered. |
...
| 12 | `2.8447655827` *(and `sigma = 0.4741275971` as a value of the framework's anchor)* | **xB016 P2 + ADDENDUM 1** | **A REFUTED CANDIDATE, printed by xB016's SEAL IN ADVANCE precisely so it could never be presented as a discovery.** It is what `σ` and `c = 6σ` *would* be **if** `log|H₁ tors|` of a cyclic cover could be read as an entropy equal to minus the on-shell action. It cannot: the on-shell action of complex Chern–Simons **is** the complex volume (DGLZ arXiv:0903.2472 eq. 4.6) and torsion is the **one-loop** Ray–Singer term (eqs. 2.1–2.3), so the identification double-counts one `ħ`-expansion at two orders; a cusp is a **massless particle's worldline, not a horizon** (Gukov hep-th/0306165 §1.2); the number is **knot-dependent** (the trefoil gives `σ = 0`); and xB016 P3/P7 show the rate it comes from is a property of the **tower**, not of m004 (87 covers span 0 → 0.474121, 19 with no torsion at all). **Never cite it as a prediction of this programme.** |
| 13 | `k must be even` *(as a consequence of the cusped mod-½ Chern–Simons ambiguity)* | **xB016 ADDENDUM 1 (P9d)** | **Withdrawn as unsupported.** Witten (arXiv:1001.2933 eq. 2.2) gives `I = −s·Im W + ℓ·Re W` with `s ∈ ℂ`, `ℓ ∈ ℤ` — **the only condition is that the level is integral**, and nothing couples `k` to `σ` (Gukov hep-th/0306165 §1.1: *"the other parameter, s, is not quantized"*; Dimofte arXiv:1409.0857 §2.1). The framing escape does not exist either: for **complex** Chern–Simons the eta-invariant **vanishes** (Gukov eq. 3.36). The literature resolves the mod-½ ambiguity by **lifting** (a decoration/spin structure, the invariant in `ℂ/4π²ℤ` — Zickert arXiv:0710.2049; DGLZ §3), **not** by constraining `k`. The inference was an artifact of mixing Meyerhoff–Neumann's *manifold-invariant* normalisation with Witten's *field-theory* one — which is exactly the unpinned normalisation xB016 P4 registered as **L223**. |
| 14 | `zero free dimensionless constants` *(of the object's action; also the gloss "σ is the one dimensionful unit")* | **xB015 K1 + xB016 ADDENDUM 1** | **Off by one.** `σ = ℓ/(4G)` is **dimensionless** — in 3d `[G] = length` — and B1015's sealed declaration already says so (*"A2 (dimensionless frame): c … equivalently σ = c/6"*, with A1 = ℓ the dimensionful unit). Calling σ the dimensionful unit collapses A1 into A2. **The action has exactly ONE free dimensionless constant, A2 = c = 6σ**, and complex Chern–Simons confirms it is genuinely free and unquantized (Gukov hep-th/0306165 §1.1; Witten arXiv:1001.2933 eq. 2.2; Dimofte arXiv:1409.0857 §2.1). Everything else on the action card stands: `Λ = −1`, the volume, `CS = 0` exact, `c = 6σ` derived twice, `S = −Vol·σ`. Surfaces corrected same-commit: B1088 FINDINGS, `THE_FRAMEWORK` §Layer-6, `THE_LADDER` X34 (and X21's owed normalisation check, now L223), `OPEN_LEADS` C1. |
