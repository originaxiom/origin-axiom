...
**It is NON-MUTATING, and that was a correction too.** The first version left its regenerations on
disk. That was wrong twice over: it **destroyed qB946's four cached values** (only git still had
them), and — because it runs *inside the suite*, alongside locks that read those same files — a
mutating sweep would make their results depend on **test order**. That is the E39 shape wearing a
different hat: a lock whose answer depends on something invisible to it. It now snapshots every
file and restores it unconditionally; findings are reported, never written. **Verified by hashing
all 28 `results.json` before and after a full sweep: zero modified.**
...
- [x] R1-15 **DONE**: `atlas-generated` is now the **29th gate** — `docs/RECURRENCE_ATLAS.md` must equal what `scripts/atlas/render.py` produces. It had **no freshness check at all** and went stale **twice in one session** (970→971 probes when qB1054 banked; a motif count 411→412 when qB1054's own prose was edited), visible only because `tests/test_atlas.py` silently **writes** that tracked file during the suite. Original text: it is generated by a different generator than `docs/views/`, so `views-generated` never covered it (owner: SEAT)
- [x] R1-17 **DONE**: the 29th gate went red the day after it was banked, on the **calendar rather than on staleness** — `render.py` stamps `> Last generated: {today}`, so a byte-for-byte comparison failed on any later day with no arc having moved; the whole diff was the date. **A gate that reds the build daily gets routed around**, which is the failure gates exist to prevent. Repaired: the volatile stamp is excluded, everything the atlas is *for* is still compared byte-for-byte. **And it mutated a tracked file to check it** — running the gates rewrote the working tree, the same defect its own docstring names in `tests/test_atlas.py`, committed by the gate written to catch it. Both the gate and that test now restore the bytes they found (owner: SEAT)
