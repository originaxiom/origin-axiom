...
- [ ] R58-1: mint the **mismatched-hypothesis** error class (a theorem quoted correctly, applied outside its hypotheses) — **after B1376 lands**, to avoid a concurrent `ERROR_LEDGER` collision (owner: unassigned; source: Review 58 §4). **EVIDENCE UPDATED 2026-09-18: the class has now fired THREE times in three days and the third one COST A REGISTERED LEAD.** **Instance 1:** xB023 K1's first version (Kawauchi's *closed*-manifold theorem applied to cusped manifolds; B1239's defect). **Instance 2:** xB023 Addendum 1 (`H₁ ≅ ℤ` used as a proxy for *knot complement in S³* — necessary, not sufficient; the seal named the fence and the cell used the proxy anyway). **Instance 3:** xB029 (Friedmann–Witten's threshold framework, which assumes **finite `π₁`**, carried to hyperbolic 3-manifolds, which have infinite `π₁` — and the violation was measured to be NOT benign: zero modes on three of six members). **Instance 3 has a feature the other two lack and the minted class should name it: the hypothesis was in the CITED paper, not the citing one.** xB029 filed Friedmann–Witten as CITED-UNREAD *in the same commit* in which it leaned on them, so `LITERATURE_READ_REGISTER`'s own rule (a CITED-UNREAD statement may not carry a load-bearing step alone) was written down, correctly applied to the filing, and violated in the reasoning. **If B1376 remains unlanded at the next review, this item should be reassigned rather than deferred again** — a class that keeps firing is worse than a merge conflict.
