...
| `B8093` | GAP | Typing an external operator-algebra construction (CLPW's crossed product, Type III₁ → II_∞ → II₁) against our own. Object-level topic — **modular flow, tracial states, von Neumann type** — that the lexicon carries no motif for, which is itself worth noting: the atlas has motifs for roots and charges but none for the **operator-algebraic** layer the observer work lives in. Not instrumental: no repository machinery appears. |
