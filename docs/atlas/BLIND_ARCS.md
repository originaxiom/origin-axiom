...
| `B1143` | GAP | the B−L fourth-Cartan-direction layer (2026-08-25) — the lexicon has no term for the B−L quantum number, the trinification-frame charge assignment, or Cartan-independence-as-a-fourth-direction; a real object topic the vocabulary misses (the arc's own finding: the physical B−L is independent of {Y, T₃R, T₃L}, closing the SM's Cartan content — confirmed two-bench with cloud memo 25, the apparent vector mismatch reconciled as a cross-frame artifact in B1144) |
