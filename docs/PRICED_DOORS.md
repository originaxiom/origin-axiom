# The priced-doors campaign — every physics word, its price, and its computable next rung

*(Registered 2026-07-03. FIREWALLED THROUGHOUT: this campaign computes the priced routes; it
asserts no crossing. Each physics word is earned in one of three currencies — derived dynamics,
a controlled limit, or physical instantiation — and shapes are free while currencies are not.
The honest anchors are stated first; framing lock GOVERNANCE §2 applies to every row.)*

## The two route-less items (stated up front, no probes registered)

- **3+1 spacetime** — no visible route from this mathematics. Not priced; not walked.
- **α as a number, today** — a fine-structure constant needs a *ratio of symmetry-forced
  couplings*; the program currently possesses at most ONE forced coupling candidate (±1/48,
  itself pending the W2.11 selection-rule theorem). Nothing to ratio. The α door is gated
  (PD4) behind two other doors and does not open by wanting.

## The doors, priced

| door | what exists (shape, banked) | the price (currency) | the computable next rung |
|---|---|---|---|
| **PD1 · the anyon door** (the REORDERED nearest crossing) | the banked structure — modular matrices on a finite theta space, seam values as matrix elements — is modular-data-shaped for a (2+1)-d topological phase; the minimal two-state sector (P56) with quasi-energy ±108° | **physical instantiation** (κ's letter): the algebra realized on hardware, its invariants measured | PD1.1 the level-45 minimal-sector KILL TEST (pinned at 108° = anyon-shaped, gapless-trending = photon-shaped); PD1.2 modular-data identification against known U(1)-type anyon models (identification, with citations — not discovery); PD1.3 the four-qubit circuit compilation of W₁₅ = perm·(W₃⊗W₅) + exact simulation of the seam as an interference amplitude; PD1.4 the W2.11 selection-rule theorem (the forced-coupling gate) |
| **PD2 · the gauge/tower door** | the Heisenberg phase XZ = ζ·ZX is literally magnetic-translation holonomy: level-N theta = lowest-Landau-level states on a torus with N flux quanta (classical math, imported with citations); level 15 = 15 flux quanta; the forced a=½ (C6) = the unique monodromy-invariant Wilson-line modulus | **a controlled limit** (finite → continuum along the level tower) — a 15-dim space holds no field modes; the modes live in the ladder 15 → 45 → 135 → … | PD2.1 the magnetic framing banked as cited context + exact algebra checks; PD2.2 the tower-limit ladder: track the minimal-sector phase and the seam normalization across 15/45/135 (B372's F_p engine scales); PD2.3 the wall-equivalence note: the four-way-measured wall (rate-forced, scale-external) IS the tower-limit gate — every particle word funnels through it |
| **PD3 · the 2+1 spacetime door** | the mapping torus IS a 2+1 Euclidean spacetime; hyperbolic 3-manifolds are 2+1 gravity in the Chern–Simons form — the figure-eight is that literature's standard example; time's arrow available in-model (Anosov mixing at rate log φ², banked) | **identification against established quantum invariants** (the bridge must agree with the known 4₁ data, not with hope) | PD3.1 the Kashaev bridge: compute the Kashaev invariant ⟨4₁⟩_N and Andersen–Kashaev-model data, identify where (if anywhere) the banked level-15 structure sits in the known quantum-invariant family; PD3.2 the arrow note (mixing → coarse-grained irreversibility), documentation-tier, firewalled |
| **PD4 · the α gate** | one forced-coupling *candidate* (±1/48, W2.11-pending) | **two theorems + a ratio + the null gauntlet** (the same firing squad that shot Λ) | GATED: opens only if PD1.4 lands the selection-rule theorem AND PD2.2 produces a second forced coupling at another rung. Registered, not walked. |

## The photon, honestly

Downstream of all four: dynamical gauge field ⊂ continuum limit ⊂ the tower. The wall the
program measured four ways is the photon question in another costume — every physics word
funnels through the same gate. Current evidence: shut, four ways. The campaign's job is to
measure the gate's hinges (PD1.1, PD2.2), not to declare it open.

## Execution order (decisiveness per cost) and discipline

PD1.1 → PD1.3 → PD3.1 → PD1.4 → PD2.2 (135-rung) → PD1.2 + documentation probes (PD2.1,
PD2.3, PD3.2). Every probe: pre-registration first (nulls + kill conditions committed before
computation), exact-first arithmetic, atlas/FAILURE_ATLAS consult, promotion only through the
§5 gates, nothing physics-adjacent promoted, hardware execution (PD1.3's far end) is an OWNER
decision outside this campaign's scope — the in-sandbox deliverable is the compiled circuit +
its exact simulation.
