...
$\alpha_s$-dominated --- $\sin^2\theta_W$ itself lands within about $1\sigma$ of the measured value. Three complete copies of the Standard-Model content are \emph{exhibited} on the object's own closings, in mirror pairs,
and they are vector-like: \textbf{the chirality bit is what the object withholds, and we prove where.} Every fixed locus of the
object counts two or nothing; the descent that supplies three is vector-like by two conjugations the object
or its coefficient field carries; and every wall met is such a conjugation. The amphichirality is the object's and not its
tower's: of its 87 covers to degree 10, 66 are chiral and all keep the arithmetic, so the genesis's handedness bit
becomes a class invariant there; the spectral wall has held on every sector computed, and the open sectors are the
multi-cusped covers.
...
and the Standard-Model shaping. All verification reported here is internal to the project's own re-runnable pipelines; no external review or endorsement is claimed.
\end{abstract}
...
count is not distinctive. Over the first $400$ one-cusped census manifolds by volume, \textbf{$36.2\%$ admit such a
surjection and $31.0\%$ admit exactly two} --- $m004$'s own count. Two of our implementations
disagreed here, at $37.2\%/32.8\%$ against $36.2\%/31.0\%$, and we recorded the discrepancy as
unresolved; it is now resolved against the first. A third, independent enumeration --- surjections
counted up to $\mathrm{Aut}(\mathrm{SL}(2,3))$, so that $m004$'s $48$ raw surjections are the two the
construction starts from --- returns $145/400$ and $124/400$, reproducing the second implementation
exactly, together with the tie list ($m003$, $m007$, $m022$, $m026$, $m027$, $m029$, $m030$, $m033$,
$m034$, $m036$) and the $28$ of the $400$ that carry presentations with more than two generators.
The first implementation's figures are moreover not integral over a denominator of $400$
($37.2\% \times 400 = 148.8$), which is the defect. The base rate is \emph{roughly one in three},
which is all the genericity argument actually needs.
...
separates them is the cusp shape ($m004$'s is $2\sqrt3\,i$, $m003$'s the hexagonal $\omega$), which is not among the data the
...
9 & Theorem & congruence, conventions named \\
...
18 & \textbf{AXIOM} & the observer's closings \\
19 & Identity & the pair-separation law of integration \\
20 & Identity & the measurement-torsor rank \\
21 & Theorem & the T1 structure \\
22 & Corollary & no canonical closing \\
23 & No-go & the T1-mover; closes an earlier door \\
...
\begin{nonclaim} that three \emph{chiral} generations are derived. Three complete generations are exhibited on the
object's own closings, in mirror pairs; they are vector-like, and \S\ref{sec:chirality} proves at every computed
...
no odd case; the control fires with two cusps, where one and three both occur. On the object itself every
isometry has $\chi(\mathrm{Fix}\,g) = 1 - s_\mu(g) \in \{0,2\}$, read off $H_1 = \Z$ alone, and on a closed
...
holonomy factoring through $\mathrm{SL}_2 \to E_6$ has $27 \cong \overline{27}$ and net chirality identically zero,
...
the fixed set: \textbf{the object supplies no charge locus.} Dropping equivariance yields the construction's
first chiral spectrum --- two $16$'s of $\mathrm{SO}(10) \times \mathrm{U}(1)$, count two --- at three choices that
are the closer's, not the object's, and they are priced as such. The lock is stated under the outer of the involution's two lifts to $E_6$ (the one fixing $F_4$); under the inner
lift, fixing $A_5 \oplus A_1$, every Cartan direction is even and the count-two configuration is equivariant and
anomaly-free. So the lock stands on the flat germ, where the lift is outer by a sign computation over two primes,
and the choice of lift in the singular frame is priced as a further identification.
...
of the restriction of $H^1(M;V)$ to the cusp, valid on a stated domain and pre-registered before evaluation. The
...
the whole cyclic tower. First, for the object's field $\Q(\sqrt{-3}) = \Q(\zeta_3)$, every twist of order prime
to three is carried to its dual by a Galois automorphism of the coefficient field that preserves every rank in
...
$27$'s is zero. \textbf{The flat $E_6$ sector carries no net count on any $\mathfrak{sl}_2$ germ}; what lies outside
...
\paragraph{The closings carry three generations, in mirror pairs.} The cyclic closings $Y_n$ have
...
which the colour triplet thins to one generation. The tree-level vacuum of the ninth leaves one abelian factor
beyond the Standard Model, a family-non-universal $Z'$, anomaly-free by the conjugate generations and neutral on
every field that acquires an expectation value. Its first consequence is a \emph{regime}, not a value: a $Z'$ at
$M_{Z'}/g_2 \gtrsim 10^2$--$10^3\,$TeV (kaon mixing) if a light family carries the expectation value, with a floor
near $2\,$TeV$\cdot g_2^{-1}$ ($B_s$ mixing) if the third does --- and which family it is, the construction does not fix. We record this as a fork on an observer-selected
vacuum and claim no prediction from it.
...
other placement of the swap squares to $RL$. So the genesis's one bit is the swap's placement inside the tick,
...
patterns twice each; no symmetry flips exactly one of the three, every pair-flip exists. In the record's own
...
orientation-aware tests that agree on all $87$. Every cover keeps the invariant trace field, so on $66$ members
of the object's own commensurability class the bit is a class invariant and the arithmetic survives together.
...
failures to close on itself --- an orientation cannot be chosen canonically --- and not only here --- a sign in a chord
...
\item \textbf{Localized chiral matter as a signed zero count} of a Higgs eigenform is Pantev--Wijnholt~\cite{pw};
...
monoid are Mignosi--S\'e\'ebold~\cite{lothaire}; the second Lagrange value is Markov~\cite{markov}. \emph{Reproduced;
...
is selected --- a regime of $10^2$--$10^3$\,TeV$\cdot g_2^{-1}$ from kaon mixing, or a floor near $2$\,TeV$\cdot g_2^{-1}$
from $B_s$ mixing --- on an observer-selected vacuum.
...
theory of everything. Three complete generations are exhibited and they are vector-like: the chirality bit
...
the one-cusped index and its two vanishing theorems: the cyclic tower is vector-like & settled & lock \\
the last flat-sector direction carries non-self-dual representations with zero net count & settled & lock \\
the tower law (a twisted class iff a product of n 2x2 matrices is the identity); the SM closings carry three complete generations in mirror pairs; the tree-level vacuum SM x U(1)\_Z' and its regime fork & settled & lock \\
...
\noindent\footnotesize All 30 claims below (over 36 claim--record pairs) carry a test lock; 4 additionally
