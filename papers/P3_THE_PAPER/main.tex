...
Finally we price the residue. What the object cannot supply, it costs exactly: a freedom ledger with one dimensionful unit that is external by design; two continuous dimensionless anchors, of which one is external \emph{by theorem} --- the object's own modular flow is trivial, so it has no continuous clock to give, and the one route that would import that datum from structure already in hand is exactly tautological; a projective line of Higgs data closed permanently one condition short of a point set, the candidates for that condition having been computed and returned negative; and four non-continuous rows --- one of which is a relational invariant of a pair, belonging to
neither relatum and requiring no act of selection, while the other three are externally supplied: an
antilinear involution, the seed of a binary label, and a choice from a finite menu.
\end{abstract}
...
\emph{not} about the table above, which additionally carries three discrete inputs that are
externally supplied --- the real-structure closing, the arrow label's seed, and the finite menus ---
and which this count does not touch. Three assignments do that work, and we note that each was established \emph{against} an earlier version of
...
without it the paper refutes itself in \S\ref{sec:observer}, since $(1,-4,2,-3,6)$ and $(1,2,-4,-3,6)$ are two
...
\begin{scopenote}[What no experiment can do to this paper]
Every falsifier above is \emph{mathematical}: each asks a reader to exhibit an object, and each is
settled by computation rather than by measurement. That is not a stylistic preference, and a reader
should see why it is forced. \S\ref{sec:withheld} proves that this construction supplies no
dimensionless value; \S\ref{sec:observer} proves the normalisation is not derivable in principle;
and rigidity gives the object its shape but never its scale. A construction that provably emits no
number cannot be contradicted by any measurement of one.

\textbf{So this paper is unfalsifiable by experiment, by construction, and we would rather say so
than let a reader discover it.} What follows is a real limitation on what the result can ever
become: a structure-only claim with provably no values cannot graduate from suggestive to
established by any future datum. It can be refuted only from inside mathematics --- which the six
items above make possible, and which is the entire warrant we offer. A reader who requires
experimental contact should stop here, and we think that reader is being reasonable rather than
uncharitable.
\end{scopenote}
