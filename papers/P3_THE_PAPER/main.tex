...
multiplicity carries no generation content: three at the ninth, one at the twelfth, are counts of copies and no more. The vacuum analysis that follows is a \emph{supersymmetric} $E_6$ model of the kind studied as the exceptional supersymmetric Standard Model~\cite{kmn} (whose premise, chiral matter, this spectrum lacks): F- and D-flatness on the singlet fields, a tree-level superpotential, and
...
so the bounds constrain a supersymmetric model suggested by the closing, not the closing --- and a vector-like pair admits a gauge-invariant supersymmetric mass and can decouple, so the model must further assume the copies are kept light, an assumption we do not derive.} Which family it is, the construction
...
\item \textbf{Jørgensen's inequality and Callahan's uniqueness}~\cite{jorgensen,callahan}: that the figure-eight complement is the unique orientable manifold attaining Jørgensen number one is Callahan's theorem; what we add is only that the record's founding selection is thereby variational rather than descriptive, which selects the manifold and nothing downstream of it.
...
\item \textbf{Kac's classification of finite-order automorphisms}~\cite{kac} and the \textbf{Tits/Knus--Tignol type table for $D_4$}~\cite{knustignol} are the two classical inputs to the closing paragraph of \S\ref{sec:chirality} and to the typing of the object's trialitarian form (of type $^6D_4$ over $\Q$, since its cubic $\Q[x]/(x^3-12x-5)$ is non-cyclic, and $^3D_4$ only over $\Q(\sqrt{77})$); both are used as stated, not re-derived.
...
\paragraph{The ledger in three columns.} The reader who wants the whole boundary in one place can have it. \emph{Parameter-free, and had}: the object up to one bit and its selection as a minimiser; the invariant trace field $\Q(\sqrt{-3})$ and the $2T$ door; $E_6$ and the $27$ by McKay; the termination of the breaking chain; the direction of hypercharge and the $\Z_6$ global form (given the colour frame and the shaping the ledger lists); three structural copies of the Standard-Model content on the closings, vector-like; the two conjugations that kill the index on the cyclic tower; the theorem that the geometric finite-twist index vanishes on every one-cusped manifold; the exhibited non-zero index on the class and on the tower; the base rates of \S\ref{sec:generic}. \emph{Not yet had, with the computation named}: a non-zero index on the object's own reducible locus beyond $m \le 4$ and twists of order dividing twelve (a finite computation; the modules run are listed); the several-cusp index reproduced from a clean checkout; the rule separating the class members that fire from those that do not; a compact $G_2$ closing carrying the apexes, and its $27$ sector; the earned identification of a count with a generation; the supersymmetric crossing; a candidate for the $\sigma$ row meeting the six conditions above; a second condition on the projective Higgs line $\PP(B_0)$ closing it to a finite point set (the named datum in progress elsewhere); and the chirality bit on the member $m004$ itself, which the first item of this column would move if it fired. \emph{Believed unreachable, each by a stated theorem}: a Standard-Model ratio among the object's periods and regulators (exhaustive at stated bounds); the hypercharge normalisation from the anomaly equations alone (homogeneity); the scale (Mostow); the modular period from the object's own algebra (traciality); a chiral spectrum from the geometric holonomy on any one-cusped manifold (injectivity), from any $\theta$-odd cusp field with transverse zeros (region swap), or from the cyclic tower (the two conjugations); a Majorana mass without a $27$ sector of the opposite chirality; and a finite label reduced further (an invariant selector cannot pick a point of its own orbit). Nothing in this third column is merely undone; each entry names the theorem that closes it, and the one chirality question that is undone rather than closed --- the member's own reducible locus --- stands in the second column, where it belongs.
...
the exact half of the closing design: the E7 apex local model's geometry (its count cited), the Z/3 = 2T/Q8 forcing b2 >= 2 for a symmetric triple, the Y3 breaking to SM x U(1)\_eta by a Q8 line times an order-4 character (root-system facts), and the algebraic no-seesaw obstruction (equal eta charges on both charged singlets; no eta-neutral Majorana source in 27\^{}3 or 78) -- the design itself and its exclusion are conditional, a design and not a theorem & settled & lock \\
...
\noindent\footnotesize All 46 claims below (over 66 claim--record pairs) carry a test lock; 4 additionally
...
\bibitem{knustignol} M.-A.~Knus and J.-P.~Tignol, \emph{Triality and algebraic groups of type $^3D_4$}, Doc.\ Math.\ Extra Vol.\ (2015), 387--405, arXiv:1409.1718.
