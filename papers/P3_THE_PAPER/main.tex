...
$\sin^2\theta_W = 0.2293$ against $0.2312$ ($0.9\%$, about fifty experimental standard deviations (a distance against the measurement's error, not a test statistic for a model already known to fail)); the construction's own
...
Two more facts about these counts belong beside them. The census is ordered by complexity, not sampled at random, and the object sits at its small end; the rate drifts from $36.25\%$ over the first $400$ to $33.92\%$ over the first $5000$, and much further down the census: sampled in blocks of $800$ at depths $0$, $20\,000$ and $80\,000$ it falls $34.25\% \to 29.38\% \to 21.12\%$. The decline is entirely in the two-generator stratum ($34.23\% \to 15.36\%$) while the three-generator rate is flat near $33\%$ and its share grows with depth, which props the aggregate up. We state the larger drift rather than the smaller because it cuts against us in the only direction that matters: the door is rarer deep in the census than at its front, so a reader re-running the sweep at depth will find the object less generic than our own headline rate says, and the argument of this section is that even the front-of-census rate is fatal to an inference from recurrence. We use these counts as bounds on specificity, not as estimates of a population parameter. And no other object was tried before $m004$: it was the programme's starting hypothesis, and the selection theorems that now single it out were found afterwards, a fact the reader should price. We did not begin with this discipline. The programme applied base-rate reasoning to numerical
