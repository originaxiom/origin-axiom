...
% ---------------------------------------------------------------------------
% AUTHOR BLOCK. The repository is public and its privacy rule keeps the owner's
% surname out of tracked files (the byline form below is the one the rule allows);
% the submission copy carries the full author block and is built OUTSIDE the repo
% (owner-supplied 2026-09-16). Acknowledgements, if any, are added there too.
\author{Drit\"ero M.\\ \small Independent Researcher, Prishtin\"e}
% ---------------------------------------------------------------------------
\date{Draft of 2026-09-16}
...
We ask what gauge structure can be extracted from one hyperbolic 3-manifold, the figure-eight knot
complement $m004$, without importing a value. We begin with what is generic: the $E_6$ recurrence is forced
by one ADE classification, about one in three one-cusped census manifolds admits the binary tetrahedral
surjection, and what survives is the invariant trace field $\Q(\sqrt{-3})$. From it the object supplies an
\emph{arena} (a rank-3 abelian sector and a fifteen-dimensional piece of the $27$); the \emph{content} is
forced by the anomaly conditions alone, in a computation with no token of the object; the object
forces the termination of the breaking chain (given the listed chirality input) and, with two further listed inputs, the global form
$[\mathrm{SU}(3)\times\mathrm{SU}(2)\times\mathrm{U}(1)]/\Z_6$. It withholds values at every bounded-height
search we ran. We report the misses: $\sin^2\theta_W=3/8$ is Georgi--Quinn--Weinberg reproduced, not
predicted, and runs to a $35\%$ miss in $\alpha_s$; three Standard-Model copies exhibited on the
object's closings are vector-like. The chirality bit is what the object withholds, on every module computed:
the cohomological index standing in for net chirality is zero by theorem on every finite twist of the geometric
holonomy, non-zero on five members of the object's commensurability class and on its
degree-four cyclic cover, and zero on the object's own reducible locus; the one closing
mechanism that could supply chiral matter is excluded as it stands. The residue is a freedom
ledger: one dimensionful unit external by design, two continuous anchors (one external by theorem), a
projective line of Higgs data, and seven non-continuous rows, one relational and six externally supplied.
All verification reported here is internal to the project's own re-runnable pipelines; no external
review or endorsement is claimed.
\end{abstract}
...
\emph{not} evidence of specificity; on a census of one-cusped manifolds~\cite{snappy,chw}, one in three ($1696$ of the first $5000$) admits the
...
$\sin^2\theta_W = 0.2293$ against $0.2312$ ($0.8\%$, about fifty experimental standard deviations (a distance against the measurement's error, not a test statistic for a model already known to fail)); the construction's own
closing's vacuum analysis assumes a supersymmetry the object does not supply --- its carrier admits no supercharge, by three exact negatives --- and the supersymmetric crossing has not been run. Three complete copies of the Standard-Model content are \emph{exhibited} on the object's own closings, in mirror pairs,
...
only through the $\Z/3$ descent, which is vector-like by two conjugations the object or its coefficient field carries. That arithmetic and handedness are independent is not only visible on the object's covers; the cleanest witness is
smaller and older than anything in this section. Among once-punctured torus bundles built from the same two shears,
four are arithmetic at word length up to seven, and two of them --- a mirror pair over $\Q(\sqrt{-7})$ --- are
\emph{chiral}, verified by both standard criteria and cross-checked against the volume of the corresponding Bianchi
orbifold. So no general implication runs from arithmeticity to amphichirality, and the object's own amphichirality is
a fact about the object (and, as we said above, about the axiom that built it) rather than a consequence of its
arithmetic. Chirality is not a commensurability invariant: of the object's 87 covers to degree 10, 66 are chiral while the object
is not, and since the invariant trace field is a commensurability invariant those 66 carry a remembered handedness
together with the arithmetic. The cohomological index that stands in for net chirality on a one-cusped manifold --- a count of twisted classes, not of generations --- is identically zero on every finite twist of the geometric holonomy, by a theorem, and non-zero in characteristic zero on reducible non-split modules: first on a census manifold, then on five members of the object's own commensurability class and on its degree-four cyclic cover, while the object's own reducible locus returns zero on every module computed. The bit, in that sense, is a property the class and the tower supply and the member does not; what fires is a non-semisimple flat module, and whether such a module is an admissible background is stated as open. The one mechanism by which a closing could supply chiral matter --- conical points with anomaly inflow --- is followed to its first obstruction and is excluded as it stands: it has no seesaw and a refuted tree-level flavour texture.
...
The work was carried out by a single author with AI-assisted verification passes, and every computation named is re-runnable from the public repository; nothing here rests on a computation the reader cannot repeat. We therefore invert the usual order.

\paragraph{What is new, in one sentence.} The mathematical content this paper adds to the record, as opposed to reproduces,
is: the census facts (the base rates of \S\ref{sec:generic}; sixty-six of the eighty-seven covers to degree ten chiral while
the invariant trace field persists; the $112$-member $\Q(\sqrt{-3})$-shape family and its three members carrying chirality,
the arithmetic, the $2T$ door and the count of three together); the filling census of \S\ref{sec:middle} as corrected; and the
twisted-index computations of \S\ref{sec:chirality} --- the vanishing on every geometric finite twist through
Menal-Ferrer--Porti, and the explicit non-semisimple modules on a census manifold and on five members of the object's class
where the index is non-zero. Everything Standard-Model-facing is, as the ledger says, reproduced or conditional, and the
paper's claim is the location of that boundary. \S\ref{sec:generic} states what is \emph{generic} about the
...
\paragraph{And the catalogue is short.} $E_6$ is moreover the only exceptional label the object reaches as a
quotient of its fundamental group, and the reason is a computation rather than an assertion: by direct enumeration
of homomorphisms (the record's own run, reproduced with a second program), $\pi_1(m004)$ has exactly two surjections onto the
binary tetrahedral group $2T$ up to automorphism and \emph{none} onto the binary octahedral group $2O$ or the binary
icosahedral group $2I$ --- that is, none onto the finite groups that would hand over $E_7$ or $E_8$ by the same
correspondence. The trace-field remark one might reach for first --- a finite subgroup of $\mathrm{SL}(2,K)$ has its traces
in $K$, and $2O$, $2I$ need $\sqrt2$, $\sqrt5$, which an imaginary quadratic field does not contain --- is a statement about
subgroups of the holonomy's field and not about quotients of $\pi_1$, and we do not rest the claim on it; it explains only
why an imaginary quadratic realisation cannot carry $2O$ or $2I$. The golden end's $2I \cong \mathrm{SL}(2,\Z/5)$ enters the
two-ended family by a different route (the monodromy matrix reduced modulo $5$, \S\ref{sec:unique}), not as a quotient of the
object's group. So on the quotient route there is not even a birthday problem to argue about: the space of outcomes has one
exceptional element in it.
...
\paragraph{The three base rates that bound \S\ref{sec:chirality}.} Extending the count above to the first $5000$ one-cusped census manifolds, $1696$ ($33.92\%$) admit the surjection. A separate count over the $112$ census manifolds whose tetrahedron shapes all lie in $\Q(\sqrt{-3})$ --- the object's own family, most of it deeper in the census than the first $5000$ --- finds $59$ that admit it, $35$ that do not, and $18$ with more than three generators that the exact enumeration did not reach: the door is common and it is not the arithmetic. Chirality runs the other way: $181$ of the $203\,123$ one-cusped census manifolds are amphichiral, on an orientation-aware test, so the requirement the object fails is one $99.9\%$ of the census passes. And the count of three --- an orientation-preserving cusp-fixing isometry with $|\det(A-I)| = 3$ --- is attained by $2$ of the first $4000$ census manifolds and by $20$ of all $212\,641$, never on one cusp; the object carries the generic value $4$, as $3996$ of the first $4000$ do. The selecting requirement is the one the object fails, and the requirement it passes selects little; we state that before \S\ref{sec:chirality} so that the section is read against it.
\end{scopenote}

\begin{scopenote}[On our own history]
Two more facts about these counts belong beside them. The census is ordered by complexity, not sampled at random, and the object sits at its small end; the rate drifts from $36.25\%$ over the first $400$ to $33.92\%$ over the first $5000$. \textbf{How far it drifts deeper down depends on which census is meant, and an earlier version of this sentence had that wrong}: on the \emph{full} orientable cusped census, blocks of $800$ at depths $0$, $20\,000$ and $80\,000$ give
$34.25\% \to 29.38\% \to 21.12\%$, a real decline living in the two-generator stratum; but on the \emph{one-cusped}
census --- the population every other rate in this section is taken over --- the same blocks give
$33.00\% \to 33.88\% \to 31.38\%$, which is flat to within a couple of points. We had quoted the first triple in a
sentence whose neighbours are one-cusped, and an outside referee caught it; both triples are recomputed here on both
populations. The consequence runs in our favour and we would rather say so plainly than quietly keep the version that
did not: the door is \emph{not} an artefact of the census's small end, where the object sits --- it stays near one in
three eighty thousand manifolds deep --- so the genericity this section argues for is stronger, not weaker, than the
drifting figure suggested. We use these counts as bounds on specificity, not as estimates of a population parameter. And no other object was tried before $m004$: it was the programme's starting hypothesis, and the selection theorems that now single it out were found afterwards, a fact the reader should price. We did not begin with this discipline. The programme applied base-rate reasoning to numerical
...
\textbf{The invariant is four, and four is not the whole price.} The derivation as stated rests on \emph{four
undischarged inputs}, three before the object and one after, and that count does not move with how finely the chain is
cut. But the chain's four are the inputs of a \emph{mathematical} derivation, and the record keeps a second ledger for
the claims of the form ``$X$ here \emph{is} $Y$ there'' that any physical reading would additionally need --- the
identification of a quotient of $\pi_1$ with an orbifold group, of a cohomology dimension with a generation count, of a
structural analogue with a measured quantity. Fourteen of its rows are unearned; reducing them by their own stated
dependencies leaves \emph{eight} irreducible ones. So the honest total is four for the structure and \textbf{twelve} for
a physical reading, and the second number is the one a reader should carry away. We give the four here because they are
what this paper's own claims rest on; \S\ref{sec:ledger} and the non-claims say where the other eight would enter, and
the paper claims no four-dimensional theory precisely because they are not paid. The table as we cut it has \textbf{fifty-seven
links} --- thirty-six theorems, six identities, eight no-go results, two censuses, one corollary, and those four axioms ---
so fifty-three of fifty-seven are forced, in the precise sense that they are not declared choices. (One of the thirty-six
is new here: the step from the object's arithmetic to the exceptional algebra had been narrated in prose for the whole life
of this construction without being a link, and we mint it as one below rather than leave the door unlisted.) \emph{That ratio is not
the invariant and we do not ask it to be}: it counts rows under this typing and moves with the table's granularity, and
``forced'' does not mean ``theorem'' --- the thirty-six theorems are the theorems, and a census or a no-go result is
forced only in the stated sense that no declared choice enters it.

\paragraph{Where the four axioms are, which is the point.} In the table's order they sit at the two ends (and the
order, we repeat, is a banking order):
...
\item \textbf{One after the algebra is in hand} --- the observer's closings (a torsor of discrete choices; the
word ``closing'' also names two \emph{geometric} operations in this paper, and \S\ref{sec:chirality} separates all three).
...
\begin{tikzpicture}[x=2.4mm,y=1mm]
  ...
  % the chain
  \draw[thick] (0.6,0) -- (57.4,0);
  \foreach \i in {1,...,57} \draw[black!55] (\i,-1.15) -- (\i,1.15);
  % the four axioms
  \foreach \i in {3,4,5,18} {
    \draw[very thick] (\i,-2.15) -- (\i,2.15);
    \fill (\i,0) circle (0.62mm);
  }
  \node[font=\scriptsize,align=center] at (4,5.6) {axioms 1--3\\ \emph{before the object}};
  \node[font=\scriptsize,align=center] at (19,5.6) {axiom 4\\ \emph{the observer}};
  \node[font=\scriptsize,align=center] at (11.5,-7.4) {\textbf{twelve links, no declared choice}\\ object $\longrightarrow$ algebra};
  \node[font=\scriptsize] at (0.6,-3.6) {$C_1$};
  \node[font=\scriptsize] at (57.4,-3.6) {$C_{57}$};
  \node[font=\scriptsize,align=center] at (41,5.6) {the Standard-Model-facing structure,\\ the crossing's no-go, and the chirality bit (\S\ref{sec:chirality})};
\end{tikzpicture}
\caption{\small The chain's fifty-seven links in the table's order, with each of the four axioms marked. The order is the
record's banking order and not a dependency order, so the \emph{position} of an axiom is not a claim about the derivation; what
is order-independent is the count --- four undischarged inputs --- and the typing of each link, and in particular that the
...
Fifty-three links are forced. The crossing's no-go sits where it fell, and the chain now runs past it to the
chirality bit.}
\label{fig:chain}
\end{figure}
...
9 & Theorem & the manifold group is congruence of level (4) in PSL(2,O\_K), index 12, and is not itself a principal congruence subgroup \\
...
17 & No-go & zero of the Standard Model's free parameters (nineteen, plus the neutrino sector) is reduced by the spectral route; two live claims dissolved \\
...
35 & Theorem & the replication shape: an su(3) index replicates fixed colour x su(2) types (sealed cell; the index is trinification structure, NOT a generation index) \\
...
42 & Identity & $\mathrm{Tr}(T_3^2) = 3$, $\mathrm{Tr}(Y^2) = 5$, $\mathrm{Tr}(T_3 Y) = 0$ on the 27: the 3/8 trace ratio, as structure -- the same ratio on the 5, 5bar, 10 and 16, so it discriminates nothing \\
43 & No-go & the sealed one-input crossing to measured values misses as single-step non-supersymmetric unification is known to ($\alpha_s$ by 35\%, $\sin^2\theta_W$ by 0.8\%); the desert is dead as a mechanism \\
...
52 & No-go & the $\theta$-odd frame is closed on every sl$_2$ germ TESTED; the universal form is a supported conjecture, as the paper states it \\
...
55 & Theorem & the geometric finite-twist index is identically zero \\
56 & Census & the index fires on the class and the tower, not on the member \\
57 & Theorem & the door: Q(sqrt-3) ramifies only at 3, its residue field is F\_3, pi\_1 surjects onto SL(2,3) = 2T in two classes, and McKay(2T) at the faithful 2-dim rep IS the affine E6 diagram (controls: Q8 gives affine D4; no 2I quotient) \\
\end{longtable}\endgroup

\noindent\footnotesize Recomputed from the table above rather than asserted:
36 theorem, 8 no-go, 6 identity, 4 axiom, 2 census, 1 corollary, totalling 57 links, of which \textbf{53} are not axioms. The four axioms are
...
discarded sibling is the nearest possible neighbour --- and, as \S\ref{sec:chirality} says where it costs us most, a square
is self-mirroring, so the amphichirality that withholds the chirality bit is downstream of this axiom.

\paragraph{The door, made a link.} One step of this construction has been narrated in prose since the
programme began and never entered the chain as a numbered link, and since it is the step everything downstream walks
through, we state it here and mint it. The invariant trace field $\Q(\sqrt{-3})$ has discriminant $-3$, so $3$ is its
\emph{unique} ramified prime, $(3) = (\sqrt{-3})^2$, and the residue field is $\mathbb{F}_3$; hence
$\mathrm{SL}(2, \mathcal O_K/(\sqrt{-3})) = \mathrm{SL}(2,\mathbb{F}_3) = 2T$. The manifold group surjects onto $2T$, in exactly
two classes up to automorphism. And the McKay graph of $2T$ at its faithful two-dimensional representation \emph{is} the
affine $E_6$ diagram --- seven nodes, six edges, marks $1,1,1,2,2,2,3$. Every step is a computation and none is a choice,
which is why it belongs in the chain rather than in the prose around it. \textbf{Both controls fire}: the same
construction on the quaternion group returns affine $D_4$, and the manifold group has \emph{no} surjection onto
$\mathrm{SL}(2,5) = 2I$, so this door does not open onto $E_8$. And the scope travels with the link: the surjection is
\emph{generic}, admitted by about one one-cusped census manifold in three (\S\ref{sec:generic}), so the link carries the
arithmetic to the algebra and carries no claim that the object is distinguished by doing so.

\paragraph{And the matrix is not merely assumed: it is forced, up to one bit.} We under-reported our own
genesis end and correct that here. The record carries a conditional uniqueness theorem for exactly this matrix,
machine-checked: from six individually motivated axioms of a minimal record-transfer system --- a two-record
substrate $\Z^2$; reversible integer transfer, so $\mathrm{GL}_2(\Z)$; orientation preservation, so
$\mathrm{SL}_2(\Z)$; a primitive one-channel update, which admits exactly the two shears $L$ and $R$; torsion-free
closure of the first mixed sector; and minimal complexity among admissible closures --- the persistent sector is
forced to $LR$, with $144$ candidates reduced to one and the trace, the $\varphi$-spectrum and the torsion identities
locked by a test that runs with the suite. The last two axioms are two routes to the same conclusion, one topological
and one algebraic, and either alone suffices. That this axiomatisation and the chain's own genesis are one
construction is itself machine-checked: the substitution morphisms' incidence matrices are exactly the non-negative
matrices of the group, the tick is a shear times the swap, orientation squares it to $LR$, and the resulting bundle is
the object. \textbf{One cost inside that dictionary we had not been carrying, and it is ours to carry}: the swap is
not a consequence of the six axioms. Our own early record says so in terms --- it ``is not currently a theorem of the
substrate'', becoming forced only on adding an exchange-symmetry axiom, ``plausible, but still an axiom'' --- so the
presentation that goes through the substitution rule spends the availability of that operation as well as the order it
produces. We list it here rather than let the two presentations appear to cost the same.

\textbf{Read the collapse as a consistency demonstration, not a search.} We say plainly how tight the forcing is,
because the number $144$ invites a stronger reading than it earns: the primitive-update axiom already fixes the two
increments to one, so the grid re-derives what that axiom stipulates; the last two axioms each suffice alone; and the
step from ``the torsion is $ab$'' to ``$ab = 1$ has the unique solution $a = b = 1$'' uses positivity, which the earlier
axioms supply and which over the integers alone would also admit $a = b = -1$. What the collapse shows is that four
independently motivated conditions agree, which is worth stating and is not the same as an outcome that could have
gone otherwise.

\textbf{What the theorem leaves is one bit, and it is the interesting one.} The six axioms are symmetric under
exchanging the two records, hence under $L \leftrightarrow R$, so they fix the sector only up to the order $LR$ versus
$RL$. The two orders are conjugate, so trace, determinant, characteristic polynomial and spectrum are blind to the
choice; what sees it is the \emph{based} invariant, the M\"obius fixed-point polynomial, which is
$\tau^2 - \tau - 1$ for one order and $\tau^2 + \tau - 1$ for the other --- $\varphi$ against $-\varphi$. That residual
choice is the smallest piece of inserted structure in the construction, and it is exactly where the golden number
rather than its negative enters.

\textbf{And the order of events, which a reader should have.} The object came first. The knot-theory turn is dated in
our own provenance record to the third week of May 2026 and the first probe naming the figure-eight to the twenty-second;
the axioms above were committed on the twenty-eighth. They were written to characterise an object already in hand, and
a reader should price them as such --- as we price the same fact about the selection theorems in \S\ref{sec:generic}. \textbf{And what the theorem does not do, in its own words, we repeat rather than
soften}: it does not derive its axioms from anything weaker --- the attempt to derive even the two shears from a bare
count-substrate stalled, with ``the substrate, positivity, primitivity, and order'' left inserted --- so the result is
conditional and is labelled so in the record. Two honest riders. The two routes are not independent: the theorem's
first axiom posits $\Z^2$, which is the punctured torus's first homology, so the geometric content enters at the start
rather than not at all, and we state the agreement as agreement and not as two independent derivations. And the
theorem stops before the field-theoretic lift by design.

\paragraph{And the axioms are not what survived a search.} It would be natural to read the genesis axioms as the
survivors of an elimination among candidate beginnings, and our own record forbids that reading. The project keeps a
registry of twenty-five enumerated routes by which ``nothing being unstable'' might produce a world. Its standing tally
is one dead, four stalled, three in progress in narrow form, and \emph{eighteen never touched at all}; no route has ever
reached the registry's own label for producing an observable; and the registry has not been updated since June while the
chain grew to its present length. So the honest statement is that four probes hit the same wall and the rest were never
run. The axioms are a characterisation we wrote, not a winner we found, and this paragraph is the evidence for the
scope note in \S\ref{sec:generic} rather than an exception to it.

\paragraph{And a negative that belongs beside it.} The charge frame above is the centraliser of the
\emph{finite} image $2T$, and the distinction is not cosmetic. The geometric holonomy is discrete and faithful, hence
Zariski-dense in its $\mathrm{SL}(2)$, so its centraliser is that of the whole $\mathrm{SL}(2)$ and not that of the finite
group: computed exactly on the constructed algebra, every centraliser available to a holonomy containing $2T$ in the
principal $\mathfrak{sl}_2$ is abelian of dimension at most four, while the Standard-Model algebra is non-abelian of
dimension twelve. \textbf{No flat connection of this kind leaves the Standard-Model algebra unbroken, whatever the
holonomy.} The Standard-Model algebra in this construction therefore arrives at the Wilson-line step on a closing, never
from the geometric representation, and we say so where the arena is built rather than leave the two centralisers to be
confused.
...
pre-registered\footnote{Pre-registration in this paper means one thing: the design text of a computation, with its outcomes and controls, is written and its hash sealed before the computation runs, on the project's own record; the seal ledger lists every such digest. The seal certifies \emph{integrity} --- that the design text has not changed since it was hashed --- and not chronology: design and results land together in the public history, so a reader cannot order-verify them from the repository alone, and the ancillary package's README says the same.} to test whether the quadratic field carrying the exceptional algebra could be reached
...
fifty-three forced links and then fails, provably and by its own sealed criterion, to reach
...
are exhibited on the object's own closings, in mirror pairs; they are vector-like; \S\ref{sec:chirality} proves the bit withheld by the object at every computed sector, reports the sectors of its class and its tower where a cohomological index is non-zero without claiming a generation from any of them, and reports that the one closing designed to supply the bit is excluded as it stands. \end{nonclaim}
...
non-supersymmetric single-step unification is known to~\cite{adf,ll}: $\alpha_s$ by $35\%$, $\sin^2\theta_W$ by $0.8\%$. The
running is imported Standard-Model dynamics, not anything the object supplies. \end{nonclaim}
\begin{nonclaim} that any four-dimensional dynamics, rate, or field action is supplied; the variational structure the record exhibits at the object's own monodromy level is a discrete Lagrangian on the object, not a field theory. \end{nonclaim}
\begin{nonclaim} that the observer's bit is consciousness, or that it is not. \end{nonclaim}
\begin{nonclaim} that every one of the chain's fifty-three forced links is stated in this paper in checkable form: \S\ref{sec:chain}
...
(C8): the faces are a property of the \emph{open} object, and closing it loses them. The manifold group is congruence: in Riley's
representation~\cite{riley} it has index $12$ in $\mathrm{PSL}(2,\mathcal O_K)$ (the covolume ratio $2.0299/0.16916$), it contains
the principal congruence subgroup of level $(4)$ --- its image in $\mathrm{PSL}(2,\mathcal O_K/(4))$ has index $12$, the group's own
index --- and not that of level $(2)$ (image index $6$), $(\sqrt{-3})$ or $(3)$ (image index $1$), so its level is $(4)$ (C9; an earlier
record read the level as $(8)$, where containment also holds); and it is not itself a
principal congruence subgroup --- $\Gamma(\sqrt{-3})$, the level-$(\sqrt{-3})$ group of the same index $12$, has four cusps. That the
figure-eight knot complement is a congruence manifold is stated in the literature~\cite{bakerreid}; the level is our own
computation, made twice on our benches. Its continuous spectrum
...
generic (in the algebraic sense: on a Zariski-open set, not the base-rate sense of \S\ref{sec:generic}) non-trivial second measurement lands on $\mathfrak{su}(3) \oplus \mathfrak{su}(2) \oplus \mathfrak{u}(1)^3$
...
structure, computed in the charge frame the holonomy's \emph{finite quotient} supplies --- the four-element torus is
$\mathfrak{z}(2T)$'s, the centraliser lattice is the algebra's, and \S\ref{sec:recognition} records that the landing algebra itself is classical --- and it is exactly what
...
\paragraph{Two structural positives on the carrier, stated at their strength.} The coupling on the constructed
carrier is \emph{unique}: the fundamental group alone leaves thousands of invariant trilinears on the matter
triple, the gauge condition cuts them to four, and the full exceptional algebra cuts them to one --- computed as a
nullspace, with no shape assumed. So the \emph{shape} of the Yukawa coupling is forced where its values are not.
And the three-family question has a classical mechanism we verified rather than cited: in the larger exceptional
algebra the adjoint decomposes with the matter representation appearing in a triple, permuted in two orbits of
three by an order-three element. \textbf{Nothing in that mentions the manifold.} The record holds three distinct
order-three structures --- the grading of the $27$, an arithmetic unit of the commensurator, and this family
rotation --- and whether any two are the same object is an open identification, not a result. We report the
mechanism as classical and the object-specific step as unchecked, because that is exactly what it is.

\paragraph{The arithmetic the object does reproduce.} One positive belongs here, stated at exactly its
strength. The object's volume is an $L$-value of its own field: $\mathrm{Vol}(m004) = 9\sqrt3\,\zeta_K(2)/\pi^2$ for
$K = \Q(\sqrt{-3})$, verified to thirty-two decimal places and reproduced by a second, independent route through the
state integral. The pattern continues into the asymptotic expansion of the object's own quantum invariant, whose
successive coefficients are rational multiples of alternating powers of $\pi$ and $\sqrt3$ against a fixed
$C_0 = |{\rm disc}|^{-1/4}$ --- a reality-parity law predicted before it was fitted and confirmed at five consecutive
orders. \textbf{This is detection, not derivation}: it shows the object's analytic data is built from the arithmetic
of its own field, which is what makes the disjointness result of \S\ref{sec:withheld} informative rather than trivial ---
the object has a rich arithmetic and it is still not the Standard Model's. We claim no value from it, and the tower's
later coefficients are known only to a stated precision floor.
...
holds, all of it structure. Of the $128$ sign-vector representatives swept --- the $\pm$-diagonal automorphisms that a $C$-stabilising automorphism of the $27$ can be, seven independent signs --- exactly two make the matter wall real, and both are the quaternionic
...
\paragraph{The seven crossings.} The construction was carried to the measured world under seal \emph{seven}
times. Three are given in detail below; the other four --- a phase exclusion where eight of eight windows missed, and
three further sealed comparisons missing at about five, about four and about three standard deviations --- are on the
record in the same form and add four negatives to this half of the paper. The three detailed here are these. The first (\S\ref{sec:recognition}) is the desert running from the $3/8$ boundary: the known
...
the object re-sees its own genesis at a canonical seam --- canonical in Lackenby's sense~\cite{lackenby}: for a once-punctured torus bundle the layered triangulation cut from the monodromy word is the manifold's own canonical decomposition, so the word is not a free label. And the two sides do not overlap: of the grid's $78$
closed hyperbolic fillings, \emph{zero} keep $\Q(\sqrt{-3})$ ($54$ in the first census, completed to $78$ of $78$ on
re-computation) --- and \emph{six are arithmetic}: $m004(\pm5,1)$, the Meyerhoff manifold, arithmetic by Chinburg~\cite{chinburg}
with invariant trace field $x^4 - x - 1$ of discriminant $-283$, and $m004(\pm6,1)$, $m004(\pm8,1)$, with cubic invariant trace
fields of discriminants $-59$ and $-31$; the other seventy-two are not (fifty-two have more than one complex place, fourteen
are unramified at a real place, and the deepest fields, of degree up to $31$, were recognised on one slope of each mirror pair
$m004(p,q) \cong m004(-p,q)$, an isometry re-verified for every pair used). An earlier version of this sentence said that none was
arithmetic. The census had tested for an imaginary quadratic invariant trace field --- the criterion for \emph{cusped}
arithmetic manifolds --- and not the closed criterion (exactly one complex place, integral traces, invariant quaternion
algebra ramified at every real place), by which the six pass; the corrected census, with the Hilbert symbols, is in the record.
The arithmetic a closing carries is in every case a different arithmetic from the object's: none keeps $\Q(\sqrt{-3})$. So the open object carries the
arithmetic that selects the exceptional algebra and no closing; a closed object carries a canonical closing and, where it is
arithmetic, an arithmetic that selects no exceptional algebra. \textbf{The two cannot be held at once} (C46) --- the same shape as the fork of \S\ref{sec:wall} one
...
\paragraph{The forcing, explicitly.} The six Standard-Model-visible field types of the content census are $(3,2)$, $(\bar3,2)$, $(3,1)$, $(\bar3,1)$, $(1,2)$, $(1,1)$ under $\mathrm{SU}(3)\times\mathrm{SU}(2)$, with $[\mathrm{SU}(3)]^3$ charges $+2,-2,+1,-1,0,0$; a content is a multiset of five, $\binom{10}{5} = 252$; the conditions applied are $[\mathrm{SU}(3)]^3$, the Witten parity of $\mathrm{SU}(2)$ doublets, the three linear hypercharge conditions below, and the cubic with rigidity (no continuous deformation of the charges preserving all conditions). On an SM-shaped $15$-plet the three linear conditions ---
...
\noindent\emph{Control, and a distinction the record keeps.} Two systems are in play. In the free-hypercharge frame of this paragraph the cubic is the condition that cuts the line to two points, and in the content census above no surviving content is isolated by the linear conditions alone (every survivor of the colour and Witten conditions has a two-dimensional linear solution space, and the cubic is what makes it rigid). In the record's \emph{in-frame} system --- hypercharges assigned within a fixed colour frame on the $27$'s states --- dropping the gravitational condition degrades the forcing from a single line to thirty-six solutions and a family of solution planes, and there the cubic adds nothing to the three linear conditions. The appendix row for this control names the in-frame system.
...
\paragraph{The cascade terminates, and the termination is the theorem under the stated menu.} The descent lands on the $A_2{+}A_1$ Levi ---
...
$352$ pairs}, of which $351$ fall below two significant figures. At that bar the expected number of accidental agreements among $352$ pairs of order-one numbers is itself of order three, so the scan bounds our search rather than tests the object; a real relation would have to survive at the target's own precision, which the record states per target. \textbf{The single exception is dismissed on
...
\paragraph{The strongest coincidence in the record, volunteered.} Before the near-miss we found by scan,
we give the one a reader would most want to test us on. The Koide angle --- the phase of the charged-lepton mass
vector in the parametrisation where Koide's relation is the statement that it lies at a fixed angle --- sits
$0.89$ standard deviations from $2/9$, a difference of $7.4 \times 10^{-6}$ against an experimental $8.3 \times 10^{-6}$.
That is far closer than anything else in this paper. \textbf{We do not claim it, for reasons that are structural rather
than statistical}: it is a property of measured masses rather than anything the object derives, the value $2/9$ was
identified after the number was computed and not before, and the quantity it would have to connect to lives in the
sector our own no-go shows carries no invariant content. It is in the record, firewalled, and it has not moved in a
year. A reader who wants a target for this programme should aim at that number rather than at ours.
...
the derived sixteen once $\nu^c$ is included, and is removed only by a second expectation value, the one \S\ref{sec:chirality}'s supersymmetric vacuum exhibits. That second expectation value is a condition, not a point: the rank drop from five to four happens only at a \emph{pure} spinor of the $16$ (stabiliser of dimension $34$ with toral rank $4$; a generic spinor is fatal, dimension $29$ and toral rank $0$), so the two expectation values are two selections, and the ledger books them as two. So rank reduction is not absent from the construction: one unit is free and one is paid for by a vacuum
...
transport to a real 3-manifold with boundary is nowhere exhibited --- and the obstruction is sharper than an absence, in two ways. Under the Lorentz double the $27$, the $78$ and their whole tensor tower are integer-spin, and the record's only half-integer object is the holonomy's own $\mathbb{C}^2$. And for flat data the transport cannot produce chirality at all: a flat bundle has zero curvature, so by Chern--Weil its rational Chern classes vanish and its Chern character is its rank, and the Atiyah--Singer index~\cite{atiyahsinger} of a Dirac operator twisted by it on a closed spin four-manifold is $\mathrm{rk}(V)\cdot(-\sigma/8)$ --- blind to which representation $V$ is. The $27$ and the $\overline{27}$ have the same rank, so \emph{any} route ending in a four-dimensional Dirac index with flat gauge data returns the same number for both, before the object is consulted. We state the two directions this cuts in. It converts the missing transport from a gap into an obstruction; and it applies equally to the non-zero values reported below, which are therefore counts of twisted classes and not four-dimensional indices. Chirality in this frame would need what the object does not supply --- curvature, flux, or a singular locus --- which is also why the one closing mechanism we follow to its obstruction is a conical one. Below, ``generation'' means a complete copy of
...
ones included, is $\chi(\mathrm{Fix}\,g) = 1 - s_\mu(g) \in \{0,2\}$, where $s_\mu(g) = \pm 1$ is the sign by which $g$ acts on $H_1 = \Z$, read off $H_1$ alone (the order-four elements fix
...
it does not. The localized count of Pantev--Wijnholt~\cite{pw}, the signed zero count of a Higgs eigenform, is $0$ or $4$ on the cyclic descent --- and for every $\theta$-odd field with a transverse zero set it is zero whichever mode leads, since $\theta$ exchanges the positive and negative regions of the cusp torus and their Euler characteristics sum to $\chi(T^2) - \chi(Z) = 0$; the ``$\pm 4$'' of a product mode is a zero set with crossings, where the count is undefined rather than non-zero --- and across the $87$ covers of the object to degree $10$ no isometry rotates a cusp by order three --- a
census that is not vacuous, and we say why: fourteen of those covers do carry a hexagonal cusp ($16$ of the $201$ cusps),
the only flat torus admitting an order-three automorphism, so fourteen pass the necessary condition and none realises the
rotation; the isometries are read from canonical triangulations, not from the triangulation as given. \textbf{Three appears only through the $\Z/3$ descent}, and the descent, we show next, is
...
golden face, since no fibration of it, nor of the other eight members of its class to nine tetrahedra (an earlier count of six rested on an isometry list read off the triangulation as given; the canonical list finds nine to nine tetrahedra and twenty in the census to ten, none keeping the golden face where tested), carries $t^2 - 3t + 1$.
...
the obstruction is exact: a charge locus --- the zero set of the Higgs direction's one-form, where the localised count of Pantev--Wijnholt is taken --- on the fixed set of $\theta$ is $\theta$-even, so equivariance forces the
...
the fixed set, so its zero set misses them: \textbf{the object supplies no charge locus there.} The involution has two lifts to $E_6$. Under the outer lift (fixing $F_4$) the chamber dichotomy just stated is the wall, and
...
in question are outside every such class, so $I$ is computed rather than cited. One proof and one lemma organise everything that follows. The proof first, with its own limit stated
in the same breath: where the cusp-trivial characters have a positive-dimensional identity component $S^0$, every
$W$ whose restriction to $\partial M$ is self-dual has $I(W \otimes \chi) = 0$ for all $\chi \in S^0$, by constancy of the
cusp invariants along $S^0$, lower semicontinuity of the restriction rank and irreducibility of the torus. It is the
record's only \emph{proof} of the vanishing rather than a computation of it, and it does not cover the cases we most
need: $\dim S^0 = 0$ for every one-cusped manifold --- the object included --- and for $52$ of the $54$ chiral covers, so
on the object itself the statement is empty and the vanishing there rests on the theorem above instead.

One lemma organises everything that follows. By definition $I(V) = n(V) - n(V^*)$, so
\textbf{$I$ vanishes on any self-dual local system}, and the useful half of that is structural: the reducible
representations used here take values in $\mathrm{SL}(2)$, where the standard module is symplectically self-dual
($J\rho J^{-1} = (\rho^{-1})^{\mathsf T}$ for $J = \left(\begin{smallmatrix}0&1\\-1&0\end{smallmatrix}\right)$, an identity
equivalent to $\det\rho = 1$ and therefore indifferent to whether $\rho$ splits). Hence
$\mathrm{Sym}^m(\rho) \cong \mathrm{Sym}^m(\rho)^*$ for every $m$, and $V = \mathrm{Sym}^m(\rho)\otimes\psi$ has
$V^* \cong \mathrm{Sym}^m(\rho)\otimes\psi^{-1}$: \textbf{the index can fire only through a twist that is not its own
inverse, never through the non-split structure alone}. The witness obeys it --- untwisted and order-two twists of the same
module give $0$, the order-six twist gives $+1$ --- and across the record's $542$ firing modules not one has
$\psi^2 = 1$. The two theorems below are the lemma's instances, and they are why the escape route is where it is:
non-self-duality is exactly what a unitary or finite-order-symmetric background cannot have. Two theorems make it vanish on
...
that frame --- the higher-rank components of the character variety, the singular closing --- is untouched. Two further facts fix the index's standing, and they pull in opposite directions. First, a theorem: for $V = \mathrm{Sym}^m(h) \otimes F$ with $h$ the geometric holonomy and $F$ of finite image, the restriction $H^1(M;V) \to H^1(\partial M;V)$ is injective~\cite{mfp}. We state the hypothesis the source states, because it is wider than we once used: the theorem asks only that $M$ be complete, non-elementary and \emph{topologically finite}, with no condition on the number of cusps. It does carry one hypothesis on the coefficients, which we state with it: the symmetric power must be non-trivial, $m \ge 1$ (the source's $n \ge 2$), so the theorem says nothing about the untwisted case and we do not use it there. The cited theorem is
about the geometric holonomy, so we say how it descends rather than leaving the step to the reader. On the finite cover
$\pi: \widetilde M \to M$ of degree $d$ that trivialises $F$ the pullback $\pi^*V$ is a direct sum of copies of
$\mathrm{Sym}^m(\widetilde h)$, where the theorem applies as stated; a class in $H^1(M;V)$ killed by restriction to
$\partial M$ pulls back to a class killed by restriction to $\partial\widetilde M$ and so vanishes upstairs; and $\pi^*$
is itself injective because transfer composed with it is multiplication by $d$. That last step is where the
\emph{characteristic-zero} hypothesis is spent, and we state it rather than assume it. So the interior image vanishes and $I(V) = 0$ identically --- on the object, on every cover, on every census manifold. Every geometric-holonomy sector above vanishes for this reason, and the pre-registered test on the cyclic tower could not have passed once it was known. Second, the index is nonetheless a demonstrated instrument: it is non-zero, exactly and in characteristic zero, on \emph{reducible non-split} modules $\mathrm{Sym}^m(\rho_\chi) \otimes \psi$ built at root-of-unity characters $\chi$ with $H^1(\pi;\chi^2) \neq 0$ --- first on the census manifold $m010$ ($I = +1$, semisimplification $0$), then, with the same instrument validated on that witness before any new number was read, on five one-cusped chiral members of the object's own commensurability class ($s958$, $v2873$, $t12833$, $t12835$, $o10\_150701$: $I = \pm 1$, once $\pm 2$, every semisimplification $0$, the $s958$ value re-derived on three presentations) and on the object's own degree-four cyclic cover $t12839$, at their cusp-trivial characters of order three among others. Four of the five are exactly the manifolds an earlier search had chosen for their cusp-trivial characters and found zero on the geometric holonomy. On the object itself the only reducible locus is the golden one ($\Delta = t^2 - 3t + 1$; $H_1 = \Z$ has no torsion), and every module computed there --- $m \le 4$, twists of order dividing twelve, $235$ modules --- returns zero. What fires is non-semisimplicity, a unipotent Wilson line rather than the geometric structure. Such a module is not a background of a compact gauge group, whose flat holonomies are semisimple; it could arise only in the complexified theory, whose boundary theory is exactly what \S\ref{sec:recognition} records as not established --- so its admissibility is an open question with a prior against it, and we grade it so. The rule that separates the five members that fire from the six examined that do not ($o10\_150697$, $o10\_150700$, $t12837$, $s956$, $t11365$, $m208$; and the degree-three cover $s961$) is not known.

\paragraph{The closings carry three structural copies of the Standard-Model content, in mirror pairs.} Two operations
are both called closing in this paper and they must be kept apart --- and a third sense, the observer's
closing set of \S\ref{sec:observer} (the torsor of discrete choices, axiom $18$), is a different object again, sharing
only the word; our own record once carried a contradiction between two of these senses and resolved it by noticing
that. The two geometric ones: Dehn filling of the cusp (\S\ref{sec:middle}'s seam,
...
mirror pairs}, vector-like; the twelfth is a second such closing, on which the colour triplet thins to one copy on $31\,488$ of its $34\,752$ Standard-Model lines, and on $768$ of them a tree-level flat direction leaves exactly one light colour-triplet pair and three light doublet pairs --- one vector-like generation with two Higgs doublets and no exotic triplet, the closest approach to the Standard Model's field content in the record, and vector-like still. On the ninth closing the doublet--triplet splitting is impossible by theorem: none of its $706\,464$ Standard-Model lines projects out a single colour triplet while keeping its doublet partner, because the triplet's weight is minus twice the quark doublet's on every line. Nothing in the construction selects the closing index, and a vector-like
multiplicity carries no generation content: three at the ninth, one at the twelfth, are counts of copies and no more. \emph{What follows is conditional on assumptions the construction does not supply, and it is recorded as a fork rather than as a result.} The vacuum analysis that follows is a \emph{supersymmetric} $E_6$ model of the kind studied as the exceptional supersymmetric Standard Model~\cite{kmn} (whose premise, chiral matter, this spectrum lacks). The supersymmetry is the model's assumption and not the object's supply: the object's own carrier admits no supercharge --- none equivariant under the fundamental group, none equivariant under the gauge algebra, and its one candidate involution squares to the meridian and is not one --- a negative the record banks; what follows is a conditional analysis of a model suggested by the closing. The model assumes F- and D-flatness on the singlet fields, a tree-level superpotential, and
...
and must be broken by the soft sector (a $Z'$ broken only at the electroweak scale, with $O(g)$ couplings to first- and second-family leptons, is excluded by the collider limits~\cite{atlasdilepton}). Its first consequence is a \emph{regime}, not a value. If a light family carries the expectation value, and
...
so the bounds constrain a supersymmetric model suggested by the closing, not the closing --- and a vector-like pair admits a gauge-invariant supersymmetric mass and can decouple, so the model must further assume the copies are kept light, an assumption we do not derive --- and three light vector-like copies of a complete $27$ change the one-loop coefficients enough to put gauge-coupling perturbativity below the unification scale in question, a cost the model would have to pay.} Which family it is, the construction
...
\paragraph{The closing that could carry the bit, and what it costs.} Since every closing of the object's tower is vector-like and every geometric sector of the index is zero by theorem, chiral matter can enter only where the closing is curved: at a conical point where the $E_6$ locus meets another ADE locus, with the chiral $E_6$-charged matter forced there by anomaly inflow~\cite{aw}. The local model exists --- the $G_2$ cone over $\mathbb{CP}^3/2T$, the twistor cone of $S^4/2T$, whose $E_6$ and $A_1$ lines meet only at the apex, where the enhancement is to $E_7$ (the $E_7$ point); its geometry is exact and its count, one $27$ per apex, is the literature's rule and is cited, not re-derived. Three such apexes permuted by a $\Z/3$ would give the three; the $\Z/3$ available is the object's own $2T/Q_8$, the deck of the flat closing $Y_3$, and a triple it permutes carries zero charge under every deck-invariant class, so a symmetric triple needs a closing with $b_2 \ge 2$ on which the $\Z/3$ moves the harmonic forms --- a necessary condition on any such closing, not a design option. On $Y_3$ itself the breaking is available by a non-abelian flat line: the Standard Model's centraliser in all of $E_6$ is $\mathfrak{su}(2)_\beta \oplus \mathfrak{u}(1)^2$ (the Standard-Model-singlet roots are exactly $\pm\beta$; inside the rank-six Levi of \S\ref{sec:middle} only the $\mathfrak{u}(1)^2$ survives, the two roots $\pm\beta$ lying outside it), a $Q_8$ in $\mathrm{SU}(2)_\beta$ times an order-four character of $H_1(Y_3) = (\Z/4)^2$ leaves exactly $\mathrm{SU}(3)\times\mathrm{SU}(2)\times\mathrm{U}(1)_Y\times\mathrm{U}(1)'$ (twelve order-four torus elements, four reaching the Standard Model, one common square --- root-system facts, recomputed), and the $\mathrm{U}(1)'$ is exactly the $\eta$ direction of the rank-five $E_6$ breaking, family-universal. Its price is the design's first obstruction: both $E_6$-charged Standard-Model singlets of the $27$ carry the same $\eta$ charge and no field of the $27$ or the $78$ carries minus twice it, so no $\eta$-neutral Majorana mass exists, $\mathrm{U}(1)_\eta$ is exact above the soft scale, and under the cubic's one coupling the neutrinos come out Dirac at the up-quark masses --- six to eleven orders of magnitude above the sub-electronvolt scale, from the up quark to the top. The design's flavour is fixed at tree level too, and refuted there: with the Higgs fields in the apex $27$s the two apex $\mathrm{U}(1)$s allow only the cubic $27_1 27_2 27_3$, so every charged-fermion mass matrix is complex symmetric with zero diagonal, whose largest singular value equals the sum of the other two exactly --- against factors of about $273$, $50$ and $17$ in the up, down and charged-lepton sectors; a deck-symmetric Yukawa has a degenerate pair, so the deck must be broken, and by Weyl's inequality the part of each mass matrix that violates the apex $\mathrm{U}(1)$s is at least a third of the third-generation Yukawa. The hierarchy, if this design carried it, would be an order-one effect of the sector that breaks the apex symmetry. \textbf{The three-apex design is excluded as it stands}; a $27$ sector of the opposite chirality (anti-apexes) is what would repair it, and no compact $G_2$ \emph{manifold} realising any of this exists in the record. What the record does hold is one compact flat $G_2$ \emph{orbifold} assembled from the object's own data --- the $E_6$ fibre $\mathbb{C}^2/2T$ closed by the Hurwitz torus under the Hantzsche--Wendt group, one invariant spinor, singular loci of types $E_6$, $\mathfrak{so}(8)$ and $\mathfrak{su}(2)$ --- and it has $b_2 = 0$, which fails the condition $b_2 \ge 2$ two sentences above. The closest thing the record has to the closing this design needs is therefore a counterexample to its own requirement, and we report it as such rather than as an absence. This paragraph reports a design verified step by step, not a theorem, and it is the closest the record comes to saying what a closing that carries the bit must be.
...
(length at most three), no three-record carrier keeps $\Q(\sqrt{-3})$; at length four one does --- the two-cusped $m412$, a three-record bundle on the twice-punctured torus, chiral, with every tetrahedron shape in $\Q(\sqrt{-3})$ and commensurable with the object at two and a half times its volume --- and it lacks the $2T$ door. The fork is therefore fragile past depth three, and its price is not the atom: on a substrate the arithmetic and a remembered handedness can be found together at the price of the door, and the object's own covers, below, are where they are found without that price.
...
is a relation}: fix which letter is the charge and which way the record grows, and the hand follows. \textbf{And we say
the uncomfortable thing first, because it is true}: the orientation axiom of \S\ref{sec:unique} (link $5$) bought
orientability by squaring the genesis matrix, and a square is self-mirroring by construction. Our own record states
this more sharply than we have, and we adopt its form: \emph{every} wall downstream of the mirror-as-self-isometry is a
property of that axiom rather than of the object --- orientation double covers of the non-orientable cusped census are
amphichiral in $40$ of $40$ cases, against a census base rate far below one. The object's
amphichirality --- the property that withholds the bit --- is downstream of the one axiom this paper already prices as its
most expensive. A reader who wants to read this section's central negative as a consequence of that axiom is reading it as
we do; what the section adds is that the negative survives every route we could find around it, including the covers that
\emph{are} chiral. That this
is the discrete shape of a familiar physical pattern is a resonance we note in the non-claims and do not use.

\paragraph{One move that is open, and has not been made.} The obstruction this section keeps meeting is
amphichirality, and there is exactly one operation in reach that breaks it while leaving a cusp: \emph{partially}
filling a multi-cusped cover --- fill some cusps, keep others. Filled manifolds are chiral, so the move escapes the
mirror; keeping a cusp preserves the boundary the index needs. A witness exists and is not ours alone: on a degree-five
three-cusped cover, filling one cusp along $(2,1)$ leaves two cusps and Chern--Simons invariant $+0.157590$, outside
$\{0,1/4\}$, interval-certified at $100$ and $160$ bits by a second seat. What it costs is three discrete choices --- which
cover, which cusp, which slope --- none of them the object's, and what it cannot do is supply a count: filling only
lowers the number of boundary tori. We have not run the index on such a manifold. It is the one construction we can name
that attacks this section's central negative without leaving the object's own commensurability class, and we state it
unrun rather than leave the negative looking unassailable.

\paragraph{And if the bit were granted.} We measured that too, rather than assume it would be decisive.
Granting chirality as a declared input de-conditionalises four downstream results and costs the construction its one
stated prediction --- the $Z'$ regime of the closing, whose family charges the anomaly conditions then forbid, since
granting chirality forces the family part to the shape $(0,t,-t)$ and the record's closing carries $(-10,5,5)$. The net
movement on the chain is zero rungs and one sharpened edge. The bit is not a key that opens the remaining doors; it is
one input among those the ledger lists, and the programme does not complete on receiving it.

\paragraph{The sharpest form in which the question stays open.} A relative index formula of the kind used for
boundary problems writes a net chirality as $\chi(M, \partial^+M)$ for a distinguished part $\partial^+M$ of the boundary.
We cite that shape rather than derive it, and note what it says here: for any compact $3$-manifold with torus boundary
$\chi(M) = 0$ and $\chi(T^2) = 0$, so the whole content sits in $\chi(\partial^+M)$ --- zero identically if $\partial^+M$
is assembled from annuli or tori, $-1$ for a disc, $+1$ for a pair of pants. \textbf{What $\partial^+M$ is, we have not
identified}, and the obstruction is worth naming exactly, because it is not a computation we have failed to finish. It is
a \emph{definition we have never written down}: which boundary condition picks out $\partial^+M$. Our own attempts split
on precisely that choice --- with the whole torus or with an annular decomposition the count is zero identically, while a
disc convention gives $-k$ for $k$ components --- so the answer to this section's central question is currently decided by
a convention rather than by the manifold. Every decomposition our record has produced is annular, which returns zero with
no representation theory entering at all. We state it as the open question, and as the one we would most like taken from
us, because it is the sharpest form of this section's subject: on a manifold with torus boundary, where could a non-zero
net count come from at all?
...
sectors on the cyclic covers, all zero. The index on several boundary tori has since been derived from the same pair sequence and computed zero on the $54$ multi-cusped chiral covers ($38\,070$ sectors). Because the injectivity theorem carries no one-cusp hypothesis, those zeros are \emph{forced} by it exactly as the one-cusped ones are, and the computation confirms a theorem rather than standing alone; what the run still owes is reproducibility --- its scripts read a cache by absolute path and do not yet run from a clean checkout --- and that is a debt of engineering, not of mathematics. Beyond the tower, the class: of the $112$ census manifolds whose tetrahedron shapes lie in $\Q(\sqrt{-3})$, chirality and the count of three vary while the $2T$ door determines neither; three two-cusped members carry chirality, the arithmetic, the door and the count of three together ($m202$, $s959$, $o10\_150726$), and none of them is a cover of the object --- the object's degree-two and degree-three covers are unique and one-cusped. The count of three is the class's and not the tower's; the index, as stated above, is both.
...
so, and so do we: that zero is forced, not passed. The index's non-vacuity, once the open question, is now settled on the class and the tower; what remains open on the object is its own reducible locus beyond the modules computed (\S\ref{sec:wall}).
\end{scopenote}
...
trace field, a complex conjugation on a cyclotomic sector, and the geometric mirror. A family-wide
strengthening we previously asserted here has been withdrawn and we state the corrected figure rather
than drop the row: over the $112$ census manifolds all of whose tetrahedron shapes lie in
$\Q(\sqrt{-3})$, $38$ are amphichiral and $74$ are chiral. The earlier ``all members are amphichiral''
reading rested on an orientation-blind isometry test; the object-level fact --- that $m004$ itself is
amphichiral --- is untouched, and it is the only one this section uses. The content of the proof is
...
\paragraph{The bit is relational, and we have corrected our reason for saying so.} The ground is a
theorem about knot complements: the manifold's mirror is the product of two other involutions, the fibre's
reflection (the swap) and the flow's reversal (the arrow), so handedness is not a third datum --- fix which letter
carries the charge and which way the record grows, and the hand follows. \textbf{That is the whole of the claim},
and it means the bit is not an independent input beside those two rather than that it is free.

An earlier version of this section argued the point differently, from a heterogeneous pair: the mirror is
realisable over $\mathrm{GL}_2(\Z)$ only with determinant $-1$, and the resulting $\Z/2$ class was read as the
orientation bit. \textbf{That reading was tested by us and refuted, and we state it rather than quietly
replace it.} The class is $2 - \kappa$ modulo squares with $\kappa$ the Fricke commutator trace, a function of
the pair's three traces, all of which the mirror fixes; the class is therefore mirror-\emph{even}, constant
across the branches a chirality bit would have to separate, and cannot be that bit. What survives the
refutation is worth keeping and is weaker than what we claimed: a pair does carry a well-defined, dimensionless,
non-vacuous $\Z/2$-valued datum; the object supplies its own partner for it, the realiser of its own
amphichirality, so the supply is internal modulo the orientation axiom; and the class trivialises over the
partner's eigenvalue field for \emph{every} pair, so only \emph{which} field appears is the object's.

\paragraph{And it requires no act of selection.} On the surviving ground the point stands: the swap and the
arrow are the inputs, the hand is their product, and no third choice is made --- which matters, because an
invariant selector cannot pick a point of a free orbit (an elementary fact, and the whole content of
``no canonical choice'' here).

\paragraph{The criterion is classical, and we decline the coincidence.} The invariant deciding
whether a pair carries that class is the Fricke commutator-trace discriminant
...
and is never demanded again. The observer's discrete cost does not accumulate with the depth or the age of what is described.
\paragraph{Which member.} One further item belongs on this inventory because \S\ref{sec:chirality} put it there. The arithmetic content of the chain is a commensurability-class invariant; chirality, the count of three and the index are not, and within the object's class they are carried by members the object does not cover. The genesis selects the member: $m004$ is the unique orientable solution of a minimisation, its Jørgensen number being exactly one~\cite{jorgensen,callahan}, and that selection is variational where every earlier selection story was descriptive. Keeping the member therefore costs the bit; moving the object to the class would recover the bit at the price of a selection the genesis no longer makes --- which member, priced like a closing. We register the fork and do not resolve it.
\paragraph{The budget of the residue, counted.} At the top of the tower examined --- the chiral five-cusped cover of degree ten --- the deformation space of the flat $\mathfrak{sl}_3$ structure has complex dimension ten, exactly the boundary count of five cusps at rank three: twenty real parameters, every one of them cusp data, with no interior modulus and no selector the object supplies. The residue this section prices is a counted one.
...
them. The construction supplies \textbf{none} of the Standard Model's nineteen free parameters (twenty-six with Dirac neutrino masses and mixings, the neutrino
...
\begingroup\footnotesize
\setlength{\LTleft}{0pt}\setlength{\LTright}{0pt}
\begin{longtable}{@{}>{\raggedright\arraybackslash}p{3.2cm}>{\raggedright\arraybackslash}p{2.7cm}>{\raggedright\arraybackslash}p{8.4cm}@{}}
\toprule
...
The object's own modular flow is trivial, as for any tracial algebra: the modular operator of a trace is the identity, and a
...
it decided nothing. What remains is a single named datum --- a character comparison whose two branches are both already proved, one leaving
the row permanently short and the other closing it --- and we must report its status honestly: the lane that held that
computation was retired before delivering it, so the datum is currently unowned rather than in progress. It is a bounded
lookup, not an unbounded search. \\[2pt]
the chirality bit & one $\Z/2$ &
\textbf{Relational, not a third input.} On every knot complement the mirror is the product of the swap
and the arrow (\S\ref{sec:chirality}), so once those two are fixed the hand follows and no further selection is
made. It is booked here as relational for that reason and for no other: an earlier justification from a
heterogeneous pair was refuted by our own computation (\S\ref{sec:observer}), and what it leaves is a
relational $\Z/2$ datum that is \emph{not} the orientation bit. The swap and the arrow are themselves
supplied, and are listed. \\[2pt]
...
\textbf{Closed, in the only available form, and for the family menu by a theorem.} An invariant selector cannot pick a
point of its own orbit, so a menu is terminal, not a deficiency. For the family index we can say more: three interchangeable
generations would need a cyclic cubic trace field, and \emph{no hyperbolic knot has one} --- a cyclic cubic field is totally
real while a hyperbolic invariant trace field has a complex place, so the two classes are disjoint; the object's own field is
quadratic, which permits multiplicities one and two and never three. The object cannot carry the index, whoever chooses.
(Three menus share this row; counted as choices rather than rows the non-continuous inputs number nine, and the filling
menu is finite only in the sense of the ten exceptional slopes --- as a choice of four-dimensional filling it is infinitely
non-unique with nothing to select among them.) \\[2pt]
...
being a declared input; that chirality is not a commensurability invariant is elementary (an orientation-reversing symmetry descends to a cover only when it normalises the subgroup), and the count on the object's own tower is ours; what \S\ref{sec:chirality} adds is only its coexistence with the
...
on the covers.}
\item \textbf{Lackenby's canonical decomposition of once-punctured torus bundles}~\cite{lackenby} is why the seam of \S\ref{sec:middle} is the manifold's own and not a choice of triangulation; used as stated.
\item \textbf{Menal-Ferrer--Porti's injectivity}~\cite{mfp} is the theorem behind \S\ref{sec:chirality}'s vanishing of the geometric finite-twist index; we transfer it through a finite cover and add nothing to it.
\item \textbf{Jørgensen's inequality and Callahan's uniqueness}~\cite{jorgensen,callahan}: that the figure-eight knot group is the only torsion-free Jørgensen group --- the only one attaining Jørgensen number one --- is Callahan's theorem; what we add is only that the record's founding selection is thereby variational rather than descriptive, which selects the manifold and nothing downstream of it.
\item \textbf{Minsky's classification of punctured-torus groups}~\cite{minsky} names the object's family --- free two-generator Kleinian groups with parabolic commutator --- and the record had every ingredient of that definition except the theorem itself.
\item \textbf{Kac's classification of finite-order automorphisms}~\cite{kac} is the classical input behind the involution count of \S\ref{sec:middle} and the order-three Wilson lines of \S\ref{sec:chirality}'s closing paragraph; used as stated, not re-derived.
\item \textbf{Chern--Weil theory and the Atiyah--Singer index theorem}~\cite{atiyahsinger} give the flat-bundle obstruction quoted in \S\ref{sec:chirality}'s scope note --- a twisted Dirac index on flat data is a function of the rank alone; we add nothing to them and use the consequence in both directions.
\item \textbf{Complex Chern--Simons theory}~\cite{witten91} has infinite-dimensional Hilbert spaces and no established boundary conformal field theory; this is why the quantum face of the object is asked no generation question in this paper.
...
Six of the following would refute a specific claim of this paper, and for each we state the computation that settles it. \textbf{The others are not falsifiers} --- they are upgrade triggers, and we list them here rather than hide the asymmetry.
...
\item \textbf{Exhibit a proper descent below the Standard Model algebra that remains registerable.} This kills the termination theorem.
\item \textbf{Exhibit a vector-like exotic outside $\{3+\bar{3},\,|Y| = 1/3\} \cup \{2+2,\,|Y| = 1/2\}$ in any closing's Standard-Model vacuum.} This kills the exotic content \S\ref{sec:chirality} reports; none has been found, and we list it as the cleanest open conditional in the record rather than as a test passed.
\item \textbf{Exhibit a non-zero index on the object's own golden reducible locus}, at a symmetric power or a twist beyond those computed ($m \le 4$, twists of order dividing twelve). This would not refute a claim --- the class and the tower already fire --- but it would move the bit onto the member and empty the last row of \S\ref{sec:wall}'s third column; the modules run are listed so that the search can be continued rather than repeated.
...
dual system.} on an unprotected twist sector of a chiral cover of the object.} Executable now on any one-cusped chiral cover with the instrument of \S\ref{sec:chirality}; on the multi-cusped covers only through the several-cusp form, whose run is not yet reproducible from a clean checkout --- though there the injectivity theorem already forces the answer, so a falsifier can only live outside the geometric finite twists. This too is an upgrade trigger rather than a falsifier: it would move the chirality
...
All six falsifiers above are \emph{mathematical}: each asks a reader to exhibit an object,
...
A second handle is weaker, conditional on every assumption \S\ref{sec:chirality} lists for it, and we state it at its true strength: the tree-level vacuum of the Standard-Model
...
\paragraph{Why the pieces do not meet, diagnosed.} The fork of the chain (link $44$) is usually stated as a
count --- the joint centraliser of spacetime, colour and hypercharge is zero, any two of the three and never all
three --- and the record can say why in structural terms. The two real forms the construction can land in are
reached differently: one is an inner twist, available to a torus element of the algebra; the other, the branch that
would carry a Lorentz signature and compact colour together, has maximal compact subalgebra $\mathfrak{f}_4$ of rank
four, and \textbf{no torus element reaches it} --- crossing to it needs an outer automorphism, a diagram symmetry,
not a conjugation. That is a rank obstruction rather than an accident of bookkeeping, and it is the sharpest form in
which we can say why the interactions do not meet inside this algebra. The one operation in the construction that
could in principle cross it is the mirror $\theta$, and the record fences that twice: $\theta$ is trivial on the
character variety, and $\theta$-odd data destabilises the very chamber the crossing would have to land in. We state
the obstruction because it is the honest content of the fork, and we do not claim the crossing.

\paragraph{What we would do next, and what each would cost.} A reader is entitled to ask what the programme's
own next moves are rather than only where it stopped. Four are named, in the order we would run them. \emph{One},
the outer-automorphism question just stated: does the object's mirror act as an outer involution, and with what
fixed subalgebra --- a bounded computation that would either cross the fork or close it, and it would discharge two
rows of the identification ledger. \emph{Two}, the boundary identification behind the $\sigma$ row: if the stage
algebra is the object's own boundary algebra, that row converts from an anchor to an output and the input list
shrinks. \emph{Three}, the reducible-locus index on the twelve chiral one-cusped covers, which is exactly the
object falsifier $6$ asks for and which our own runs have not covered. \emph{Four}, an independent
re-implementation of the hypercharge forcing from a different starting stack: it is the one result of this paper
that has never been checked by a route sharing no code with ours, and we say so rather than let its two internal
confirmations stand as three. None of the four is a search; each is a stated computation with a stated cost.
...
\paragraph{The ledger in three columns.} The reader who wants the whole boundary in one place can have it. \emph{Had, and parameter-free}: the object up to one bit and its selection as a minimiser; the invariant trace field $\Q(\sqrt{-3})$ and the $2T$ door; $E_6$ and the $27$ by McKay; the termination of the breaking chain under the stated menu; three structural copies of the Standard-Model content on the closings, vector-like; the exhibited non-zero index on the class and on the tower; the base rates of \S\ref{sec:generic}. \emph{Had, given the inputs the freedom ledger lists} (the colour frame, the Standard-Model shaping, the electromagnetic identification): the direction of hypercharge and the $\Z_6$ global form; the breaking of the flat closing $Y_3$ to the Standard Model times $\mathrm{U}(1)_\eta$. \emph{Not yet had, with the computation named}: a non-zero index on the object's own reducible locus beyond $m \le 4$ and twists of order dividing twelve (a finite computation; the modules run are listed) --- which would move the chirality bit onto the member; the several-cusp index reproduced from a clean checkout; the rule separating the class members that fire from those that do not; a compact $G_2$ closing carrying the apexes, and its $27$ sector of the opposite chirality; the earned identification of a count with a generation; the supersymmetric crossing; a candidate for the $\sigma$ row meeting the six conditions above; a second condition on the projective Higgs line $\PP(B_0)$ closing it to a finite point set (the named datum in progress elsewhere). \emph{Closed, each by a stated theorem} --- what we believe cannot be had, and why; the theorems are themselves had, and are listed here once, with the universal ones marked, since a no-go true of every theory weighs less than one about this object: a Standard-Model ratio among the object's periods and regulators (exhaustive at stated bounds; object-specific); the hypercharge normalisation from the anomaly equations alone (homogeneity; universal); the scale (Mostow, universal --- and, object-side, a finite-group-equivariant map cannot name a unit at all, so the row is closed twice); the modular period from the object's own algebra (traciality; object-specific); a chiral spectrum from the geometric holonomy (injectivity; universal on one-cusped manifolds), from any $\theta$-odd cusp field with transverse zeros (region swap; object-specific), or from the cyclic tower (the two conjugations; object-specific); supersymmetry from the object's carrier (three exact negatives; object-specific); a Majorana mass without a $27$ sector of the opposite chirality (the charge table; algebraic); a tree-level flavour hierarchy from the apex design (the hollow texture; algebraic); the doublet--triplet splitting by a Wilson line on the ninth closing (the weight identity; object-specific); and a finite label reduced further (an invariant selector cannot pick a point of its own orbit; universal). Nothing in the last column is merely undone; the one chirality question that is undone rather than closed --- the member's own reducible locus --- stands in the second column, where it belongs.
...
\paragraph{Two things a reader will look for and not find.} Electroweak symmetry breaking and proton decay are
absent from this paper, and not because they were tried and failed: both need a scale, and the construction's
dimensionful no-go forbids typing any dimensionful observable at all. There is no potential, no vacuum expectation
value with a magnitude, and therefore no breaking scale and no lifetime. A reader who wants either from this
programme should read the freedom ledger's first row as the reason.

\paragraph{What we are not claiming.} This is not a parameter-free Standard Model, and it is not a
theory of everything. Three structural copies of the Standard-Model content are exhibited and they are vector-like: the chirality bit is withheld by the object at every computed sector and supplied, as an index on non-semisimple modules, by members of its class and its tower; \S\ref{sec:chirality} states what that does and does not license, and the third column above names what it would take to move it. No unique four-dimensional field
theory follows, and we do not claim uniqueness of the construction. There is no four-dimensional dynamics, no rate and no field action (the one variational structure the record exhibits lives at the object's own monodromy level, as an autonomous same-state Lagrangian, and is not a field theory)
...
the chain is 57 links, 53 of them forced (39 of 43 when the census instrument was first recorded); axioms only at the two ends & settled & lock \\
the golden substitution matrix squared is the object's monodromy (M\^{}2 = LR), and that square root is UNIQUE up to sign in GL(2,Z) and is orientation-REVERSING; L\_a R\_b admits such a root iff a = b (Cayley-Hamilton, with enumeration and two controls) & settled & script + lock \\
...
the one-cusped index vanishes on every sector of the cyclic tower by two theorems (its non-vacuity as an instrument, open when this was banked, is now settled on the class and the tower) & settled & lock \\
...
the geometric finite-twist index is identically zero on every one-cusped hyperbolic manifold (Menal-Ferrer-Porti injectivity through a finite cover), and an exact non-semisimple module with I = +1 exists on m010 & settled & lock \\
chi(d+M) = 0 for every theta-odd cusp field with transverse zeros (the region-swap lemma); the product mode's +-4 is a non-transverse zero set & settled & lock \\
the index is non-zero in characteristic zero on reducible non-split modules of five members of the object's commensurability class and of its degree-4 cyclic cover t12839, and zero on the object's own golden reducible locus (235 modules); the class census (m202, s959, o10\_150726 carry all four requirements, none a cover of m004) & settled & lock \\
the three base rates: 1696 of 5000 admit the 2T surjection (of the 112 family members 59 admit it, 35 do not, 18 not enumerated), 181 of 203123 one-cusped census manifolds are amphichiral, 20 of 212641 attain the count of three (2 of the first 4000; 3996 of 4000 carry the object's 4); the three-line class to nine tetrahedra has nine members, none keeping the golden face & settled & lock \\
the exact half of the closing design, and only that half: the E7 apex cone's geometry (its count cited), the Z/3 = 2T/Q8 forcing b2 >= 2 for a symmetric triple, the Y3 breaking to SM x U(1)\_eta by a Q8 line times an order-4 character (root-system facts), and the algebraic no-seesaw obstruction (equal eta charges on both charged singlets; no eta-neutral Majorana source in 27\^{}3 or 78); the design as a whole is not a claim of this paper & settled & lock \\
the object's carrier admits no supercharge (no pi\_1-equivariant, no gauge-equivariant, the beat squares to the meridian): the closing's supersymmetry is the model's assumption & computed (enquiry open) & script + lock \\
fork F9 is fragile past depth three: the three-record carrier m412 keeps Q(sqrt-3) and is chiral, at the price of the 2T door & settled & lock \\
the apex design's tree-level flavour: only 27\_1 27\_2 27\_3 survives the apex U(1)\^{}2, the mass matrices are hollow (sigma\_1 = sigma\_2 + sigma\_3, against 273/50/17), the deck must be broken, and the U(1)\^{}2-violating part is at least a third of the top Yukawa & settled & lock \\
Y\_12's 768 one-triplet vacua (one vector-like generation, two Higgs doublets, no exotic triplet) and the theorem that none of Y\_9's 706 464 SM lines splits the doublet from the triplet (w\_D = -2 w\_Q) & settled & lock \\
the second expectation value lies on the pure-spinor cone of the 16 (stabiliser 34, toral rank 4; a generic spinor is fatal): two selections, booked as two & settled & lock \\
within the in-frame anomaly system (hypercharges assigned in a fixed colour frame on the 27's states) the grav\^{}2 Y condition is load-bearing (dropping it: 36 solutions plus solution planes) and there the cubic adds nothing to the three linear conditions & computed (enquiry open) & script + lock \\
the tower's budget: at the chiral 5-cusped degree-10 cover the flat sl\_3 deformation space has complex dimension 10 = k(n-1), twenty real parameters, all boundary data & settled & lock \\
under the Lorentz double the 27, the 78 and their tensor tower are integer-spin; the record's only half-integer object is the holonomy's C\^{}2 & settled & lock \\
...
of the 87 covers to degree 10, fourteen carry a hexagonal cusp (16 of 201 cusps) -- the only flat torus admitting an order-three automorphism -- and none realises an order-three cusp rotation, on canonical triangulations & settled & lock \\
no flat connection whose holonomy contains 2T in the principal sl2 leaves the Standard-Model algebra unbroken: every such centraliser is abelian of dimension at most 4, the SM algebra is non-abelian of dimension 12 & settled & lock \\
three interchangeable generations would need a cyclic cubic trace field and no hyperbolic knot has one: a cyclic cubic field is totally real while a hyperbolic invariant trace field has a complex place & settled & lock \\
the object's quadratic trace field permits multiplicities 1 and 2 and never 3, so the object cannot carry a family index & computed (enquiry open) & script + lock \\
the coupling on the constructed carrier is unique: pi\_1 alone leaves 6615 invariant trilinears, the gauge condition cuts to 4, full e6 to exactly 1, by direct nullspace with no shape assumed & settled & lock \\
the E8 family mechanism verified from E8's own definition (248 = 78 + 8 + (27,3) + (27bar,3bar); an order-3 element cycles six classes in two orbits of three) -- and nothing in it mentions the object: the object-specific step is unchecked & settled & lock \\
the crossing to measured values, sealed: alpha\_s 0.077 against 0.118 and sin\^{}2 theta\_W 0.2293 against 0.2312 under single-step non-supersymmetric running; the 16-sigma framing withdrawn, the corrected solver moves the distance negligibly & settled & lock \\
Vol(m004) = 9 sqrt3 zeta\_K(2)/pi\^{}2 to 32 decimal places, reproduced by a second independent route; the reality-parity law of the quantum invariant's expansion confirmed at five consecutive orders (detection, not derivation) & settled & lock \\
the fork is a rank obstruction: the branch carrying Lorentz signature and compact colour has maximal compact f4 of rank four, which no torus element of e6 reaches -- crossing needs an outer automorphism & settled & lock \\
the door, verified end to end: Q(sqrt-3) ramifies only at 3 with residue field F\_3, SL(2,O/(sqrt-3)) = SL(2,3) = 2T, pi\_1(m004) surjects onto 2T in exactly two classes up to automorphism, and the McKay graph of 2T at its faithful 2-dimensional representation IS the affine E6 diagram (7 nodes, 6 edges, marks 1,1,1,2,2,2,3); controls fire -- Q8 gives affine D4 and there is no surjection onto SL(2,5) = 2I, so the door does not open onto E8 & settled & lock \\
a conditional uniqueness theorem for the genesis matrix, machine-checked: six motivated axioms of a minimal record-transfer system force the persistent sector to LR (144 candidates to one), leaving exactly one inserted bit -- the LR vs RL order, which is what selects the golden polynomial; the axioms are NOT derived from anything weaker and the result is labelled conditional & settled & lock \\
the pair class 2 - kappa mod squares is mirror-EVEN, so it cannot be the orientation bit; the earlier identification is refuted and what survives is a relational Z/2 datum with the object supplying its own partner & settled & lock \\
the reducible index can fire only through the twist: rho has det 1, so Sym\^{}m(rho) is symplectically self-dual and I(V) = 0 whenever psi\^{}2 = 1 (verified on the witness and on all 542 firing modules of the record) & settled & lock \\
...
the manifold group is congruence of level (4) in PSL(2,O\_K), index 12, not of level (2), (sqrt-3) or (3); the continuous spectrum is one channel, phi(s) = Lambda\_K(s-1)/Lambda\_K(s) & settled & lock \\
...
the seam: exactly ten exceptional fillings; the fibre slope is the unique torus bundle with monodromy LR; of the grid's 78 closed hyperbolic fillings none keeps Q(sqrt-3) (54 in the first census, completed to 78/78); six are arithmetic by the closed criterion (m004(+-5,1) Meyerhoff, (+-6,1), (+-8,1)) and the other 72 are not & settled & lock \\
...
\noindent\footnotesize All 67 claims below (over 95 claim--record pairs) carry a test lock; 8 additionally
...
\bibitem{atiyahsinger} M.~F.~Atiyah and I.~M.~Singer, \emph{The index of elliptic operators III}, Ann.\ of Math.\
\textbf{87} (1968), 546--604.
\bibitem{riley} R.~Riley, \emph{A quadratic parabolic group}, Math.\ Proc.\ Cambridge Philos.\ Soc.\ \textbf{77}
(1975), 281--288.
\bibitem{chinburg} T.~Chinburg, \emph{A small arithmetic hyperbolic three-manifold}, Proc.\ Amer.\ Math.\ Soc.\
\textbf{100} (1987), 140--144.
\bibitem{bakerreid} M.~D.~Baker and A.~W.~Reid, \emph{Congruence link complements---a 3-dimensional Rademacher conjecture},
in: Cohomology of Arithmetic Groups, Springer Proc.\ Math.\ Stat.\ \textbf{245} (2018), 177--200.
...
\bibitem{chw} P.~J.~Callahan, M.~V.~Hildebrand and J.~R.~Weeks, \emph{A census of cusped hyperbolic 3-manifolds}, Math.\ Comp.\ 68 (1999), 321--332.
\bibitem{lackenby} M.~Lackenby, \emph{The canonical decomposition of once-punctured torus bundles}, Comment.\ Math.\ Helv.\ 78 (2003), 363--384.
\bibitem{mfp} P.~Menal-Ferrer and J.~Porti, \emph{Twisted cohomology for hyperbolic three manifolds}, Osaka J.\ Math.\ 49 (2012), 741--769, arXiv:1001.2242.
\bibitem{jorgensen} T.~J\o rgensen, \emph{On discrete groups of M\"obius transformations}, Amer.\ J.\ Math.\ 98 (1976), 739--749.
\bibitem{callahan} J.~Callahan, \emph{J\o rgensen number and arithmeticity}, Conform.\ Geom.\ Dyn.\ 13 (2009), 160--186.
\bibitem{minsky} Y.~N.~Minsky, \emph{The classification of punctured-torus groups}, Ann.\ of Math.\ (2) 149 (1999), 559--626.
\bibitem{kac} V.~G.~Kac, \emph{Infinite dimensional Lie algebras}, 3rd ed., Cambridge University Press, 1990, Ch.~8.
\bibitem{witten91} E.~Witten, \emph{Quantization of Chern--Simons gauge theory with complex gauge group}, Comm.\ Math.\ Phys.\ 137 (1991), 29--66.
