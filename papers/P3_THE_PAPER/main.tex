...
by one ADE classification, one in three one-cusped census manifolds admits the binary tetrahedral
...
forced by the anomaly conditions alone, in a computation containing no token of the object; the object
forces the termination of the breaking chain and, given two listed inputs, the global form
...
predicted, and runs to a $35\%$ miss in $\alpha_s$; three Standard-Model copies are exhibited on the
object's closings, in mirror pairs, and are vector-like. The chirality bit is what the object withholds:
the cohomological index that stands in for net chirality is zero on every finite twist of the geometric
holonomy by a theorem, non-zero on five members of the object's commensurability class and on its
degree-four cyclic cover, and zero on the object's own reducible locus on every module computed; the one
mechanism by which a closing could supply chiral matter is excluded as it stands. The residue is a freedom
...
The work was carried out by a single author with AI-assisted verification passes, and every computation named is re-runnable from the public repository; nothing here rests on a computation the reader cannot repeat. We therefore invert the usual order. \S\ref{sec:generic} states what is \emph{generic} about the
...
There are \textbf{fifty-six links}. Their types are: thirty-five theorems, six identities, eight no-go results, two censuses, one corollary --- and \textbf{four axioms}. So fifty-two of fifty-six links are forced, in the precise sense that they are not declared choices.
...
\end{tikzpicture}
\caption{\small The chain's fifty-six links, with each of the four axioms marked. The point is
...
pre-registered\footnote{Pre-registration in this paper means one thing: the design text of a computation, with its outcomes and controls, is committed to the public repository and its hash recorded before the computation runs; the record's seal ledger lists every such digest, and a reader can check that the digest predates the result.} to test whether the quadratic field carrying the exceptional algebra could be reached
...
holds, all of it structure. Of $128$ involution classes (Kac's count~\cite{kac}) exactly two make the matter wall real, and both are the quaternionic
...
\noindent\emph{Control.} Each of the three linear conditions is load-bearing in this frame: dropping the gravitational one degrades the forcing from a single line to thirty-six solutions and a family of solution planes, while the cubic condition is redundant once the three linear ones hold.
...
within the in-frame anomaly system the grav\^{}2 Y condition is load-bearing (dropping it: 36 solutions plus solution planes) and the cubic is redundant & computed (enquiry open) & script + lock \\
...
\noindent\footnotesize All 54 claims below (over 74 claim--record pairs) carry a test lock; 6 additionally
