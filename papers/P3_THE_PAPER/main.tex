...
\textbf{The invariant is four}: the derivation rests on \emph{four undischarged inputs}, three before the object and
one after, and that count does not move with how finely the chain is cut. The table as we cut it has \textbf{fifty-six
...
in question are outside every such class, so $I$ is computed rather than cited. One lemma organises everything that follows. By definition $I(V) = n(V) - n(V^*)$, so
...
that frame --- the higher-rank components of the character variety, the singular closing --- is untouched. Two further facts fix the index's standing, and they pull in opposite directions. First, a theorem: for $V = \mathrm{Sym}^m(h) \otimes F$ with $h$ the geometric holonomy of \emph{any} one-cusped hyperbolic 3-manifold and $F$ of finite image, the restriction $H^1(M;V) \to H^1(\partial M;V)$ is injective~\cite{mfp}. The cited theorem is
...
orbit, so a finite menu is terminal, not a deficiency. (Three menus share this row; counted as choices rather than rows, the
non-continuous inputs number nine.) \\[2pt]
...
dual system.} on an unprotected twist sector of a chiral cover of the object.} Executable now on any one-cusped chiral cover with the instrument of \S\ref{sec:chirality}; on the multi-cusped covers only through the several-cusp form, which is reported rather than reproducible. This too is an upgrade trigger rather than a falsifier: it would move the chirality
...
\noindent\footnotesize All 56 claims below (over 79 claim--record pairs) carry a test lock; 6 additionally
