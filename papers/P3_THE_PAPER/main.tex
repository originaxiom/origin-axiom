...
the paper claims no four-dimensional theory precisely because they are not paid. The table as we cut it has \textbf{fifty-six
links} --- thirty-five theorems, six identities, eight no-go results, two censuses, one corollary, and those four axioms ---
so fifty-two of fifty-six are forced, in the precise sense that they are not declared choices. \emph{That ratio is not
the invariant and we do not ask it to be}: it counts rows under this typing and moves with the table's granularity, and
``forced'' does not mean ``theorem'' --- the thirty-five theorems are the theorems, and a census or a no-go result is
...
\begin{tikzpicture}[x=2.4mm,y=1mm]
  ...
  \node[font=\scriptsize] at (56.4,-3.6) {$C_{56}$};
...
\end{tikzpicture}
\caption{\small The chain's fifty-six links in the table's order, with each of the four axioms marked. The order is the
...
35 theorem, 8 no-go, 6 identity, 4 axiom, 2 census, 1 corollary, totalling 56 links, of which \textbf{52} are not axioms. The four axioms are
...
fifty-two forced links and then fails, provably and by its own sealed criterion, to reach
...
\begin{nonclaim} that every one of the chain's fifty-two forced links is stated in this paper in checkable form: \S\ref{sec:chain}
...
the chain is 56 links, 52 of them forced (39 of 43 when the census instrument was first recorded); axioms only at the two ends & settled & lock \\
...
\noindent\footnotesize All 66 claims below (over 93 claim--record pairs) carry a test lock; 7 additionally
