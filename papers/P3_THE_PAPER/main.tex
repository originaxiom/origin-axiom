...
26 theorem, 6 identity, 5 no-go, 4 axiom, 1 census, 1 corollary, totalling 43 links, of which \textbf{39} are not axioms. The four axioms are
...
% 2026-09-01 scope note added (fresh-seat application of the owner's 2026-09-01 election — verbatim "T5, T1, T4 green. act" against the batch-1 election list, where T5 = "adopt T5's PROPOSAL.md" (A6 → closing #0), T4 = the prior-art corrections, T1 = commit the 27 connecting-block values; T2 was NOT elected;
% sources: reports/fresh_physics_seat_2026-09-01/campaign/)
\begin{scopenote}[A relabeling this axiom admits]
This axiom admits a re-typing that changes its book-keeping and none of its mathematics: the
un-squared, determinant $-1$ manifold exists before any orientation is chosen, and it provably
cannot orient itself, so ``orientation'' can be carried not as an axiom of the object but as the
\emph{observer's first closing} --- the same act-type as the sign choices \S\ref{sec:observer}
prices, and prior to all of them. An audit of the record found no result that consumes an
orientation before the object exists, so nothing downstream distinguishes the two labelings; under
the relabeling the axiom count at the entrance drops by one and the orientation cost moves,
unchanged in size, onto the observer's ledger. We state the option and keep the axiom labeling in
this paper's counts.
\end{scopenote}
...
\paragraph{The global form is reproduced.} The Standard Model's own data leaves the global form ambiguous
among $\{1,\Z_2,\Z_3,\Z_6\}$. Here it is fixed:
$\bigl[\mathrm{SU}(3)\times\mathrm{SU}(2)\times\mathrm{U}(1)\bigr]/\Z_6$, with the kernel computed
exactly and uniformly across six independent realisations of the relevant Weyl data.
% 2026-09-01 correction: novelty deflated per the path-independence finding (fresh-seat application
% of the owner's 2026-09-01 election — verbatim "T5, T1, T4 green. act" against the batch-1 election list, where T5 = "adopt T5's PROPOSAL.md" (A6 → closing #0), T4 = the prior-art corrections, T1 = commit the 27 connecting-block values; T2 was NOT elected; sources: reports/fresh_physics_seat_2026-09-01/campaign/)
\textbf{This is the clearest \emph{reproduced} structural statement in the paper}, and we no longer
present it as a novelty: the kernel is a path-independent property of the (algebra, matter-content)
pair, and the answer has been known since 1980. \textbf{The derivation is standard}: the subalgebra
exponentiates inside $\mathrm{SU}(5)$ to
$S(\mathrm{U}(3)\times\mathrm{U}(2))$, whose kernel is $\Z_6$ --- which is why $\mathrm{SU}(5)$
quantises hypercharge~\cite{slansky}, and is decades old. Nothing here needs to \emph{select} the
embedding --- any route delivering this algebra with this generation content computes the same
kernel, so the embedding was never doing the work; what this setting supplies is upstream, the
content the kernel is computed from. The modern observation that the four quotients are experimentally near-indistinguish\-able
...
failures to close on itself --- an orientation cannot be chosen canonically --- a sign in a chord
sector cannot be fixed, and two further probes of the same kind.
% 2026-09-01 (fresh physics seat, ring R2 cell R20, seat spot-check): the sentence that stood here
% --- "all 83 members of the family ... are amphichiral, with no exceptions" --- is WITHDRAWN. The
% 14/14 -> 83/83 -> 112/112 census was one orientation-blind instrument (reverse_orientation +
% is_isometric_to, which SnapPy evaluates without orientation and which cannot return False).
% Orientation-aware (symmetry_group().is_amphicheiral()): 38 of the 112 Q(sqrt-3)-shape-field
% members are amphichiral, 74 chiral (e.g. m202, s118 at CS = 1/12; m208 chiral at CS = 0).
% The object itself IS amphichiral, and the a-priori theorem below is what the paper rests on.
% Sources: frontier/B1186_family_is_112/ADDENDUM_2026-09-01_amphichirality_38_of_112.md,
% reports/fresh_physics_seat_2026-09-01/recompute/R20_family_separator/seat_spotcheck_d5.txt.
% 2026-09-01: census upgraded to the a-priori theorem (fresh-seat application of the owner's
% T4/T5 election; sources: reports/fresh_physics_seat_2026-09-01/campaign/T3_amphichirality/)
The amphichirality census is in fact a theorem wearing a statistic's clothes: every orientation
double cover of a non-orientable finite-volume hyperbolic $3$-manifold is amphichiral \emph{a
priori}, with the mirror self-isometry realised by the deck involution itself --- an orientation
invariant under the deck group would descend to an orientation of the non-orientable base, and
Mostow rigidity~\cite{mostow} makes the conclusion metric-independent --- so every mirror-odd
invariant of such a cover is $2$-torsion by construction; the object's amphichirality is an
instance of this theorem, not a statistic of its family (the family is mixed: $38$ of $112$
members are amphichiral, and the earlier ``$83$ of $83$'' count was an artefact of an
orientation-blind test). These are \emph{one} $\Z/2$-torsor class
...
$\kappa(A,M) = \mathrm{tr}[A,M] - 2$.
% 2026-09-01 convention note (fresh physics seat; defect found by B1233): the frontier record
% (B285, B126, B330, B131, frontier/README) uses kappa = tr[A,B] RAW; this paper shifts by 2.
% The founding value is omega under the raw convention and 2 + omega = sqrt(3) e^{i pi/6} under
% this one. Both are internally consistent; the harmonisation (which to keep) is the owner's.
% Until then every kappa value quoted here must be read with the "-2" in view. The same polynomial governs the object's own founding
...
% 2026-09-01 correction: the selects-the-embedding clause retracted (path-independent kernel, known
% since 1980) and the termination clause narrowed to the conjunction against the survival-hypothesis
% prior art (fresh-seat application of the owner's 2026-09-01 election — verbatim "T5, T1, T4 green. act" against the batch-1 election list, where T5 = "adopt T5's PROPOSAL.md" (A6 → closing #0), T4 = the prior-art corrections, T1 = commit the 27 connecting-block values; T2 was NOT elected;
% sources: reports/fresh_physics_seat_2026-09-01/campaign/)
\noindent What is \emph{not} on this list, and to our knowledge is not standard: the termination
result \emph{jointly with} its rule-independence --- the selection criterion itself is the survival
hypothesis of 1979--80 grand unification~\cite{georgi1979,barbierinanopoulos,fonseca}, so what may
be non-standard is only the conjunction, terminality below the Standard Model algebra together with
the endpoint's independence of the selection rule, within the fences \S\ref{sec:forced} states; the
exhaustive disjointness of the object's
periods from the target set, at the counts given in \S\ref{sec:withheld}; and the pricing of the
observer in \S\ref{sec:observer}. \textbf{Five items have been moved off this list during revision},
each after a precedent was found --- the fifth, the claim that anything in this setting
\emph{selects} the $\mathrm{SU}(5)$ embedding whose kernel is $\Z_6$, retracted when the kernel was
found to be a path-independent property of the (algebra, content) pair, known since 1980 --- and we
offer that number as a better guide to the paper's
...
arXiv:1705.01853.
% 2026-09-01: survival-hypothesis prior art added (fresh-seat application of the owner's T4/T5
% election; sources: reports/fresh_physics_seat_2026-09-01/campaign/T4_prior_art/)
\bibitem{georgi1979} H.~Georgi, \emph{Towards a grand unified theory of flavor}, Nucl.\ Phys.\
\textbf{B156} (1979) 126.
\bibitem{barbierinanopoulos} R.~Barbieri and D.~V.~Nanopoulos, \emph{An exceptional model for
grand unification}, Phys.\ Lett.\ \textbf{91B} (1980) 369.
\bibitem{fonseca} R.~M.~Fonseca, \emph{On the chirality of the SM and the fermion content of GUTs},
Nucl.\ Phys.\ \textbf{B897} (2015) 757, arXiv:1504.03695.
