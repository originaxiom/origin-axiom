...
\item \textbf{Between the object and the algebra: none.} Across the twelve consecutive links running
...
mathematics, not ours. Morse--Hedlund fixes what an aperiodic sequence must cost in factor
complexity; extremality in the Hurwitz sense then selects, by a single criterion, the golden case.
...
derivation earns its keep, and it is the stretch with no axioms in it: the mapping torus is realised
as the hyperbolic manifold; its intrinsic arithmetic forces its faces; the interface structure is
...
the generation shape, the trace identities --- and the chain ends where this paper ends, on a no-go:
