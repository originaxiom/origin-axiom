...
The post-object continuous inputs are separately tabled in \S\ref{sec:ledger}, and two of those rows
are not closed.
\end{scopenote}
...
\textbf{External; gated, with its first hypothesis unmet.} The weight completing the object's
tracial core. One sub-route is closed by a textbook fact (the relevant unit rank is $0$, so that
regulator is identically $1$); an attempted exhaustion proof failed and is withdrawn. The criterion
is this: anything expressible from the object must be invariant under the automorphisms the object
itself fixes --- a group \emph{read off} the object, never chosen --- after which existence,
uniqueness and cost are settled in three regimes (finite and transitive: a unique counting measure
at a cost of exactly $\log_2|T|$ bits; the irrational rotation: Lebesgue, by equidistribution;
non-compact: none, the prior-versus-point distinction being ill-typed exactly where the invariant
measure fails to normalise). $\lambda$ fails at the \emph{first} step --- no such pair has been read
off --- so the row is not derived. We record it as the ledger's weakest entry, but the weakness is
now located rather than merely conceded. \\[2pt]
...
\begin{scopenote}
One row --- $\lambda$ --- is neither derived nor closed. It is, however, \emph{gated}: a measure is
expressible from the object only if it is invariant under the automorphisms the object itself fixes,
and that group is \emph{read off} the object rather than chosen; existence and cost are then settled
in three regimes, and the prior-versus-point distinction is ill-typed exactly where the invariant
measure fails to normalise. $\lambda$ does not yet meet the \emph{first} hypothesis of that
criterion, no such pair having been read off. We prefer this to calling the row ungated: it is a
sharper and more falsifiable statement than ``no criterion exists''. $\PP(B_0)$ is closed
permanently, subject to the one datum named above. One row, $\ell$, is external by design and will not close. The
