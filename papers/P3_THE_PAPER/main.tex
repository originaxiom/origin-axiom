...
\textbf{\S} & \textbf{Claim} & \textbf{Backing} & \textbf{Re-runs by} \\ \midrule
2 & E6 recurrence is one ADE classification -- a graph identity, not an ob & settled & lock \\
2 & the census base rate against which any recurrence must be judged & settled & lock \\
2.1 & seven genesis forks: five robust or geometry-necessary, two fragile an & settled & lock \\
3 & the chain is 43 links, 39 of them forced; axioms only at the two ends & settled & lock \\
3 & the golden substitution matrix squared is the object's monodromy (M\^{}2  & settled & lock \\
3 & the combinatorial carrier reaches only Q(sqrt5); Q(sqrt-3) is bought a & settled & lock \\
3 & the exceptional algebra is handed over at the hyperbolic end by McKay, & settled & lock \\
4 & 252 SM-visible contents, 222 killed by the colour cubic alone, exactly & computed (enquiry open) & script + lock \\
4 & the forcing package is arena-generic: no object token appears in it & computed (enquiry open) & script + lock \\
6 & the hypercharge line is cut by three linear conditions, then the cubic & settled & script + lock \\
7 & the landing stratum is selected, not chosen: A2 is the unique projecti & settled & lock \\
7 & the eighteen hypercharge directions are two orbits: 4.17 bits re-price & settled & lock \\
7 & that one bit IS P, the two orbits fused by the arithmetic mirror & settled & lock \\
7 & the spin lift is assigned by the object's own beat, not free & settled & lock \\
7 & the P\^{}3 is closed permanently, one condition short of a point set & settled & lock \\
7 & ...and that closure is hardened by an independent cross-seat pass & computed (enquiry open) & script + lock \\
7 & lambda is external by theorem: the object's own clock is tracial, triv & settled & lock \\
7 & an external weight completes the tracial core to type III\_lambda & settled & lock \\
7 & compactness needs an antilinear involution; one conjugation buys Loren & settled & lock \\
5 & no object period is an SM ratio (the value negative, exhaustive) & settled & lock \\
9 & the mirror-isospectral split: the two hands share their spectrum exact & settled & lock \\
\bottomrule\end{tabular}\end{center}

\noindent\footnotesize All 22 claim--record pairs carry a test lock; 4 additionally
