...
There are \textbf{fifty-six links}. Their types are: thirty-five theorems, six identities, eight no-go results, two censuses, one corollary --- and \textbf{four axioms}. So fifty-two of fifty-six links are forced, in the precise sense that they are not declared choices. The tally is a count of the table's rows under this typing and moves with the table's granularity; it is not an invariant of the derivation, and ``forced'' does not mean ``theorem'': the thirty-five theorems are the theorems, and a census or a no-go result is forced only in the stated sense that no declared choice enters it.
...
transport to a real 3-manifold with boundary is nowhere exhibited --- and the obstruction is sharper than an absence: under the Lorentz double the $27$, the $78$ and their whole tensor tower are integer-spin, and the record's only half-integer object is the holonomy's own $\mathbb{C}^2$. Below, ``generation'' means a complete copy of
...
\noindent\footnotesize All 54 claims below (over 76 claim--record pairs) carry a test lock; 6 additionally
