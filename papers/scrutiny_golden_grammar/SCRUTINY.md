...
## ROUND 4 — the 37-page revision (v6, 2026-08-15 23:06) with source

Target changed: 37 pages, and the LaTeX source plus a `verify/` tree now ship. This is a
re-scrutiny of the new document, not a re-application of the old findings.

### The suite ships and it runs

```
$ cd verify && python3 verify_all.py
[PASS] check_charge_bracket.py   [PASS] check_charge_field.py   [PASS] check_global_form.py
[PASS] check_homology.py         [PASS] check_jones_wall.py     [PASS] check_measurement_plane.py
[PASS] check_shadow_modulus.py
7 / 7 verifications reproduced
```

From the tarball alone, no repository. **T5 and M11 are closed** — and closed well:
`check_charge_bracket.py` builds `e6` from the Cartan matrix, verifies Jacobi, and computes
`[x14,x22] = 0` exactly, which is the input §9.2 previously called the paper's sharpest defect.
Errata (18) records that block (a) *"did not contain block (a) at all until this was found by a
reader who opened the built artifact instead of trusting the sentence."*

### Closed by the revision

| finding | how |
|---|---|
| **F2** — Thm 10.10 unstatable | §10 (real forms, atoms, `D₂`) **removed entirely**; errata (19) withdraws it as "unrefereeable" rather than false |
| **R1** — abstract carried withdrawn CS claims | the sentence is gone from the abstract; Scope 11.4 now reads *"What is a family phenomenon is the two-torsion `2CS ≡ 0`"*; errata (8) and (9) |
| **R2** — "the second plane is free" | gone from both the body and the audit table |
| **M6** — "the seven above" | ledger restructured |
| **M8** — "charge lattice" rank collision | gone with §10 |
| **M10** — `A_m` undefined | Scope 5.21 reworded, and it now handles the `|tr| = 2` subtlety explicitly |
| **M11 / T4(i)** — the bracket had no script | `check_charge_bracket.py`, block (a) |
| **m4, m5, m6, m12** | gone with §10 |
| **m7** — Cohn absent from the bibliography | now [3]; [2] is cited for the trace identity |
| **T5** — block (a) absent | ships, passes |

Appendix C is the right instrument and I want to say so: nineteen numbered errata, each naming
what was claimed and what replaced it, ending *"every entry was found by a reader other than the
author… the rate at which they were found had not fallen by the last one."* That paragraph is
worth more to a referee than any forcedness table.

### Still standing, unchanged

- **T1** — the mod-3 reduction is still not in the paper. Riley is now cited [9], but
  `π₁(4₁) ↠ SL(2,𝔽₃) ≅ 2T` is still asserted without proof at Scope 7.2, and §7.3 still opens
  *"the step … that consumes the arithmetic"* over Theorem 7.7, which contains no arithmetic.
  Proposition 8.1 does not supply it either — it is a simplex/`SO(3)`/Weyl-group argument with no
  manifold in it. **The arithmetic-consuming step is still not exhibited anywhere.**
- **T2** — Goodman–Heard–Hodgson 2008 is still uncited (zero occurrences), and Scope 11.2 still
  withdraws the biconditional the bank proves.
- **M1** — §1.3 is verbatim unchanged: the entrance is still counted among "the 20 steps below the
  entrance" while being "counted separately", and *"attacking the cascade attacks Slansky"* is
  still contradicted by §10.5's own audit table.
- **M3 / M4** — errata (6) reaffirms the classification route (*"it now follows from the
  classification, applied power-closed"*), so the commensurability→powers inference stands as the
  load-bearing step. Maclachlan–Reid is now [12] but 8.3.2 is not invoked for the tail.
- **M5** — Theorem 7.7 still has no row in Appendix B. It is now, with T1, the entrance's only gap.
- **M9** — the abstract still drops the non-negativity hypothesis. `[[−1,1],[1,0]]` is hyperbolic,
  `det = −1`, dominant eigenvalue `−φ`: no purely periodic expansion. Theorem 4.2 has the
  hypothesis; the abstract's "exactly" does not.
- **R7** — the abstract now says *"intrinsically characterized rather than ad hoc… weaker than
  saying it is not a choice"*, but §1.1 still says *"We prove that this family is **canonical**"*
  and the title still says "canonical family".

### NEW — on the new material

Both new propositions **verify exactly** (added to `verify_scrutiny.py`, now 116 checks). The
findings are about what is claimed for them.

**N1 [MAJOR] — the new principal result hangs on a locus with no real point, in a paper that has
just deleted its real-form analysis.**

Proposition 10.18 is a statement about **compact groups**: `SU(3)×SU(2)×U(1) → S(U(3)×U(2)) ⊂
SU(5)`, kernel `Z/6` generated by `(ω²I₃, −I₂, e^{iπ/3})`. I verified it — the kernel is exactly
`Z/6`, cyclic, with that generator.

But the stratum it is attached to is not real. Corollary 10.15: *"all four 26-points are
non-real."* And the abstract now says *"we do not treat real forms."* So the group in
Proposition 10.18 and the `su(5)` the cascade locates are not connected by anything the paper
contains — exponentiating to `SU(5)` presupposes the compact real form, which the paper has
explicitly stopped supplying.

Scope 10.20 concedes both limitations. **The abstract carries neither**, and calls it *"the
paper's principal result."* That is the same failure mode as R1 — abstract asserting what the body
qualifies — in the one section R1's repair did not touch.

**N2 [MAJOR] — Proposition 10.18 is standard, and is cited to nothing.**

That the Standard Model gauge group embedded in `SU(5)` is `[SU(3)×SU(2)×U(1)]/Z₆` is textbook
grand-unification material, and the `Z₆` quotient specifically has a literature (the
Georgi–Glashow lineage; Hucks 1991 on the global form; Tong on line operators in the Standard
Model; Baez–Huerta on the algebra of grand unified theories). **The paper cites none of it** — the
bibliography has zero GUT entries, and Slansky [20] is not invoked here. The mathematics is right;
the novelty framing is not. §1.4 exists precisely to separate what is new from what is standard,
and this is the paper's largest advertised output.

This is also the exact error class the programme's own novelty process catches: B134's audit
graded all three candidate-novel results KNOWN or PARTIALLY-KNOWN against located prior art. That
process was not run on §10.2.

**N3 [MODERATE] — the "principal result" is not composed.**

Scope 10.20: *"reaching `su(3)⊕su(2)⊕u(1)` inside `su(5)` requires a further step that this paper
does not take… **We do not claim the cascade performs that embedding.**"* So the result is two
statements set side by side — the cascade reaches `A₄`; separately, a standard embedding has
kernel `Z₆` — and the abstract's *"the route through `su(5)` determines the global form"* asserts
the composition the scope note declines. Pick one.

**N4 [MODERATE] — the ledger has now been recounted three times, and each recount reduced it.**

7 rows → 9 rows → §11.2's *"The rows do not have equal status, and grouping them as 'nine choices'
was the error of an earlier draft."* (C7) has moved from a priced declared choice (Scope 9.17, v5)
to *"not a row: it is the method of §2."* (C6) and (C8) are *"dissolved to an acceptance."*

Each move may be individually right — a method is not an input, and §8's dissolutions are real
mathematics. But the pattern across three drafts is that the count falls by reclassification, and
a hostile referee will read the sequence rather than the current row. The fix is presentational
and cheap: keep the nine rows, type them, and say plainly that the count did not change — the
classification did.

**N5 [MINOR] — Proposition 8.1's simplex framing does less than it looks.**

The proposition is correct (verified: `A_{d+1} < SO(3)` exactly for `d ∈ {2,3,4}`; among the
exceptionals only `E₆` has `−1 ∉ W`; the `D₄`/`D₆` caveat about triality is right and worth
keeping). But the object never ranges over `d`. What it offers is exactly two candidates — `2T` at
the hyperbolic end and `2I` at the spherical end (Theorem 7.1) — so the filter's real work is
"complex fundamental picks `E₆` over `E₈`", which is one line over that dichotomy.

And *"`d = 3` is what `M₁` is glued from"* proves too much as written: **every** ideal-triangulated
3-manifold is glued from 3-simplices. The load is carried by **regular** — only `m003` and `m004`
decompose into two *regular* ideal tetrahedra — and Remark 8.2 says "regular" but the summary
sentence and the abstract drop it. One word, and the step stops looking generic.

**N6 [MINOR] — block (b) was not audited the way block (a) was.**

Four of its nine rows name files absent from this repository:
`test_levi_classification_forces_the_rungs.py`, `test_second_measurement_is_exhaustive.py`,
`test_arithmetic_tail_and_quaternionic_two.py`, `forcedness_census.py`. Errata (18) fixed block
(a) after a reader opened the artifact; the same check has not been run on block (b).

---

## Priority for the author

*(rewritten for v6, the 37-page revision. Items closed by that revision are struck from the list.)*

**Blocking for submission — five items:**

1. **N1** — the abstract must carry Scope 10.20's two limitations, or stop calling §10.2 the
   principal result. A compact-group statement attached to a stratum with no real point, in a
   paper that has deleted its real-form section, cannot go in an abstract unqualified.
2. **N2** — cite the GUT literature for the `Z₆` global form, and re-grade the novelty of §10.2
   against it. Run the programme's own novelty-audit process on that subsection.
3. **T1 / M5** — the entrance still has no arithmetic-consuming step and Theorem 7.7 still has no
   certificate. B210's mod-3 reduction supplies the first (`π₁(4₁) ↠ SL(2,𝔽₃) ≅ 2T`, with the
   Riley parameter and the two parabolic generators) and also proves Scope 7.2's assertion.
4. **M1** — §1.3's twenty-step census is verbatim unchanged and still self-contradictory; it is on
   page 4 and it is about counting, which is the thesis.
5. **T2** — cite Goodman–Heard–Hodgson 2008 and restore the amphichirality biconditional. This is
   the one place the paper is *weaker* than what it can prove.

**Then, before an expert reads it:**
