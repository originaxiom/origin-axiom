...
The extremality itself is classical (Markov; and the corresponding twisted spectrum
statement is a corollary of Andersen--Duke). What is ours is the \emph{application}
to grammar selection, not the spectral theorem.
\end{scope}
...
That the golden ratio, icosahedral symmetry, $2I$ and $E_8$ form a connected cluster
is well established~\cite{Baez,Dechant}; \S\ref{sec:suspicious} already concedes that
arrival there is not evidence. The claim defended here is a \emph{selection}
statement over a family, each member evaluated at its own modulus.
\end{scope}
