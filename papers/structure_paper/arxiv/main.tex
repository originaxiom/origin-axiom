...
distinguished four-element charge torus, and a cascade of centralizers of that
torus terminates, exactly, at $\su(3)\oplus\su(2)\oplus\uu(1)^3$. \textbf{No measured
quantity enters any computation at any point.}

\begin{remark}[a distinction the reader must not let us blur]
The terminal centralizer above has dimension $8+3+3=14$ and rank $6$, as every rung
of the cascade must, being the centralizer of a subset of a rank-$6$ torus. It is
\emph{not} the same object as the $12$-dimensional
$[\SU(3)\times\SU(2)\times\mathrm{U}(1)]/\Z_6$, which arises at a different step---as
a \emph{group inside} $\SU(5)$---and which quotienting by a finite $\Z_6$ could never
produce from a $14$-dimensional algebra. Earlier drafts of this paper stated the two
in one breath and were wrong to; they are separate results about separate objects,
and the second is not reported here.
\end{remark}
...
fiber field. Then $L/\Q$ is biquadratic with $\mathrm{Gal}(L/\Q)\cong V_4$, and its
three quadratic subfields are exactly
$\Q(\sqrt{-3})$, $\Q(\sqrt5)$, $\Q(\sqrt{-15})$.
\end{theorem}

\begin{remark}
The content is that the construction produces \emph{both} generators of $L$ and
therefore the third field as a composite, not that a biquadratic field has three
quadratic subfields, which is automatic. We flag also that $\Q(\sqrt5)$ enters
already at the golden slope, \emph{before} the manifold exists; only
$\Q(\sqrt{-3})$ is bought at geometrization (C4).
\end{remark}
...
unique one whose shadow modulus $m^2+4$ reduces to an \textbf{exceptional} ($E$-type)
McKay group. Completeness is a proof, not a search: writing $|\SL(2,\Z/N)|=N^3\prod_{p\mid N}(1-p^{-2})$ and
...
$|\SL(2,\Z/N)|$ is the order of one of $2T,2O,2I$ only for $N\in\{3,4,5\}$, and
$m^2+4\in\{3,4,5\}$ has the unique metallic solution $m=1$.
\end{theorem}

\begin{remark}[why the qualifier is not decoration]
The unqualified statement---``$|\SL(2,\Z/N)|$ is a McKay-group order''---is
\emph{vacuous}. The McKay correspondence covers every finite subgroup of $\SU(2)$,
and the cyclic subgroups (type $A_n$, order $n+1$) realise \emph{every} positive
integer as an order, the binary dihedrals being unbounded as well. So that predicate
holds for every $N$ and cannot discriminate. Only the restriction to the three
exceptional groups gives a finite target set, and it is that restriction the bound
above exploits. Earlier statements of this result, including in the source material,
omitted the qualifier and were vacuous as written.
\end{remark}
...
directly~\cite{ChristoffelMatrices}---there via the Burrows--Wheeler matrix, of size
the word length, reduced modulo that length, with the Zolotareff symbol as the
invariant. The construction here is a $2\times2$ monodromy reduced modulo the
\emph{discriminant} $m^2+4$, with a group order as the invariant; the two agree only
in shape. The claim defended is accordingly neither ``the golden is connected to
$E_8$'' nor a statement about Christoffel matrices, but a \emph{selection} over a
family in which each member is evaluated \textbf{at its own modulus}. We have not
found that step in either literature.
\end{scope}
...
dimension $11$ and centre of dimension $3$, whence the derived algebra is of
algebraic type $A_2\oplus A_1$---that is, $z(x_1,y^*)\otimes\overline{K}\cong
\mathfrak{sl}_3\oplus\mathfrak{sl}_2\oplus\mathfrak{t}^3$ exactly.
\emph{Which real forms occur is a separate question, settled only in
\S\ref{sec:realform}}; writing $\su(3)\oplus\su(2)$ here would presuppose the
compact forms and beg it. The within-$C$ centralizer
ladder is $\{78,46,30,18,14,12\}$: the dimension $26$ is not attained. Since every
rung is the centralizer of a subset of a rank-$6$ torus and therefore has rank $6$,
the algebra of dimension $26$ that could have occurred is
$\su(5)\oplus\uu(1)^2$ (rank $4+2$), not $\su(5)\oplus\uu(1)$, which has rank $5$ and
dimension $25$ and cannot arise as a rung at all. \textbf{The second measurement
skips it.}
\end{theorem}
...
\bibitem{ChristoffelMatrices} C. Reutenauer and J. Shallit, \emph{Christoffel
matrices and Sturmian determinants}, arXiv:2409.09824.
