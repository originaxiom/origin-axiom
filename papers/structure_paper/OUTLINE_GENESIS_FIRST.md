...
| **D1a** | the fiber-field-side group is `2I × ℤ/3` at congruence level 15, order 360; non-CM subgroup Q₈; five absolute tones | chain Part II | ## **GAP — arc not identified** |
| **D1b** | ## **the congruence-level-15 form is IRREDUCIBLE** (the Kleinian-trace-field and fiber-field sides INTERFERE; 59/60 primes falsify L-factorization) | `LAW_MAP` — the congruence-level-15 handshake row | ## **GAP — arc not identified** |
| **D1c** | E₆/E₈ at the two **curvature ends**: hyperbolic `ℚ(√−3)/2T/E₆`, spherical `ℚ(√5)/2I/E₈`; Chern–Simons pair 0 vs π²/5 | **B981 / B248** (via `THE_LAMBDA_POSITION`) | ## **GAP — locks not identified; cc3 read at one level of indirection** |
...
3. ## ⚠ **D1 — the doorway. cc3's FIRST VERSION OF THIS ROW WAS FALSE AND WAS BANKED** (`27d9ceb9` §2.1, withdrawn in `1a0b5a90`). cc3 wrote *"15 = 3·5 ⟹ CRT ⟹ 2T × 2I ⟹ McKay ⟹ E₆ × E₈"* — but the fiber-field-side group is **`2I × ℤ/3` (order 360)**, not `2T × 2I` (2880); and `LAW_MAP` banks the congruence-level-15 form as **IRREDUCIBLE**, the opposite of a factorization. **The real mechanism is the curvature ends (D1c).** ## **The paper's spine still has no citation row, and the first attempt to supply one produced a false theorem — this row must be sourced by the bench before §5 is drafted.**
