#!/usr/bin/env python3
"""
Appendix B -- rho is BUILT, so Prop (2880) and the coupling law are no longer certificates.

## WHY THIS SCRIPT EXISTS

Scope (2880) recorded that rho's structure constants "are specified in the source
computation and are not reconstructed in this paper", making Proposition (2880) a
certificate whose ambient representation was cited rather than rebuilt.  Its companion
check_coupling_law.py could therefore only verify trace SETS, not rho.  This script builds
rho, and both statements follow from the construction.

CONSTRUCTION, from the Kac-Peterson data alone.  SU(3) at level k = 2: dual Coxeter g = 3,
so k+g = 5 and c = k*dim(g)/(k+g) = 16/5.  The six integrable weights are the (a,b) with
a+b <= 2.  The inverse Cartan matrix of A2 gives

    h(a,b) = (a^2 + b^2 + ab + 3a + 3b)/15,    T = diag(exp(2*pi*i*(h - c/24))),

and c/24 = 2/15, so the six T entries are zeta_15 to the (a^2+b^2+ab+3a+3b-2) mod 15 --
printed below -- and ord T = 15 is derived, not assumed.  S is the Kac-Peterson sum over
the six Weyl elements of A2.  Everything lands in Q(zeta_60), the field this paper's own
proof of the coupling law names, and is evaluated at primes p = 1 mod 60 so that zeta_60
lies in F_p and the arithmetic is exact integer arithmetic.

CONTROLS FIRST, and they are what make this rho rather than some other matrix: the four
modular relations T^15 = I, S^4 = I, S^2 = C (charge conjugation) and (ST)^3 = S^2.  A
wrong normalisation fails them.

Then: the image order 2880; theta = charge conjugation with eigenspaces of dimensions 4
and 2, commuting with the image so they are invariant; the theta-odd block of order 360
(the 360-versus-2880 distinction Scope (2880) exists to draw); and the 63 = 7 x 9
class-by-class match of (size, chi(A)trV2(B), trV2(A)trV2(B)) against an independently
built quaternion model of 2T x 2I.
...
print(f"""
  ...
  rho now exists in the source, so neither statement is a claim about an absent matrix.""")
