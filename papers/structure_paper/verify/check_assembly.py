#!/usr/bin/env python3
"""
Appendix B -- the polyhedral "assembly" condition narrows NOTHING.

## WHY THIS SCRIPT EXISTS

Scope (assembly) recorded that the classification behind Theorem (what the polyhedral
candidates do, and do not, decide) was described but not deposited, so that theorem was an
assertion ABOUT a computation.  This is the computation, and it REFUTES the classification.

The six groups are constructed as the theorem describes -- the binary ones as unit
quaternions (2T the 24 Hurwitz units, 2O the 48, 2I the 120 icosians), the others as
permutation groups -- and conjugacy classes, power maps and characters are COMPUTED by
Dixon's algorithm, never transcribed.  The prime is the least p > C(29,3) = 3654 with
p = 1 mod 120, the lcm of the six exponents: every character value then lies in F_p and
every multiplicity below is recovered unambiguously.  All arithmetic is exact mod p.

THE DECIDER is the block-sum lemma.  If W is an IRREDUCIBLE module carrying a non-zero
invariant cubic f, its trilinear form has ZERO radical -- the radical is G-stable, hence 0
or W, and W would force f = 0.  So a block-diagonal sum of such modules is an assembly in
the paper's sense, and 27 is reachable for every candidate:

    A4, 2T:  27 x (a linear character of order 3)
    S4, 2O:   9 x 3
    A5, 2I:   3 x 4  +  3 x 5

The witnesses for A4 and 2T are 27 copies of a non-trivial LINEAR character -- the
construction the first version of the theorem was refuted for (27 copies of the TRIVIAL
module), twisted just enough to survive the first repair.  What the definition fails to
control is MULTIPLICITY, not triviality.

The S4 witness is additionally checked here on explicit matrices over Q: invariance on
every group element and every triple, and radical zero.

Exact throughout.  sympy is used only for the explicit-matrix witness.
"""
import itertools
import random
