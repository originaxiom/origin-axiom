#!/usr/bin/env python3
"""
Appendix B -- the quotients of pi_1(4_1) onto the polyhedral groups.

## WHY THIS SCRIPT EXISTS

Scope (assembly) records that the polyhedral "assembly" condition narrows nothing, so the
entrance to 2T rests entirely on the arithmetic surjection.  This script is that surjection's
exhaustive check, and it also settles two statements the paper previously carried as a
conjecture and an unqualified count:

  pi_1(4_1) surjects onto NEITHER 2I NOR A_5  -- exhaustively, over all 14400 and 3600 pairs;
  the count "exactly two quotients" onto 2T holds UP TO Aut(2T) -- 48 surjections, 4 classes
  up to conjugacy, 2 up to the full automorphism group -- and the paper must say which.

CONTROL, before anything is read: the relator's exponent sums must be (1,0).  If they differ,
the presentation is not the object's and nothing below counts.

Exact integer arithmetic mod 3 and mod 5 throughout; no dependencies beyond the standard
library.
"""
...
gate("the count 'exactly two such quotients' holds UP TO Aut(2T)", aut3 == 2,
...
print("READING")
print("=" * 76)
print("  The conjecture is now a THEOREM: pi_1(4_1) surjects onto neither 2I")
