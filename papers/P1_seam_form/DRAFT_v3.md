...
**Complete draft v3 — 2026-07-09. v3 folds in the adversarial prior-art assessment (B491): the seam
selection rule is APPEARS-NOVEL / NEEDS-SPECIALIST, with the citations and the Dong–Lin–Ng
signed-permutation argument added to §8. v2 (2026-07-08) revised after the adversarial panel: v1
mis-stated the value count (a channel-support size, 44, was reported as the number of distinct
...
orientation-reversal): the *seam form* (a,b) ↦ tr(Par·P_a·Q_b), with P_a, Q_b the spectral
projectors of W₁ and W₂, is defined on a 49-point support of the (a,b) order grid and takes
exactly **30 distinct values, all lying in ℚ(√5, √−3)**. Fourier-transforming its four Galois
...
**Theorem S1 (values).** The seam form is supported on 49 points of the (a,b) grid (a ranges over
{0,1,4,5,6,9,11,14,15,16,19}, b over {0,1,2,3,4,5,7,8,9,10,11}, with 49 of the pairs occurring),
and on those 49 points it takes exactly **30 distinct values, all nonzero, all in ℚ(√5, √−3)**.
...
*Why it should hold.* Each channel is a Galois-isotypic component of the character sum
tr(Par·P_a·Q_b); by the equivariance of the Weil representation under the Galois action on level-15
roots of unity (the mechanism behind Coste–Gannon's Galois symmetry in rational conformal field
theory), the component is a sum over a residue class and should vanish exactly when that class
carries no surviving frequency. Making "should" into "does", uniformly per channel, is what remains.
...
Per-claim script map: S1–S4/S11 (the seam bank B358–B367, B449; the 30-value/49-support count and
the per-channel supports {32,45,28,44} recomputed at revision via `B358_seam_certification` +
`B367_value_map`); S5–S7 (`B459_klein_four_verification/verify_patterns.py` — the five patterns,
counts 120/20/20/10/70, the two laws, and the never-realized ℚ(√−15) node, all reproduced at
revision); S8/S12 (`br1_br2.py`, B469); S9/S10 (`cross_table.py`, `kq_verify.py`, B474/B472). Each
carries a lock re-run at draft. The v1→v2 corrections (30 distinct values not 44; the broken
lattice with ℚ(√−15) absent and the zero stratum present; the value-torus / dual-torus separation;
Observation S5 demoted from a proved lemma to the central open problem) are recorded in the
