...
carried two fatal errors — a misidentified trace field (ℚ(√5) for ℚ(√−3), with a vacuous
absence) and an over-strong orientation law (claimed uniformly orientation-reversing; the paper's
own triple is orientation-preserving). Both are corrected here, and the thesis is weakened to what
the computation supports. Full prose in the owner's register. NOT yet through its own literature
gate or a re-panel. Claims traceable to the banked child-program and census record (B435–B443,
B467; L-INV, T-COLLIDE) and re-verified at revision (snappy trace fields, conjugate embeddings).**
...
field of 4₁(5,1) is the quartic field ℚ[x]/(x⁴ − x − 1) — Galois group S₄, discriminant −283
(prime, hence not a square), so the field has **no quadratic subfield whatsoever**. The
figure-eight's invariant trace field ℚ(√−3) therefore cannot occur inside it, and the parent's
arithmetic is laundered away by the filling.

*Remark (a correction to fold in).* One must not confuse two fields of the figure-eight. Its
...
mapping-class data, not in the hyperbolic trace field. The laundering statement is about the trace
field ℚ(√−3); the golden field ℚ(√5) is not a subfield of ℚ(√−3) and was never a candidate to
appear in the child. (An earlier draft asserted "ℚ(√5) is absent"; since the child has no
quadratic subfield at all, that absence is *vacuous* — true but empty. The non-vacuous, correct
statement is the absence of ℚ(√−3).)
