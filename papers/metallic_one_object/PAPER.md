...
**A draft (motivated, status-labelled).** Dritëro M. (with AI-assisted computation). 2026-06-28.
...
(quantization). The classical side is computable — via the Ptolemy variety the three SL(3,ℂ) components carry
complex volumes `{4·Vol(4₁), 0, 0}`, the geometric one the `Sym²` factor `(n³−n)/6=4` **[banked B244]**; and two
quantum limits must be kept apart (a control on the quantization itself): large *level* (fixed rep) encodes the
geometric saddle's Chern–Simons action in the WRT *phase*, whereas the large-*color* symmetric volume conjecture
gives the knot-*complement* volume `2.0299` — group-independently, since `a=qᴺ→1` — so it does **not** see the SL(3)
geometric `4·Vol` (which is principal-rep classical data, not any large-color limit) **[banked B246]**. Face (3) is
genuinely separate — so the recurrence of "SU(3)" is three objects, not one (a further check that the rhyme with the
Standard-Model gauge group, §5.4, is not an identification).
...
iff the knot is **amphicheiral**: for `4₁` (amphicheiral) `SU(2)₃=SU(3)₂=−2/φ` (the reduced colored-Jones value of
the *knot* `4₁⊂S³` — distinct from §4.3's *closed-bundle* WRT `−1/φ`; same golden origin, different 3-manifold and
normalization), while chiral knots give complex-conjugate pairs. So *amphichirality is precisely the condition that makes the level-rank coincidence exact* —
...
Finally, the "=conjugation" form is itself the fundamental-rep shadow of a **transpose duality**. The higher-color
level-rank duality `SU(2)_k` in color `N` (symmetric `[N−1]`) ⟷ `SU(k)₂` in the *transpose* (antisymmetric
`[1^{N−1}]`) holds **exactly** for `4₁` at every color and level (verified `N≤5`, reduced colored HOMFLY), because
the duality factors as `(a=q²↦q^k=−q⁻²) ∘ (transpose q↦q⁻¹)`; the fundamental `[1]` is self-transpose, so there it
collapses to the pure conjugation above — which is why the clean `=conjugation` is fundamental-specific. **[banked
B245]**
...
| does the holonomy break `E₆` to the SM gauge group? | `C_{E₆}(SU(2)_long)` via Sage branchings; and is the geometric holonomy `SU(2)`-valued? | **[resolved — the bridge fails, B247]** The Lie theory is real but generic: `C_{E₆}(SU(2)_long)=SU(6)` (`E₆⊃A1×A5`; a textbook SU(6) GUT), `C_{E₆}(SU(2)_short)=SU(3)×SU(2)` (no `U(1)`) — *not* the `SU(3)×SU(2)×U(1)²` proposed (a rank-argument error). The object-specific step then fails: the **geometric** holonomy is irreducibly `SL(2,ℂ)` (complex word-traces, `ℚ(√−3)`), while the `SU(2)` reps that *could* break `E₆` are a different character-variety component carrying a different field (`ℚ(√5)`, golden, `tr(ab)=φ`). The connection that *selects* `E₆` is provably not the one that could *break* it; no canonical identification exists. Gauge group real but generic; bridge unforced. |
...
Naculich–Schnitzer / arXiv:2106.15012 (level-rank duality of knot invariants); Itoyama–Mironov–Morozov–Morozov
arXiv:1203.5978 (colored HOMFLY of `4₁` in symmetric/antisymmetric reps); Kashaev, H.&J. Murakami (volume
conjecture), Aganagic–Vafa arXiv:1204.4709 (Q-deformed A-polynomial / fixed-`A` volume); Garoufalidis–Thurston–
Zickert (Ptolemy / SL(n) complex volumes). In-repo provenance: B210, B211, B221–B224, B228, B230, B233–B246 (and
`SYNTHESIS.md`, `ARITHMETIC_SELECTION.md`, `S041`).
