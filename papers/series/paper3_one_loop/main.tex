...
\hypersetup{pdfauthor={Dritero Mehmetaj},pdftitle={The boundary-graviton one-loop determinant},pdfsubject={58J52}}
...
\begin{center}\small
\textit{Paper III of four.} The others are I, \emph{The period-one locus in $\mathrm{GL}(2,\mathbb{Z})$} · II, \emph{A finite spectrum on an infinite lattice} · IV, \emph{What a class invariant cannot supply}.
...
\medskip\noindent\small\textbf{MSC 2020.} Primary 58J52; Secondary 11M36, 37C30, 81T30, 57K32.\\
\textbf{Keywords.} Ruelle zeta function, analytic torsion, one-loop determinant, hyperbolic three-manifold, cusp.\normalsize
...
\end{equation}
which is elementary and we claim no novelty for it. \textbf{As stated,
\eqref{eq:symfactor} is an identity of Euler products on the half-plane where every factor
converges absolutely, namely $\mathrm{Re}(s)>2+m$; we verified it there.} Each side continues
meromorphically, so by the identity theorem the two agree \emph{as meromorphic functions} wherever
both are defined, and in particular at $s=0$. \textbf{This continuation step is what licenses the
next display, and it is worth stating because the individual factors on the right are no longer
Euler products there.} At $s=0$ it reads
\[
...
\textbf{One route to closing it can, moreover, be ruled out.} Taking the ratio of \eqref{eq:symfactor} at $m$ and $m-1$ isolates
$\Rz(-m,\sigma_m)\cdot\overline{\Rz(m,\sigma_m)}$. \emph{If} one could write
$\Rz_{\rho(m)}(0)=T_X(\rho(m))^2$ --- Fried's theorem --- then Proposition~\ref{prop:tail} would
evaluate that ratio and give a reflection formula expressing $|\Rz(-m,\sigma_m)|$ through the
absolutely convergent $|\Rz(m,\sigma_m)|$.

\textbf{That route is closed.} Fried's hypothesis is acyclicity, and here it fails at every $m$:

\begin{proposition}[the coefficients are never acyclic]\label{prop:acyclic}
For the figure-eight knot complement and $\rho(m)=\Sym^{2m}\C^2$,
\[
   \bigl(h^0,h^1,h^2\bigr)=(0,1,1)\quad\text{for every }m\ge1,
   \qquad (1,1,0)\ \text{for } m=0 .
\]
The obstruction is peripheral and independent of $m$: the cusp's parabolic subgroup fixes
\emph{exactly one} line in $\Sym^{n}\C^2$ for every $n$, which forces $h^1=h^2=\kappa(X)=1$.
\end{proposition}

Consequently $\Rz_{\rho(m)}(s)$ has a nonzero order of vanishing at $s=0$ rather than a finite
value equal to a torsion, and the reflection formula does \emph{not} follow by this route.

\textbf{What would close it:} the cusped Ray--Singer theory of Park and Pfaff, whose acyclicity
hurdle is met by the canonical one-dimensional cusp-cohomology basis, with the \emph{leading
Laurent coefficient} at $s=0$ in place of the value. \textbf{We have not carried out that
derivation, and residue~2 remains open.} What is unconditional is \eqref{eq:symfactor} alone.
\end{residue}
...
The computations are reproduced by \texttt{verify/check\_n2\_abscissa.py} (\textbf{5 checks}, and
it \emph{aborts} unless the bite control first diverges below the abscissa) together with
\texttt{verify/dictionary.py}, over the complex length
