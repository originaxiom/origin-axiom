...
\textbf{Moreover, if Fried's theorem does apply to $\rho(m)$, the missing relation is forced.}
Taking the ratio of \eqref{eq:symfactor} at $m$ and $m-1$ isolates
$\Rz(-m,\sigma_m)\cdot\overline{\Rz(m,\sigma_m)}$; Fried converts the left-hand side into
$[T_X(\rho(m))/T_X(\rho(m-1))]^2$, and Proposition~\ref{prop:tail} evaluates that ratio. Cancelling
the convergent factor gives
\begin{equation}\label{eq:reflect}
   \bigl|\Rz(-m,\sigma_m)\bigr|
   =\Bigl(\frac{c(m)}{c(m-1)}\Bigr)^{2\kappa(X)}
     e^{-4m\vol(X)/\pi}\,\bigl|\Rz(m,\sigma_m)\bigr| ,
\end{equation}
whose right-hand side is absolutely convergent and computable. For $m004$ this yields
$2.109\times10^{-4}$, $1.925\times10^{-5}$ and $1.641\times10^{-6}$ at $m=3,4,5$, decaying by the
factor $e^{-4\vol/\pi}=0.0754$ as the derivation requires.

\textbf{What would close it:} verifying that Fried's hypotheses hold for $\rho(m)$ in the cusped
setting. \textbf{We do not verify them, and residue~2 remains open.} Without them
\eqref{eq:symfactor} still holds unconditionally, but \eqref{eq:reflect} becomes an implication
with an unchecked antecedent. It is in either case falsifiable: an independent evaluation of $\Rz$
at a negative integer confirms \eqref{eq:reflect} or refutes its antecedent. The residue has moved
from a wish across three theorems to a single hypothesis with a computed consequence.
\end{residue}
