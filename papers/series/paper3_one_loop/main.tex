...
lives at $s=n\ge2$. These are three evaluations of one twisted Ruelle family, and Fried's is
the single point the graviton product never visits. \textbf{What would close it:} a relation
between the family's value at $0$ and its values at the positive integers. This reframes the
residue from an existence gap --- ``no cusped torsion theorem'' --- to an evaluation gap, which
is a sharper and more attackable statement.
\end{residue}
