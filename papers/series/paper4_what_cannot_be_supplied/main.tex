...
We had $|\mathcal{F}|=14$. A scan of the $212{,}641$ orientable cusped census manifolds finds $77$
whose tetrahedra are \emph{all} regular ideal (shape $e^{i\pi/3}$, hence shape field
$\Q(\sqrt{-3})$); together with the original fourteen --- six of which are \emph{not} all-regular,
so the two criteria differ --- that is at least $83$ members. \textbf{This is a lower bound, not the
family.}
...
\item the cusp shape $2\sqrt3\,i$: \textbf{$o10\_150684$, $o10\_150685$, $o10\_150693$ and
      $t12840$} each carry a cusp of exactly that shape. These are multi-cusped and match on one
      cusp; a weaker statement restricted to single-cusped members might survive, but that is a
      \emph{different} claim, is untested here, and we do not retreat to it.
\end{itemize}
