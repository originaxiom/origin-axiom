...
**Draft v5 — 2026-07-08. v4 + panel round 2 (structure seat; see PANEL_v2.md): two
verified math-errors fixed — the (2,3)-minimality clause (A₂⁶ is central mod 15, so (1,6)
precedes it: "nontrivial" now means non-central mod 15) and the misprinted E″ witness
word (LLLRRLRRRLLR, trace 146, not the trace-73 misprint). Definitional cleanups
(breathable, the master-table legend) folded in. The correctness/novelty/overclaim/hostile
seats are re-queued for the next panel pass. Below is otherwise v4. The literature gate is COMPLETE (both rounds, six hunts;
`LIT_GATE.md`). Round 2 recalibrated four claims: the symmetric-pair identity is the
coordinate form of classical facts (Sarnak; Gehring–Martin) with the (1,2) instance
verbatim in Reutenauer; the root criterion is Northshield's; the amphichirality
criterion is Tian–Wang–Wang's (closed case) over Baake–Roberts/Floyd–Hatcher; and the
converse half of Theorem F(ii) is a folklore corollary of Appleby's injectivity — a
DOWNGRADE this draft accepts in full. What survives as ours: the family question, the
two-parameter formula and uniqueness scan, the principal-breather assembly, the
hierarchy witnesses, and the N = 15 quantum tables with their divisor-lattice reading.
...
M₁₂ − M₂₁ = mn(n−m): hence tr[A_m, A_n] = 2 − (mn(n−m))², and exactly one pair — (1,2),
...
form) — details in the text. (iii) is the Latimer–MacDuffee correspondence applied verbatim to x² − mx − 1
(Latimer–MacDuffee 1933; Taussky); Sarnak's reciprocal-geodesic count is the same
correspondence in negative-Pell/class-number form. Which t give h⁺(t²+4) = 1 is the
Yokoi problem (Yokoi 1986–88; cf. Biró's resolution of the companion conjecture). The
equivalence as displayed — family = locus ⟺ h⁺ = 1 — we found nowhere stated; it is an
immediate corollary of the correspondence, and we claim the assembly only. ∎
...
amphichiral. (Known in substance: the full criterion — both mechanisms, with
sufficiency-not-necessity — is proved for closed Sol torus bundles by Tian–Wang–Wang
(arXiv:2406.13241, Lemma 3.1, Thm 3.5), and the proof transplants to the punctured
case; the algebra "rev·swap = conjugate-to-inverse" is Baake–Roberts (J. Phys. A 30
(1997), Props. 3–5); the symmetry group of a punctured-torus bundle is the monodromy's
GL(2,ℤ)-normalizer (Floyd–Hatcher 1982, p. 268). Morimoto's unoriented classification
marks the trap the lemma must avoid: plain conjugacy to A^{±1} ignores orientation. We
state the word form with its proof; the witnesses and tables are ours.)
...
multiplicativity U(A)U(B) = U(AB mod N) at odd N — Kelmer); ⟹ follows from the
injectivity of the projective Weil representation in odd dimension (Appleby, J. Math.
Phys. 46 (2005), with the Bolt–Room–Wall lineage): [W₁ʲ, W₂ˡ] = I forces the image of
[A₁ʲ, A₂ˡ] to be scalar, hence the identity mod 15. Our exact check on all 240 points
of the order torus stands as confirmation, not claim. (iii) In particular
...
(each cell: κ_q value, tier. Legend — the four Galois channels are q = √5, r = √−3,
s = √−15 [and the fourth, √5·√−3 = √−45 ∝ √−5, is never isolated]; a tier names which
channels VANISH: *free* = none vanish (fully active, count 120), *qs* = {√5,√−15} vanish
(20), *rs* = {√−3,√−15} vanish (20), *qrs* = {√5,√−3,√−15} vanish (10), *dark* = all four
vanish (70). Cell sizes are φ(20/gx)·φ(12/gy).) **The mechanism is
...
[As v1, extended by two chapters: this paper's own referee panel found the two-block
gap in the first amphichirality theorem, the class-group counterexamples to "family =
locus", the generic-symmetric-pair mechanism behind Theorem A, and the missing
Hannay–Berry shelf; the literature gate then downgraded the converse of Theorem F(ii)
from "ours, verified" to "folklore corollary (Appleby injectivity), confirmed" and
returned the root criterion to Northshield. Each demotion is recorded with the same
equanimity as the promotions. The panel is part of the method; the method is part of
the result.]
