...
**Draft v6 — 2026-07-08. v5 + panel round 2 COMPLETE (all five seats; see
PANEL_v2_FINAL.md). No mathematical claim was broken; every computation reproduced. The
...
*Remark (provenance of the correction).* An earlier draft claimed
...
1. The family quantum column (κ_q at each pair's product trace; does closure track CRT
   ...
   constants (both numerically located to the working precision recorded in the reproducer,
   neither identified in closed form; the exclusions below are height-bounded and are NOT
   non-algebraicity certificates — reproduce them from the reproducer's full-precision
   values, not from the truncated digits printed here). c = 0.9341020180577879802641878…
   is the limit of the tower's Fibonacci-additivity defect (Fibonacci-additive to < 10⁻²⁷
   by rung 13; a numerical limit, not proved to converge); λ_chain = 1.5770574412266694…
   At the reproducer's working precision (≈ 40 digits) PSLQ finds no integer relation of
   degree ≤ 8 with height ≤ 10⁴ for either — but the digit strings printed here are too
   short to reproduce that exclusion (at 18 digits PSLQ returns spurious relations), so the
   claim lives with the reproducer. 4. Why 15
...
returned the root criterion to Northshield, and the round-2 panel then reframed §4 as the
level-15 shadow and corrected the two §4.2 curiosities. Each demotion is recorded with the
same equanimity as the promotions, as a methodological note on how the draft was built —
the result.]
