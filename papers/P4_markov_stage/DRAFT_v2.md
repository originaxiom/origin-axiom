...
**Draft v2 — 2026-07-08. Rewritten after adversarial panel round 1 (adjudication in
`PANEL_v1_ADJUDICATION.md`; all FATAL/MAJOR findings addressed; two results are now
STRONGER than v1's — the symmetric-pair mechanism and the principal-breather theorem).
Narrative beats ⟡ are the owner's. Novelty language: §5's "we could not locate" only,
with the quantum-half items SUSPENDED pending the Hannay–Berry gate.**
...
N, e.g. Nobs–Wolfart, Gérardin; the arithmetic of quantized cat maps: Degli Esposti,
Kurlberg–Rudnick). Par is the parity operator (Par ψ)(x) = ψ(−x) — the Weil image of
−I. **Proposition 4.0**: under the level-15 Weil representation R ↦ W_R and L ↦ W_L up
to projective scalars; the representation at odd level is genuine (linearizable), so
commutators [W(u), W(v)] are scalar-free and depend only on the images of u, v in
SL(2, ℤ/15). ord(W₁) = 20, ord(W₂) = 12 (computed exactly; these orders define the
...
(each cell: κ_q value, tier; cell sizes are φ(20/gx)·φ(12/gy); the table IS the proof of
(ii), verified exactly on all 240 points). Consequences read off the table: a fully
...
**SUSPENDED pending the quantized-cat-map gate** (Hannay–Berry corpus): any novelty
statement about §4's closure theorem, two-character formula, or divisor-lattice
factorization — these may live in the cat-map literature in some form, and we say
nothing until that search is done.
