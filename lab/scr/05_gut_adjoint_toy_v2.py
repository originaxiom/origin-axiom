# grokscr/scr/05_gut_adjoint_toy_v2.py
# Step 2: Add m² Tr(Φ²) for spontaneous breaking
...
DT = 0.05 / N
DX = 1.0
LAMBDA = 0.01  # Weak nonlinearity
M2 = -0.1  # Negative mass squared for breaking
DISORDERED = True
SEED = 42
DIM = 24

def get_run_name(n, eps, lam, m2):
    return f"gut_v2_N{n}_eps{eps:.0e}_lam{lam:.2f}_m2{m2:.2f}"

parser = argparse.ArgumentParser(description='GUT Adjoint v2 with m²')
...
RUN_NAME = get_run_name(args.n, args.epsilon, args.lam, args.m2)
...
def potential_deriv_adj(phi, lam, m2):
    phi_mat = phi.reshape(-1, DIM, DIM)
    phi2 = phi_mat @ phi_mat
    phi3 = phi2 @ phi_mat
    dV_dPhi = 2 * m2 * phi_mat + 4 * lam * phi3
    return dV_dPhi.reshape(phi.shape)

def evolve_gut(phi, pi, steps, dt, epsilon, lam, m2, with_constraint=False):
    traces = []
    energies = []
    constraint_count = 0
    ...
    for step in range(steps):
        ...
        force = laplacian_adj(phi_half) - potential_deriv_adj(phi_half, lam, m2)
        pi_new = pi + dt * force
        ...
        pot = m2 * np.mean(np.real(np.trace(phi @ phi, axis1=-2, axis2=-1))) + lam * np.mean(np.real(np.trace(phi @ phi @ phi @ phi, axis1=-2, axis2=-1)))
...
print(f"\n=== GUT Adjoint v2 with m² ===")
print(f"Folder: {DATED_PREFIX}")
print(f"N={args.n}, ε={args.epsilon}, λ={args.lam}, m²={args.m2}, θ★={theta_star:.6f}")

print("Unconstrained...")
tr_no, e_no, _ = evolve_gut(phi.copy(), pi.copy(), args.steps, DT, args.epsilon, args.lam, args.m2, False)

print("Constrained...")
tr_yes, e_yes, count = evolve_gut(phi.copy(), pi.copy(), args.steps, DT, args.epsilon, args.lam, args.m2, True)
