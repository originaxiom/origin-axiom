...
**Seat:** fresh physics seat (fc) · **Date:** 2026-09-06 · **Status:** seat report, frontier, not banked; every number computed on this bench. Scripts `computations/r72_inner_lift.py` (run record `r72_inner_lift_run.txt`, ~5 min) the SnapPy session recorded in `computations/r72_m202_snappy.py` (run record `r72_m202_snappy_run.txt`; SnapPy 3.x, cypari), `computations/r72b_m202_lines.py` (run record `r72b_m202_lines_run.txt`, ~3 min: the isometries as automorphisms of π₁(m202) with their conjugators, and the cusp of every endpoint of every fixed line), and `computations/r72c_d2_is_the_so10_axis.py` (run record; the SM seat's D₂ in this frame). Corrections at source: banners on R69 §4, R70 §2, R71 §0/§3 (their "θ-even" statements are the record's *outer* lift, §1 below). Follows B1291 (main: "three is excluded with one cusp, the escape is two — leave the knot, keep the field") and R71.
...
**The SM seat's B1265 is the same fork in real-form language, and it answers on the inner side.** B1140 banked "the fork": E₆(−14) (𝔨 = so(10)⊕u(1), inner) took the charges, E₆(−26) (𝔨 = 𝔣₄, outer) the geometry. B1265 (PROVED there, 2026-09-06) computes that **D₂ — the object's own twist (B916's 11-flip, decoded by B1250 as the SO(10) grading) — is an involution by a torus element, hence inner, of signature −14**, and that the outer branch is reachable only by θ_D — adding, in its own fence, that *"the object's θ is that automorphism … is not established here … θ is trivial on the character variety, which is a reason to doubt it carries the geometric content."* Verified in this frame (`r72c`): with the invariant form scaled by 3, D₂(w) = (−1)^{3⟨w13,w⟩+1} flips exactly 11 weights of the 27 (w13 and the 10) and fixes the 16, for every choice of w13; on the roots it fixes exactly the 40 of the D₅ orthogonal to w13, so 𝔨 = so(10)⊕u(1), rank 6, and **the coweight dual to w13 has centraliser D₅⊕T¹ — D₂'s axis is the SO(10) direction, the Weyl class of ω₁^∨.** For the count only innerness matters: any torus-element involution fixes 𝔥 pointwise and acts innerly on every C(u), so **under the record's own D₂ the untwisted count of §5 is the count**, with no "θ-even" restriction at all. The record thus already holds an inner involution (D₂, E₆(−14)) and an outer one (θ_D, E₆(−26), F₄), and which of them the geometric isometry induces on the gauge algebra is exactly the row named above; B1265's own doubt about the outer one and B353's H¹-only scope point the same way.
...
| the direction u | **the record's, conditionally**: the SO(10) direction is the axis of the object's own D₂ (B916 → B1250 → B1265 on the SM branch, PROVED there; B1250's form verified here in `r72c`, B916's provenance not re-derived), while main's B1017 lists the rank-closing VEV direction ⟨1⟩ as the *unsourced* fifth input — the two benches differ on whether it is sourced, and this seat has not adjudicated B916. If D₂'s derivation stands, u is not a choice and the count is 3 × (16 ⊕ 10 ⊕ 1) with no direction chosen |
...
- B1268's bound N(27) ≤ 1 on m004 is for smooth flat deformations of the holonomy; it does not constrain a singular Higgs field with sources (R69 §2), on m004 or on m202.
