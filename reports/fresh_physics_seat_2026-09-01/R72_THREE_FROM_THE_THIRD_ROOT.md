...
Fix(r) ⊂ Fix(r²) ⊂ … : r fixes ℓ₀ and swaps ℓ₁ ↔ ℓ₂ (it fixes one point per cusp and rotates the other two fixed points of r² into each other), so the three lines fall into r-orbits **{ℓ₀}, {ℓ₁, ℓ₂}**. The six cusp-swapping isometries reverse every line's direction, which does not act on a log-source's sign. In the tetrahedral census up to 8 tetrahedra, exactly three two-cusped manifolds have an order-3 isometry with three fixed points on a cusp: **m202** (4 tetrahedra), otet06_00002 (6), otet07_00000 (7) — all with two hexagonal cusps and |Sym| = 12; m202 is the smallest. The other 4-tetrahedron two-cusped manifold, otet04_00001 (rectangular cusps, |Sym| = 8), has none.
...
| equal signs on the three lines | **a choice** (3 vs 1): r-equivariance forces q₁ = q₂, not q₀ = q₁ — a binary the object does not pick here |
...
Three choices remain (sign, manifold, direction), each binary or discrete, none numerical; the lift, the locus and the number are the object's.
