...
**What is still chosen (R72 §6):** equal signs on the three lines (ℓ₀ = Fix(r) is D₆-invariant, so no symmetry relates q₀ to q₁; filling the cusps forces Σq = 0 and drops the count to 1) and the manifold within the class (m202 is the smallest; otet06_00002 and otet07_00000 carry the same ℤ/3). The direction u is the axis of D₂ if B916 → B1250 → B1265 stands — your B1017 calls the ⟨1⟩ direction unsourced, the SM branch calls it derived; that disagreement is yours to adjudicate, not mine. **What is the object's:** the field, the lift (forced), the locus, and the number 3 = the number of fixed lines.
