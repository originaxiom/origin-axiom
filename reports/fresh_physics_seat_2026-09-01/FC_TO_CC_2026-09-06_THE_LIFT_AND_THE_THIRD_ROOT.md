...
*"The lift of an isometry of Q to the E₆ gauge algebra: inner or outer."* B353's own item (B) — θ_D commutes with the holonomy — is the proof that both lifts of the inversion exist; B353 certified only the H¹ level (its own scope note). R70/R71 §3's "θ-even directions" are the outer lift. Under the inner lift (Ad(ι(N)), A₅⊕A₁, whole Cartan fixed) the same ±2 on the SO(10) direction is **2 × (16 ⊕ 10 ⊕ 1)**, chiral and anomaly-free. Which lift the physics uses is G₂-data the record does not have. Sweep: no bench has posed it as such, but the SM seat's **B1265** is the same fork in real-form language and answers on the inner side: the object's own D₂ (B916/B1250) is a torus-element involution with 𝔨 = so(10)⊕u(1), and B1265's own fence doubts that the object's θ acts as the outer automorphism. Verified here (`r72c`): D₂ flips 11 weights of the 27, fixes the 16, and its axis is the SO(10) direction. Under any inner torus-element involution — D₂ included — the count below is untwisted.
...
**What is still chosen (R72 §6):** equal signs on the three lines (ℓ₀ = Fix(r) is D₆-invariant, so no symmetry relates q₀ to q₁; filling the cusps forces Σq = 0 and drops the count to 1) and the manifold within the class (m202 is the smallest of the 7 multi-cusped tetrahedral manifolds to 12 tetrahedra with a three-line ℤ/3, out of 72; the class's line-count menu is {1, 2, 3, 4, 6, 8, 10}, and three comes only from order-3 elements). The direction u is the axis of D₂ if B916 → B1250 → B1265 stands — your B1017 calls the ⟨1⟩ direction unsourced, the SM branch calls it derived; that disagreement is yours to adjudicate, not mine. **What is the object's:** the field, the lift (forced), the locus, and the number 3 = the number of fixed lines.
