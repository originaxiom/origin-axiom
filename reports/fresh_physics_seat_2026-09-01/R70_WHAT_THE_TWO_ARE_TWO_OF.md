...
> **Correction banner (R71, 2026-09-06).** The pairing in this section was checked by *dimension* ("6 with 6, 3 with 3"). At the representation level (`computations/r71_theta_even_pairing.py`) the pairing is the θ-twist, S(u) = 2·⊕_{q>0}(27_q ⊕ 27_q∘θ), which is a theorem for every θ-even u; it is complex conjugation on A₅ (ω₂^∨) and on D₄ (ω₁^∨+ω₆^∨), so those two rows are vector-like as stated, but on **ω₄^∨** (A₂⊕A₂⊕A₁) and **ω₃^∨+ω₅^∨** (A₁⊕A₂⊕A₁) θ swaps or conjugates the wrong factor and the spectrum is **chiral** under the non-abelian centralizer — and carries a nonzero SU(3) cubic anomaly (2 and −4 in units of the fundamental), so it is not a spectrum of any consistent configuration. The headline "the two of R69 are two copies of a vector-like pair" is therefore true on the anomaly-free θ-even directions and replaced by "vector-like or anomalous" on all 928 tested. See R71 §3. **And (R72):** all of this is under the record's *outer* lift of θ to E₆; the inner lift (B353 (B) proves both exist) fixes the whole Cartan, and under it the count on the SO(10) direction is 2 × (16 ⊕ 10 ⊕ 1). Which lift the physics uses is G₂-data the record does not have.
