"""R34 normal-form v2: validate operands BEFORE subtraction; v1 stays."""
...
def residual(left, right):
    operands = (sp.sympify(left), sp.sympify(right))
    if any(value.has(sp.Float) for value in operands):
        raise ValueError('exact expressions required')
    return sp.cancel(sp.together(operands[0]-operands[1]))
