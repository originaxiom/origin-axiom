# Read-only Git-content retrieval for R28. Not a physics producer.
...
PATTERN = /renormali[sz]|defect.{0,24}action|source.{0,24}action|action.{0,24}(source|defect)|BPS.{0,24}(energ|potential)|non.?normali[sz].{0,30}(source|amplitude)|source.{0,30}(kinetic|norm)/in
EXTRA_TIPS = %w[
...
abort('Usage: ruby source_action_history_scan.rb NEW_OUTPUT.json') unless ARGV.size == 1
