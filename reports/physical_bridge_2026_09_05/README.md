...
**Preceding: [R21's anomaly completion and holonomy cost](ANOMALY_COMPLETION.md).**
...
[CC's banking/numbering relay receipt](BANKING_RECEIPT.md): no new B number
is taken; all R labels here are path-local. Reserved ranges and sB aliases
are respected. A local research checkpoint is not a completed main-bank
...
**Latest source-map result:** [R11's family action](FAMILY_ACTION.md) separates
the founding ratio's literal left action into a pure family rotation and
a commuting E6 Weyl action, exactly for all twenty order-three units.
The marked group map, root actions and covariance are verified; physical
zero modes are not inferred from a triplet of fiber components. A newer
...
latest combined focused run has **126 passed, 3 failed, 8 errors**, including
eleven R11 adapter/mathematical checks, the eight R10 tests, all R8/R9 tests,
the five B1255 tests, G2/export controls, four B1084 locks and three B1105
scope checks. Failures are the preserved original G2 exporter and the two
R7 small-step controls. The eight errors are NEW R11 original-fixture
integer-domain failures; the separately sealed adapter passes the same
eight mathematical lock functions and three conversion controls. No
original test or seal was overwritten. Archive and publication-gate failures remain
...
FAMILY_ACTION.md now discharges the named root-action computation, not
that full duty. Next verify the source branch's actual three-fold closing,
gauge action, zero modes, cubic and kinetic forms. Its three-class/nonzero-
cubic claim is present, not missing. Preserve the Lg/Wg distinction and
do not identify the family SU3 outside E6 with trinification inside E6.
