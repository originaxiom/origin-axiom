...
| Phase B — Frontier | **in progress** — probes B1–B94 logged (ledger V1–V78). Trace-map selector packaged as conditional C5; metallic `SL(3)` arithmetic isolated as PC12. **Headline results, all honestly labeled:** the `SL(4)` figure-eight **`M⁴=L` proved symbolic-exact** over ℚ(ω) (B89); the `SL(n)` trace-map **tower** `char(J(m))=∏char(±Mᵏ)` proved `n≤4` (B80, `Z[m]` / CRT-F_p) and reduced to an explicit `GL(2,ℤ)`/Sym plethysm conjecture (B89-T; three shortcut routes foreclosed); **degree=rank** `[A,B]=(−1)ⁿ⁻¹μⁿ` proved `n=3,4` (B71/B89), peripheral form L1b uniform (B90, audited V75); the figure-eight **A-polynomial family** `L=(−1)ⁿ⁻¹Mⁿ` (B83). **Paper 0 — the self-reference grounding** (B92–B94, V76–V78): the metallic family = the `det=−1`/period-1 slice up to `GL(2,ℤ)` conjugacy (`proven`), and `det=−1` is *structurally distinguished* — it is exactly what gives the tower its parity (*"universal catalog, det=−1 parity"*); motivation quarantined in `paths/philosophical/`. The systematic **physics-paths sweep is robustly negative** and the physics chapter is **closed** (`frontier/physics_probes/`: every reachable anchor is special to `n=2`/`m=1`). Open: the uniform-`n` tower (the Sym plethysm) and degree=rank; the Lawvere/renormalization grounding (L2); genus-2 generality |
