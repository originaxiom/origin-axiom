# grokscr/scr/04_frw_embedding_v2.py
# Complete updated FRW embedding: less frequent constraint, physical floor, lower H0
...
NOISE_AMP = 0.5
SEED = None
H0 = 1e-4  # Slower expansion
G = 1.0
CONSTRAINT_FREQ = 10  # Apply axiom only every 10 steps

def get_run_name(n, eps, lam, dis):
    return f"frw_3d_N{n}_eps{eps:.0e}_dis{int(dis)}_lam{lam:.2f}"

parser = argparse.ArgumentParser(description='Grok FRW v2')
...
# [laplacian_3d, potential_deriv, initialize_field functions — same as before]
...
def evolve_frw(phi, pi, a, steps, dt, epsilon, lam, with_constraint=False):
    ...
    comoving_volume = args.n**3
    for step in range(steps):
        kin = 0.5 * np.mean(np.abs(pi)**2)
        grad = -0.5 * np.real(np.mean(np.conj(phi) * laplacian_3d(phi)))
        pot = lam * np.mean(np.abs(phi)**4)
        rho = kin + grad + pot
        H = np.sqrt(8 * np.pi * G * rho / 3) + H0
        
        phi_half = phi + 0.5 * dt * pi
        force = laplacian_3d(phi_half) - potential_deriv(phi_half, lam)
        pi_new = pi + dt * force - 3 * H * dt * phi_half
        phi_new = phi_half + 0.5 * dt * pi_new
        
        physical_A = np.abs(np.sum(phi_new)) * a**(3/2)
        min_A = epsilon * np.sqrt(comoving_volume * a**3)
        
        if with_constraint and (step % CONSTRAINT_FREQ == 0):
            if physical_A < min_A:
                constraint_count += 1
                phase_dir = np.sum(phi_new) / physical_A if physical_A > 1e-15 else 1.0
                target_A = min_A * phase_dir
                adjustment = (target_A - np.sum(phi_new) * a**(3/2)) / (comoving_volume * a**3)
                phi_new += adjustment / a**(3/2)
        
        phi, pi = phi_new, pi_new
        a += dt * H * a
        
        global_As.append(physical_A)
        energies.append(rho)
...
# Save raw
np.savez(os.path.join(DATED_RAW, 'data.npz'), **{
    'As_no': As_no, 'Es_no': Es_no, 'as_no': as_no, 'Hs_no': Hs_no,
    'As_yes': As_yes, 'Es_yes': Es_yes, 'as_yes': as_yes, 'Hs_yes': Hs_yes
})

# Plots
for label, no, yes in [('amplitude_eff', As_no, As_yes), ('energy_density', Es_no, Es_yes),
                       ('scale_factor', as_no, as_yes), ('hubble', Hs_no, Hs_yes)]:
    plt.figure(figsize=(10,6))
    plt.plot(no, label='No Constraint')
    plt.plot(yes, label='With Origin Axiom')
    plt.title(f'{label.replace("_", " ").capitalize()} – {RUN_NAME}')
    plt.xlabel('Time Step')
    plt.ylabel(label.split('_')[0].capitalize())
    plt.legend()
    plt.grid(True, alpha=0.3)
    for ext in ['.png', '.pdf']:
        plt.savefig(os.path.join(DATED_FIG, f'{label}{ext}'), dpi=200, bbox_inches='tight')
    plt.close()

print("Run complete! Check new dated folders.")