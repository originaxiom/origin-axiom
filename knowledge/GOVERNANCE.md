...
> but it introduces no new result and is never a premise of a proof. Standard external facts are labelled as such;
> the project's own contributions are pointed to, not re-derived here.

## Rules

- **Self-contained.** A `K`-doc should be readable on its own; assume only general mathematical literacy.
- **External vs ours.** Clearly separate standard textbook material (cited to the literature) from the project's
