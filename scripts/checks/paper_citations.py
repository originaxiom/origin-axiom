...
def load():
    """Return (whole file, PROSE body).

    The scanned region is prose only. Excluded, because a citation there would be wrong rather
    than missing: the keywords / MSC / arXiv metadata block (a keyword list names results the way
    an index does), and the GENERATED provenance appendix (a table of claim rows, not argument).
    This was found the honest way -- adding the keywords block in S3 made "McKay" first appear in
    metadata and the checker flagged it, which is the checker measuring the wrong region rather
    than the paper missing a citation."""
    ...
    body = s[i:j]
    # Strip EVERY generated block, not a hand-maintained list of them. Three times now a new
    # generated block has moved a name's first occurrence out of prose and into machine output
    # (the keywords/MSC metadata, the provenance appendix, then the chain table, whose link titles
    # legitimately read "Thurston/Riley" and "Morse-Hedlund"). A citation inside generated output
    # would be wrong rather than missing, so the rule is general: only PROSE is scanned.
    body = re.sub(r"% ---- GENERATED.*?% ---- end generated[^\n]*\n", "", body, flags=re.S)
    if "\\textbf{Keywords.}" in body and "\\normalsize" in body:
        body = body[:body.index("\\textbf{Keywords.}")] + body[body.index("\\normalsize") + 11:]
    return s, body
