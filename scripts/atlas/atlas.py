...
LEXICON = {
    ...
    # B1341 (2026-09-12) extends B1247's repair with the two questions it still had no word for.
    # Measured the same way B1247 measured its own: "feedback" returned 4 corpus hits in 1300+ arcs
    # and "least action" returned zero arcs joined to the object, while B6 (an Euler-Lagrange
    # equation, OPEN since week one), B21 (the anti-Poisson half step) and B37 (the feedback
    # predicate, measured and then fenced) had all been banked for a year. Same mechanism, same
    # fix: a word for the QUESTION. NOTE the Markov hazard B1247's row X31 flags -- "markov
    # blanket" is matched as a PHRASE here, never bare "markov", which in this corpus overwhelmingly
    # means Markov TRIPLES (the trace map's phase space), an unrelated object.
    "action":         dict(kind="question", conserved="structural", domain="dynamics",
                           gloss="is there a least-action principle, and at which step -- the "
                                 "variational structure the object supplies rather than one lifted "
                                 "onto it (B6's derived potential, B21's bracket, B1341's theorem "
                                 "that the half step is generated by no Lagrangian and the "
                                 "monodromy is)",
                           patterns=["least action", "variational", "Lagrangian", "Euler.?Lagrange",
                                     "Hamiltonian", "symplectic", "anti.?Poisson", "Poisson bracket",
                                     "generating function", "kinetic term", "equation of motion",
                                     "action principle", "stationary point", "field equation"]),
    "feedback":       dict(kind="question", conserved="no", domain="dynamics",
                           gloss="does the object act on its own state, and does it READ what it "
                                 "acts on -- B37's operational split (feedback and an invariant: "
                                 "yes; self-model: no), fenced against awareness readings, and "
                                 "the closed-blanket question X31 poses",
                           patterns=["feedback", "self.?model", "invariant.?memory", "quotient memory",
                                     "closed loop", "markov blanket", "autonom", "reads its own",
                                     "acts on its own", "operational self"]),
