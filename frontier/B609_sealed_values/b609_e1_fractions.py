"""B609-E1 — the Killing-class mixing fractions (sealed prereg 0bcc2ad4...): which theta-odd root content sits in which
...
# ---- E1: the Killing-class mixing fractions ---------------------------------
# The prereg's trace-form realization: the compact (Hermitian) trace norm
# on the 27, tr(X X^dagger) = sum |X_ij|^2 with |a+bw|^2 = a^2 + 3b^2 —
# root spaces are orthogonal in it and class fractions are scale-free
# exact rationals.
from fractions import Fraction as FR
...
def hnorm2(X):
    ...
    for row in X:
        for x in row:
            if not x.is_zero():
                tot += FR(x.a) * FR(x.a) + 3 * FR(x.b) * FR(x.b)
    return tot


def knum2(x):
    return FR(x.a) * FR(x.a) + 3 * FR(x.b) * FR(x.b)


# theta scalar per pair: theta(x_a) = c_a * x_{sigma a}
pairs_by_h = {}
for (a, b) in pairs:
    pairs_by_h.setdefault(heights[a], []).append((a, b))

print("E1 — the Killing-class fractions per odd line:", flush=True)
for m in (4, 8):
    print(f"block V(m={m}):", flush=True)
    for j, vec in enumerate(BLOCKS[m]):
        ...
        for (a, b) in pairs_by_h.get(hw, []):
            xa = roots[a]
            txa = theta(xa)
            # c: txa = c * x_b (solve on the first nonzero entry of x_b)
            xb = roots[b]
            c = None
            for i in range(27):
                for jj in range(27):
                    if not xb[i][jj].is_zero():
                        c = txa[i][jj] * xb[i][jj].inv()
                        break
                if c is not None:
                    break
            assert is_zero_mat(msub(txa, mscale(c, xb))), "theta scalar"
            odds.append(msub(xa, txa))
            rootinfo.append(((a, gsm_class(a), hnorm2(xa)),
                             (b, gsm_class(b), knum2(c) * hnorm2(xb))))
        co = solve_in(odds, vec)
        # accumulate class masses: coefficient r on combo -> r*x_a - r*c*x_b
        masses = {}
        for r, ((a, ca, na), (b, cb, nb)) in zip(co, rootinfo):
            if r.is_zero():
                continue
            r2 = knum2(r)
            masses[ca] = masses.get(ca, FR(0)) + r2 * na
            masses[cb] = masses.get(cb, FR(0)) + r2 * nb
        tot = sum(masses.values())
        fr = {k: v / tot for k, v in masses.items()}
        s = sum(fr.values())
        assert s == 1, "E1-a FAIL: fractions do not sum to 1"
        pretty = "  ".join(f"{k[0]}{k[1]}:{v}" for k, v in sorted(fr.items()))
        print(f"  wt {2*hw:+3d}: {pretty}", flush=True)
        if m == 8 and hw == 8:
            assert list(fr.values()) == [FR(1)] and \
                list(fr.keys())[0] == ("multiplet", (1, 1, 2)), "E1-b FAIL"
print("E1-a PASS (all lines sum to 1); E1-b PASS (the tip is pure (1,1,2))",
      flush=True)
