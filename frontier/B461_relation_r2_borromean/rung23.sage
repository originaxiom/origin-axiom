...
print("== rung 2: SL(3) F_p dimension probes (p=31991) ==")
for nm in ['L6a4', 's776']:
    dims = []
    for ci, sys_ in enumerate(d[nm]['3']):
        Rp = PolynomialRing(GF(31991), sys_['vars'], order='degrevlex')
        I = Rp.ideal(build(Rp, sys_))
        dims.append(int(I.dimension()))
    print(f"  {nm}: per-class dims = {dims}")

print("== rung 3: SL(2) exact eliminations (all classes) ==")
for nm in ['L6a4', 's776', 'm129']:
    ...
    print(f"  {nm}: {zero_dim} zero-dim SL(2) classes; quad/higher factor fields: {sorted(fields)}")
