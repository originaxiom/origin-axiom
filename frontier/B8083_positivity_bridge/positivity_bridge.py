...
The arithmeticity theorem's proof argues on BLOCK SEQUENCES.  But the classification it
cites gives the arithmetic monodromies as those CONJUGATE to a power of one of three
words, and conjugacy in SL(2,Z) is a priori coarser than cyclic rotation of a positive
R,L word: nothing in GL(2,Z) preserves positivity of a word, and the paper itself notes
(for phi_1^3 and phi_4) that equal trace does not imply conjugacy.  So the step from
"conjugate to a power" to "same block sequence" needs a bridge, and the paper does not
state one.

THE BRIDGE.  On POSITIVE words the two relations coincide:

    two positive words in R, L are conjugate in SL(2,Z)
                    <=>  they are cyclic rotations of one another.

One direction is free: if w = uv then vu = u^{-1} w u.  The other is the content, and it
is classical -- the positive R,L word of a hyperbolic matrix is its continued-fraction
reduction cycle, equivalently the cutting sequence of its axis on the Farey tessellation,
and conjugation only moves the starting point.  This arc CHECKS it, over every positive
word of length <= 10 that contains both letters.

THE INSTRUMENT.  Conjugacy is decided by a COMPLETE invariant, not by a search: to
M = [[a,b],[c,d]] attach the indefinite binary quadratic form (c, d-a, -b), whose proper
SL(2,Z)-equivalence class is a complete conjugacy invariant (Latimer-MacDuffee, which the
paper already invokes elsewhere), and compute its cycle of reduced forms.  Two matrices
are conjugate exactly when trace and form-cycle agree.

QUANTIFIER (COMPUTE_THE_PROGRAM): the AXIOMS layer -- words in two letters and their
images in SL(2,Z).  Nothing about the member, the class, the sisters or the rows.

NOT PREREGISTERED.  The falsifier is stated in advance and is sharp: if any two cyclic
classes shared an invariant, the block-sequence argument would be unsound as written.
"""
import itertools
import json
import math
