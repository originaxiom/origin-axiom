...
spec = Counter(); SUBS_CANON = set()
def canon_sub(els):
    if not els: return ()
    A_=np.zeros((len(els),E.DIM),dtype=np.int64)
    for i_,z in enumerate(els):
        for k,val in z.items():
            A_[i_,k]=(val.numerator%P)*pow(val.denominator%P,P-2,P)%P
...
for coeffs in CAND:
    ...
    SUBS_CANON.add(canon_sub(sub))
# MEET-CLOSURE: z(S u T) = z(S) ^ z(T), so every SUBSET of C is a meet of singles.
# The single-direction sweep above covers the singles; closing under meet covers ALL
# subsets exactly -- which is what the paper's cascade actually does (z(x1), z(x1,y)).
def canon(els):
    if not els: return ()
    A_=np.zeros((len(els),E.DIM),dtype=np.int64)
    for i_,z in enumerate(els):
...
def to_els(cf):
    ...
    for row in cf:
        ...
        for k,co in enumerate(row):
            if int(co)%P: xx=E.vadd(xx,E.vmul(Fraction(int(co)%P),{k:Fraction(1)}))
        out.append(xx)
    return out
def meet(cA,cB):
    if not cA or not cB: return ()
    M1=np.array(cA,dtype=np.int64); M2=np.array(cB,dtype=np.int64)
    Big=np.vstack([M1,-M2])%P
    ker=nullspace(Big.T%P)
    res=[]
    for v in ker:
        comb=np.zeros(E.DIM,dtype=np.int64)
        for co,row in zip(v[:len(cA)],cA): comb=(comb+int(co)*np.array(row,dtype=np.int64))%P
        if np.count_nonzero(comb): res.append(tuple(int(t)%P for t in comb))
    return canon(to_els(res)) if res else ()
singles=set(SUBS_CANON)
lat=set(singles); frontier=set(singles)
while frontier:
    nxt=set()
    for A_ in frontier:
        for B_ in singles:
            C2=meet(A_,B_)
            if C2 not in lat: lat.add(C2); nxt.add(C2)
    frontier=nxt
print(f"\ndistinct centralisers over ALL SUBSETS of C: {len(lat)}", flush=True)
spec2=Counter()
for cf in lat:
    els=to_els(cf); spec2[(len(els),kill_rank(els))]+=1
print("su(5) INTERSECT z(S), over ALL subsets S of C:")
for (d,kr) in sorted(spec2):
    tag="   <<<<<< su(3)+su(2)+u(1) >>>>>>" if (d,kr)==(12,11) else ("   <- su(5)" if kr==24 else "")
    print(f"  dim {d:3d}  reductive {kr:3d}{tag}")
print(f"\nIS (12,11) REACHED over all subsets?  {(12,11) in spec2}")
print(f"does reductive rank 11 occur?           {any(kr==11 for _,kr in spec2)}")
print("\n--- the single-direction table, for comparison ---")
print(f"su(5) INTERSECT centraliser of c, over {len(CAND)} single directions in C:")
