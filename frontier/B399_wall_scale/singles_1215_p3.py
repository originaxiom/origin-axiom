"""Phase 1a -- the THIRD prime for the 1215 triple identification."""
import json, os, sys
import numpy as np
...
N, o1 = 1215, 1620
prs = primes_1_mod(4*N, 3, start=3*10**7)
p = prs[2]                      # the third prime (first two already banked)
print("third prime:", p, flush=True)
g = primitive_root(p)
zN = pow(g, (p-1)//N, p)
...
for a in range(o1):
    ...
    t = t * inv % p
    if t: cells[a] = t
json.dump({str(p): cells}, open(os.path.join(HERE, "singles_1215_p3.json"), "w"), indent=1)
