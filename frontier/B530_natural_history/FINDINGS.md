...
| M mod 11 order | 30 |
| M mod 19 order | 90 |
| period-k torsion |det(M^k−I)| | k=1:11, 2:11, 3:44, 4:319, 5:341, 6:3344, 7:9251, 8:28391, 9:151316, 10:378851, 11:1783661, 12:6206464 |
...
## Movement XXXII — the wall-crossing inventory, verified
A second seat delivered a "wall-crossing handoff" — a comprehensive inventory of what the object carries to the
physics firewall. Independently verified every computable claim (`listen_37_wall_crossing_verification.py`).

**1. Three frozen gap labels: VERIFIED (Bellissard gap-labeling theorem).**
The tight-binding Hamiltonian on the substitution sequence has spectral gaps at IDS positions:

| gap | IDS value | = cumulative Perron freq | V-independent? |
|---|---|---|---|
| 1 | **0.2720** | freq(a) = φ/S | YES (4 digits, V=0.5,1,2,5) |
| 2 | **0.4401** | freq(a)+freq(b) = (φ+1)/S | YES |
| 3 | **0.7862** | 1−freq(B) = (S−√φ)/S | YES |

where S = φ+1+φ√φ+√φ ≈ 5.948 is the sum of the Perron weights. The gap labels live in
(1/S)·ℤ[φ,√φ], the rank-4 module generated by the four letter frequencies. **The gaps follow
the letter hierarchy:** gap 1 separates old-structural (a) from everything else; gap 2 separates
old (a+b) from new (A+B); gap 3 separates everything from tunnel-new (B). The spectral structure
mirrors the decider/courier architecture.

This is the Bellissard gap-labeling theorem applied to the substitution — a standard result for
primitive quasicrystalline systems. The labels are topological (V-independent), frozen, and testable:
a physical realization of this quasicrystal would show gaps at these IDS positions.
**[MATH, computed and cross-checked at 4 potential strengths]**

**2. Dynamical zeta function: VERIFIED.**
|det(M^k−I)| = |∏(λᵢᵏ−1)| gives the period-k tiling-count invariant:

| k | |det(M^k−I)| | factorization | note |
|---|---|---|---|
| 1 | 11 | 11 | N(1−β), the tiling prime |
| 2 | 11 | 11 | |
| 3 | 44 | 2²·11 | parity (2) enters |
| 4 | 319 | 11·29 | first fully-split prime |
| 5 | 341 | 11·31 | |
| 6 | 3344 | 2⁴·11·19 | |
| 7 | 9251 | 11·29² | |
| 8 | 28391 | 11·29·89 | parity disc prime (89) enters |
| 9 | 151316 | 2²·11·19·181 | |
| 10 | 378851 | 11²·31·101 | 101 = f(−3) |
| 11 | 1783661 | 11²·14741 | |
| 12 | 6206464 | 2¹⁰·11·19·29 | |

**11 divides every |det(M^k−I)|** — it is the universal factor, the object's signature prime. Each
prime's entrance period is structural: 89 (parity discriminant) at k=8, 29 (first splitting prime)
at k=4, 101 = f(−3) = char poly evaluated at −3 at k=10. Cross-checked via eigenvalue products.
**[MATH, exact]**

**3. Handoff corrections applied.**
Two values in the handoff's inventory table repeat errors already corrected in movement XXXI:
- **Recognizability:** handoff says "radius = 9"; correct value is centered R=3, diameter 7 (XXXI).
- **Bounded remainder:** handoff says "≈ 15 in each coordinate"; correct value is ≈ 1.6 (XXXI).
- **Mixed-chain gap-opening:** the handoff states "slopes determined by the Perron vector's
  projection" as a prediction; these slopes are NOT computed in the handoff. This is a
  perturbation-theory conjecture, not a frozen prediction. Flagged as OPEN/UNCOMPUTED.
