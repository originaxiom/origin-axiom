# ---------------------------------------------------------------------------
# BORROWED AND MINIMALLY ADAPTED -- the algorithm is NOT this seat's.
#   source : frontier/B1375_the_towers_generation_count/verification/tower_generations.py
#   lane   : <remote>/standard-model-derivation-0qt6ao at 23532539
#   author : seat cc
#   adapted by seat xb for xB027.
#
# THE ONLY CHANGE, and it is the whole point of the arc:
#   the published driver hard-codes, in TWO places,
#       snappy.Manifold('m004').covers(n, cover_type='cyclic')[0]
#   which is the object's own tower.  Here that is replaced by a BUNDLE NAME built from
#   a prefix, so the same census can be pointed at b++(LR)^n (the object's tower, used as
#   the binding control C1) or at b+-(LR)^n (its A5 image, the actual question).
#
#   WHY A BUNDLE NAME AND NOT covers(): m004 has H_1 = Z, so its degree-n cyclic cover is
#   UNIQUE and is the fibre-direction one, and equals b++(LR)^n.  m003 has H_1 = Z/5 + Z,
#   its cyclic covers are NOT b+-(LR)^n (n=2 gives Z/5+Z where the bundle gives Z+(Z/3)^2)
#   and at n=5 there are SIX of them, which trips the original's `assert len(M) == 1`.
#   Pointing covers() at m003 would have censused the wrong manifolds silently.
#
# Nothing else is altered: the characters, the T5 restriction, the index, the
# generation-shaped search, the multi-prime re-check and the printed fields are cc's.
# ---------------------------------------------------------------------------
#!/usr/bin/env python3
...
PREFIX = argv[1] if len(argv) > 1 else 'b++'          # ADAPTED: 'b++' or 'b+-'
B1374 = os.path.join(os.path.dirname(os.path.abspath(__file__)), "borrowed_from_sm_lane")          # ADAPTED: borrowed copy
...
class Level:
    def __init__(self, n, p, N):
        self.M = snappy.Manifold(PREFIX + 'LR' * n)          # ADAPTED (was: m004.covers(n, cyclic)[0])
...
def run_level(n):
    t0 = time.time()
    H1 = str(snappy.Manifold(PREFIX + 'LR' * n).homology()); e = torsion_exponent(H1); N = 12 * e // math.gcd(12, e)          # ADAPTED
...
if __name__ == '__main__':
    out = {}
    for n in [int(x) for x in argv[2:]] or [2, 3, 4]:          # ADAPTED: argv[1] is the prefix
        out[n] = run_level(n)
        json.dump(out, open(os.path.join(HERE, f'census_{PREFIX.replace("+","p").replace("-","m")}_{"_".join(argv[2:]) or "2_4"}.json'), 'w'), indent=1, default=str)
    print("DONE")
