"""B607 — THE CHARGE TABLE: which theta-odd root content sits in which
...
# ---- B607: the exact charge table of the odd-block lines --------------------
# E6 > D5 x U(1)_P: charge_P(alpha) = n_endP(alpha) (coefficient = pairing
# with the fundamental coweight of the deleted node). The theta-odd
# combination o_alpha mixes alpha (charge q) with sigma(alpha) (charge
# q' = n_endQ(alpha)), so each line's charge content is a multiset.
# SU(5) < D5: delete the D5-fork end that is NOT part of the A4 chain —
# identified from the computed adjacency below. SM classes of A4 roots by
# support on the su(3) vs su(2) sub-chains after deleting the hypercharge
# node of A4... reported as the (q_P, q_S) bicharge + A4-membership: the
# math-tier table. No physics identification is made.

def charge_pair(al):
    return (al[endP], al[endQ])


print("\nTHE CHARGE TABLE (exact coefficients; classes by "
      "(n_endP, n_endQ) bicharge + D5/16 label):", flush=True)
ODD = {}
for h in range(1, 9):
    ph = [p for p in pairs if heights[p[0]] == h]
    ODD[h] = ph

for m, dim in ((4, 9), (8, 17)):
    print(f"\nblock V(m={m}) (dim {dim}):", flush=True)
    for j, vec in enumerate(BLOCKS[m]):
        hw = m - j                              # ad-weight = 2*hw
        if hw == 0:
            # Cartan line
            oC1 = msub(hs[endP], hs[endQ])
            oC2 = msub(hs[midP], hs[midQ])
            cc = solve_in([oC1, oC2], vec)
            print(f"  wt {2*hw:+3d}: CARTAN mix (hP-hQ){fmtk(cc[0])} + "
                  f"(hmP-hmQ){fmtk(cc[1])}  -> charge classes: "
                  f"{{(1,-1)-type dual}}", flush=True)
            continue
        h = abs(hw)
        side = "+" if hw > 0 else "-"
        odds = []
        ...
        for (a, b) in ODD[h]:
            ...
            if hw < 0:
                o = theta_neg(o)
            odds.append(o)
            qa, qb = charge_pair(a), charge_pair(b)
            labels.append((a, b, qa, qb))
        co = solve_in(odds, vec)
        assert co is not None, f"expand fail m={m} wt={2*hw}"
        parts = []
        for cf, (a, b, qa, qb) in zip(co, labels):
            if cf.is_zero():
                continue
            parts.append(f"{fmtk(cf)}*[o(a={a}) q_P/Q={qa}; "
                         f"sig q={qb}]")
        print(f"  wt {2*hw:+3d}: " + "  +  ".join(parts), flush=True)

print("\nB607 charge census per line (distinct (nP,nQ) bicharges touched):",
      flush=True)
for m in (4, 8):
    for j, vec in enumerate(BLOCKS[m]):
        hw = m - j
        ...
        for (a, b) in ODD[h]:
            ...
            if hw < 0:
                o = theta_neg(o)
            odds.append(o)
            charges.append({charge_pair(a), charge_pair(b)})
        co = solve_in(odds, vec)
        touched = set()
        for cf, chs in zip(co, charges):
            if not cf.is_zero():
                touched |= chs
        pure = len(touched) == 1
        print(f"  V(m={m}) wt {2*hw:+3d}: bicharges {sorted(touched)}"
              f"  [{'PURE' if pure else 'MIXED'}]", flush=True)

print("\nB607 DONE", flush=True)
