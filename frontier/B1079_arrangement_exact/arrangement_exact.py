#!/usr/bin/env python3
"""B1079 -- the rung arrangement, EXACT over Q: B1078's residue closed, and the 64 Levis.

B1078 proved the paper's eleven-element rung bound is TIGHT, but registered a residue: the
...
This closes that residue, by a construction that shares no code path with B1078 -- no
...
NOT PREREGISTERED.  The protection is the same as B1078's: every control is falsifiable by
something this arc did not choose -- B1078's independently computed multiplicity profile,
...
gate("multiplicity profile 12x1 + 18x3 -- B1078 computed this from the CHARGES, "
...
gate("109 flats -- the same lattice B1078 found at three primes", len(flats) == 109)
...
print(f"""
  THE RESIDUE IS CLOSED.  B1078 registered the flat lattice as certified at three
  ...
  Two constructions, no shared code path, one answer:
      B1078: charges -> ad-matrices -> charpoly orbits -> weights mod three primes
      B1079: E6 roots -> restrict to (A2)-perp -> weights over Q
  ...
  only after base change to K.  That is why B1078's (8,16)-plane cubic is irreducible
...
RES = {"n_roots": 72, "levi_root_counts": sorted(bycount),
       ...
       "scope": ("Everything here is exact over Q and needs no primes and no numerics: "
                 ...
                 "Q-enumeration is its Qbar-enumeration. This CLOSES the residue B1078 "
