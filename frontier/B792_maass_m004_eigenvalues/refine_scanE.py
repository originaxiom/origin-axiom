r"""Refine the scanE window (r in 10-13.5; B796 Cell 1): two-system
stability, multiplicities, S-invariance old/new split. GH-rung
candidates (11.008, 12.500, 13.294) refined FIRST — the sealed
falsifier decision. Writes scanE_refined.json.
...
with open(f"{OUTDIR}/scanE_dips.json") as f:
...
S1 = System(lat, moves, 0.75, 13.6)
S2 = System(lat, moves, 0.62, 13.6, seed=23)
...
GH = (11.008, 12.500, 13.294)
dips.sort(key=lambda d: min(abs(d['r'] - g) for g in GH))
results = []
for d in dips:
    r0 = d['r']
...
with open(f"{OUTDIR}/scanE_refined.json", 'w') as f:
    json.dump({'window': [10.0, 13.5], 'eigenvalues': stable_list,
...
print("Saved scanE_refined.json")
