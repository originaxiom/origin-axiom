"""D5 — the triality decomposition (prereg 4145e2da, sealed). Exact throughout the
load-bearing steps; the commutant diagnostic is float-labeled.
Stages: exec B575 prefix -> pkl load + C27 rebuild + gates -> weight bridge ->
pattern solves -> (if found) H1 grading."""
...
def pattern_solve(pperm):
    rows = []
    for M in (A27, B27, C27):
        # (X M - M X)[i][j] = d_j' terms: X M: sum_k X[i][k] M[k][j] = d_k M[k][j] at i=pperm[k]
        for i in range(d):
            for j in range(d):
                coef = [K0] * d
                for k in range(d):
                    if pperm[k] == i:
                        coef[k] = coef[k] + M[k][j]
                # M X: sum_k M[i][k] X[k][j] = M[i][pperm[j]] d_j
                coef[j] = coef[j] - M[i][pperm[j]]
...
for pi, (pperm, perm) in enumerate(patterns):
    ker, Rr, piv = pattern_solve(pperm)
    print(f"pattern {pi} (relabel {perm}): solution dim = {ker}", flush=True)
...
XM = mmul(X, mmul(X, X))
lam = XM[0][0]
assert all((XM[i][j] - (lam if i == j else K0)).is_zero() for i in range(d) for j in range(d)), \
    "X^3 not scalar"
print(f"ACTION-FOUND at pattern {pi}: X^3 = lambda I; grading H1...", flush=True)

def act_on_class(u):
    return {g: [sum((X[i][k] * u[g][k] for k in range(d)), K0) for i in range(d)]
            for g in u}
