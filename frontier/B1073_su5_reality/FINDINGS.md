# B1073 — the object's `su(5)` is NOT τ-stable: a cross-seat claim that does not reproduce
...
At all three primes: `A = Stab_{e₆}(e_i, ēbar_j, s) = (34, 24)` — B1068's `cell11_compose.py`
...
- **Which real form anything is.** All four real forms of `su(5)` have dim 24 and Killing rank 24;
  rank cannot separate them. That needs `dim(B ∩ k)` in characteristic zero — `frontier/B1071_reality_gate/`
