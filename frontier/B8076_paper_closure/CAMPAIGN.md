...
- **2026-08-19** — **item 9 GREEN.** The proof compared block sequences while the cited
  ...
  stated, proved and checked: **on positive words, conjugacy is cyclic rotation.** The check uses a
  **complete** invariant rather than a search for a conjugator, which matters — a bounded search
  finding nothing proves nothing. 2026 words, 241 classes, 241 invariants, zero collisions.
- **2026-08-19** — **item 8 GREEN, and it cost nothing extra.** B8081 had to build `ρ` to close
  item 5, and once `ρ` exists the order 2880 and the 63-class comparison are the same computation
  — so item 8's deposit was already made before item 8 was reached. The block-(b) row that cited the repository-side
  McKay-tensor test is retired as superseded. **Two of the twelve items turned out to be
  one piece of work**, which is an argument for building the object rather than certifying around
  it: the certificate had to be re-earned per claim, the construction covers every claim about it
  at once.
