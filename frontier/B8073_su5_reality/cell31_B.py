"""B8073 -- is the object's su(5) tau-stable?   B = A ^ tau(A).

The banked negative "su(5) is real in NO real form -- 254 of 254" (B8068 CELL_ITEM1) was
...
B8068 = os.path.join(HERE, "..", "B8068_j2t_charge_field")
sys.path.insert(0, B8068)

PRIME = int(sys.argv[1]) if len(sys.argv) > 1 else 1093

# ---- reproduce the banked environment (cell5 -> cell3) in this process ------------------
src = pathlib.Path(os.path.join(B8068, "cell5_spinor_test.py")).read_text()
src = src.split('print("\\nSTABILISER')[0].replace(
    "PRIME = int(sys.argv[1]) if len(sys.argv) > 1 else 1093", f"PRIME = {PRIME}")
# the exec'd source resolves cell3 relative to __file__; point it at B8068, not here
src = src.replace("os.path.dirname(os.path.abspath(__file__))", repr(os.path.abspath(B8068)))
