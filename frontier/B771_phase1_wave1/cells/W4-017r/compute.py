#!/usr/bin/env python3
"""
B771 Phase-1 Wave-4 cell W4-017r -- RZ residuals (carry/fix of W3-017).

W3-017 reported the L6a4 (Borromean-adjacent) SL(3) Ptolemy census class 10 as
EMPTY (solutions=0).  The wave-3 verifier found genuine SVD-verified isolated
points there under two seedings, contradicting B461's addendum.  The false-empty
was a SEEDING failure, NOT a real emptiness: class 10 is a 33-variable system and
the W3-017 solver seeded random Newton in the FULL 33-dim complex space, whose
basins of the class-10 real points are effectively unreachable.

FIX (this cell): reduce the class-10 system first -- fix u to each primitive cube
root of unity (its cubic 1+u+u^2=0) and eliminate every linear pin -- collapsing
33 unknowns to 26, then seed Newton in that reduced space.  The class-10 points are
then found robustly.  Their fields are recognized with the SAME high-precision
mpmath+PSLQ pipeline used for every other class (no methodological divergence in
field recognition).  A COMPARATOR CONTROL applies the identical reduced pipeline to
a class the full-dim solver already populated (class 9) and asserts it reproduces
the same recognized field -- proving the reduced method finds real points and does
not fabricate.

The Ruelle spectral-gap (>=0.19) leg (PART B) was sound in W3-017 and is kept
VERBATIM and re-certified in-cell (no cited facts).
...
def main():
    ...
    with open(os.path.join(HERE, "results.json"), "w") as fh:
        json.dump(dict(cell="W4-017r", verdict="PENDING_PART_B", part_A=A), fh,
    ...
    results = dict(cell="W4-017r", verdict=verdict, reason=reason,
