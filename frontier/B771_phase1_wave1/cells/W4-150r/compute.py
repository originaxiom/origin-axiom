#!/usr/bin/env python3
"""
W4-150r -- B178/B171: exact gap-power / rank-3 label. VERDICT BLOCK ADDED
(B771 Phase-1 Wave-4, carrying Wave-3 forward after the verifier's finding).

WAVE-3 CARRY, restated (from the batch context): W3-150r's math and its S4 enforcement (the
N-selection diagnostic promoted from comment to `chk(...)`) were confirmed SOUND by the verifier.
The one remaining gap: W3-150r's compute.py had NO in-code verdict block -- "S4 verdict: ...
PASSES" and "RESOLVED-A" were agent PROSE in a print string, not a computed, checkable conclusion.
Per house method ("verdict logic MUST be able to emit UNRESOLVED and MUST be IN compute.py, not
agent prose"), this is a real gap even though every underlying `chk(...)` was itself sound.

THIS CELL'S ONLY CHANGE: S1, S2, S3, S4 below are REUSED VERBATIM from cells/W3-150r/compute.py
(byte-for-byte identical maths and checks -- diff-checked below). The only two deltas are (i) the
shared `chk()` helper now also appends every check's (name, cond, detail) to an in-memory log
(bookkeeping only -- it does not change what any S1-S4 check tests or its pass/fail outcome), and
(ii) a new section S5 -- VERDICT BLOCK -- appended at the end, which is the actual deliverable of
this Wave-4 cell.

S5's logic: RESOLVED-A requires (a) the 3 S4 N-selection-diagnostic checks (CHECK 1/2/3) and the
S4 vacuity self-test to be found in the log and HOLD, and (b) every OTHER check in the entire cell
(S1-S4) to HOLD, with exactly ONE named exception -- the 2 pre-existing S2 (2,1) secondary
diagnostics (the shrink-factor and quadratic-scaling checks for label (2,1), known-VIOLATED since
W3-150r for a stated, non-blocking reason: see the S2 HONEST NOTE below). Those 2 are allow-listed
BY EXACT CHECK-NAME STRING, with a self-consistency guard: if either allow-listed name is not
found in this run's log, or is found but no longer VIOLATED (i.e. the allowlist has gone stale),
S5 refuses to call RESOLVED-A and reports UNRESOLVED instead -- the allowlist cannot silently
swallow a NEW or DIFFERENT failure. Any VIOLATED check anywhere in S1-S4 that is not one of these
2 exact, named, pre-characterized checks blocks RESOLVED-A and S5 emits UNRESOLVED.

For provenance, the original W3-150r header (describing the Wave-2->Wave-3 S4 fix) is preserved
below verbatim, since S1-S4's content is unchanged:
---
...
SEALED CRITERION (B771 Phase-1 Wave-4, W4-150r): verdict block emits RESOLVED-A with S4 enforced
(3 N-selection checks + vacuity self-test HOLD, and every other S1-S4 check HOLDS except the 2
named, pre-characterized S2 (2,1) secondary diagnostics) => RESOLVED-A; else the block emits
UNRESOLVED (or, were S1's rank-3 finding itself to be overturned in-cell, RESOLVED-B -- not
triggered here since S1 is unchanged and untouched).
...
overall_ok = True
CHECK_LOG = []   # every chk() call's (name, cond, detail) -- bookkeeping only, added for S5.
...
def chk(name, cond, detail=""):
    ...
    W4 addition: also appends to CHECK_LOG (bookkeeping for S5's verdict block below -- does
    NOT alter cond, overall_ok, or any printed HOLDS/VIOLATED outcome for S1-S4)."""
    global overall_ok
    cond = bool(cond)
    overall_ok = overall_ok and cond
    tag = "HOLDS   " if cond else "VIOLATED"
    print(f"  [{tag}] {name}" + (f"   -- {detail}" if detail else ""))
    CHECK_LOG.append({"name": name, "cond": cond, "detail": detail})
...
# =====================================================================================
print("=" * 92)
print("S5 -- VERDICT BLOCK (Wave-4 deliverable: promotes RESOLVED-A from agent prose in the S4")
print("      print string above to an in-code, checkable, UNRESOLVED-capable decision)")
print("=" * 92)
print("""
  Logic (stated before it runs, not fitted to the outcome): RESOLVED-A requires (a) the 3 S4
  N-selection-diagnostic checks named CHECK 1/CHECK 2/CHECK 3 in the S4 source above, plus the S4
  vacuity SELF-TEST, to be present in this run's CHECK_LOG and HOLD; AND (b) every other check
  logged anywhere in S1-S4 to HOLD, with EXACTLY ONE named exception: the 2 pre-existing S2 (2,1)
  secondary diagnostics (shrink-factor, quadratic-scaling), which are allow-listed by their exact
  `chk()` name string, each with its own stated non-blocking reason (see the S2 HONEST NOTE).
  The allowlist is self-guarded: if a listed name is missing from the log, or is present but no
  longer VIOLATED, that is itself flagged and blocks RESOLVED-A (an allowlist must not be able to
  silently absorb a NEW or DIFFERENT failure by pattern-matching too loosely). Any VIOLATED check
  outside this exact 2-entry allowlist also blocks RESOLVED-A. Verdict is UNRESOLVED otherwise.
""")

# --- the 3 S4 N-selection-diagnostic checks (CHECK 1/2/3) + the S4 vacuity self-test, by their
# exact `name` string as passed to chk(...) above (deterministic strings -- none of them embed a
# runtime-computed float, only fixed labels/constants, so exact-string matching is stable run to
# run as long as the underlying claim text is not edited).
REQUIRED_S4_NAMES = [
    "golden-silver (1,1): the N=250,000 width is SMALLER than both its neighbours in the sweep "
    "(N=100,000 and N=400,000) -- confirms a genuine non-monotonic dip/artifact at 250k, matching "
    "the claim that 250k undermeasures the gap (not a smooth monotonic trend that happens to pass "
    "through a small value there)",
    "golden-silver (1,1): the width is STRICTLY MONOTONICALLY INCREASING across all four sweep "
    "points from N=400,000 through N=3,200,000 (matches 'stabilizes [no more dips] and grows "
    "monotonically from N=400k on' -- the erratic 250k-style dip does not recur in this range)",
    "golden-silver: the (1,1) [sum=2] width exceeds the (3,-3) [sum=6] width at EVERY tested N "
    "from 400,000 to 3,200,000 (the sum-2-wider-than-sum-6 ordering the main hunt reports at one "
    "N is ROBUST across a full N-sweep, not a single-point coincidence)",
    "SELF-TEST (vacuity guard): shrinking lam from 0.15 to 0.05 SHRINKS the (1,1) golden-silver "
    "gap width (confirms the measured width tracks the coupling and is not a lam-independent "
    "density-floor artifact that would pass regardless of lam)",
]

# --- the 2 known, pre-existing, non-blocking S2 (2,1) secondary-diagnostic violations, allow-listed
# BY EXACT NAME with a stated reason each (both are the SAME root cause: label (2,1)'s residual is
# already so small in absolute terms -- 7.7e-6 to 3.5e-5 -- that it is dominated by a subleading
# term the two 50x/quadratic heuristics were not built to isolate; the PRIMARY S2 claim for (2,1),
# i.e. the leading power itself, rests on the nonzero-S-to-40-digits proof and the <5e-3 match
# check, both of which HOLD for (2,1) -- see the S2 HONEST NOTE printed above).
ALLOWLIST = {
    "(2, 1): gap/lam1^2's relative step-to-step change SHRINKS by >=50x from the first to the "
    "last tested decade of lam1 (genuine convergence to a constant, not noise or a coincidental "
    "local slope)":
        "shrink observed = 14.0x vs the arbitrary 50x bar; (2,1)'s absolute mismatch is already "
        "tiny (order 1e-5), so it is more exposed to a subleading (plausibly lam1^2-order) "
        "correction than (1,1)/(1,2)'s larger residuals are -- does not touch S2's primary claim "
        "(the leading lam-power itself), which holds for (2,1) via the nonzero-S and <5e-3-match "
        "checks.",
    "(2, 1): the path-sum/numerics mismatch shrinks QUADRATICALLY when lam2 shrinks 10x (observed "
    "ratio within [0.2x,5x] of the ideal 0.010 -- confirms the residual IS the expected O(lam2^2) "
    "sub-leading term, not a formula error)":
        "observed shrink ratio = 0.17 vs ideal 0.01; same root cause as the shrink-factor miss "
        "above (a genuinely tiny residual at this label is not cleanly lam2^2-dominated) -- does "
        "not touch S2's primary claim for (2,1).",
}

log_by_name = {}
for _e in CHECK_LOG:
    log_by_name.setdefault(_e["name"], []).append(_e)

# vacuity guard on the allowlist itself: every allowlisted name must (i) exist in this run's log
# and (ii) currently be VIOLATED. An allowlist entry that no longer fires (e.g. floating-point
# noise moved the boundary) would silently stop testing anything if accepted blindly -- that is
# flagged as an allowlist_issue, not silently passed through.
allowlist_issues = []
for _name, _reason in ALLOWLIST.items():
    if _name not in log_by_name:
        allowlist_issues.append(f"allowlisted check text NOT FOUND in this run's log: {_name[:70]}...")
    elif all(_e["cond"] for _e in log_by_name[_name]):
        allowlist_issues.append(f"allowlisted check now HOLDS (stale allowlist entry): {_name[:70]}...")

required_missing = [n for n in REQUIRED_S4_NAMES if n not in log_by_name]
required_failed = [n for n in REQUIRED_S4_NAMES
                    if n in log_by_name and not all(e["cond"] for e in log_by_name[n])]
s4_enforced = (not required_missing) and (not required_failed)

unexpected_violations = [e for e in CHECK_LOG if (not e["cond"]) and (e["name"] not in ALLOWLIST)]
allowlist_sound = not allowlist_issues

print(f"  S4 required checks (3 N-selection diagnostics + vacuity self-test) found & HOLD: "
      f"{s4_enforced}   (missing={len(required_missing)}, failed={len(required_failed)})")
print(f"  allowlist self-consistent (both entries present & still VIOLATED): {allowlist_sound}")
if allowlist_issues:
    for _msg in allowlist_issues:
        print(f"     ISSUE: {_msg}")
print(f"  unexpected VIOLATED checks outside the 2-entry allowlist: {len(unexpected_violations)}")
for _e in unexpected_violations:
    print(f"     - {_e['name']}")

if s4_enforced and allowlist_sound and not unexpected_violations:
    verdict = "RESOLVED-A"
    reason = ("S4 fully enforced in-code: the 3 N-selection-diagnostic checks (CHECK 1/2/3) and "
              "the S4 vacuity self-test all HOLD; every other check across S1-S4 HOLDS except the "
              "2 named, pre-characterized, non-blocking S2 (2,1) secondary diagnostics, which "
              "remain exactly the 2 known violations (no new or different failure appeared).")
elif not allowlist_sound:
    verdict = "UNRESOLVED"
    reason = "allowlist is not self-consistent this run: " + "; ".join(allowlist_issues)
elif not s4_enforced:
    verdict = "UNRESOLVED"
    reason = (f"S4 not fully enforced -- missing from log: {required_missing}; "
              f"failed (found but VIOLATED): {required_failed}")
else:
    verdict = "UNRESOLVED"
    reason = ("unexpected VIOLATED check(s) outside the 2-entry allowlist: " +
              "; ".join(e["name"] for e in unexpected_violations))

print(f"\n  VERDICT: {verdict}")
print(f"  REASON: {reason}")

n_holds = sum(1 for e in CHECK_LOG if e["cond"])
n_violated = len(CHECK_LOG) - n_holds
print(f"\n  cell-wide tally: {len(CHECK_LOG)} checks total, {n_holds} HOLD, {n_violated} VIOLATED "
      f"({n_violated - len(ALLOWLIST) if n_violated >= len(ALLOWLIST) else n_violated} of which "
      f"unaccounted-for by the allowlist).")

results = {
    "cell": "W4-150r",
    "wave": "B771_phase1_wave4",
    "sealed_criterion": "verdict block emits RESOLVED-A with S4 enforced => RESOLVED-A; else per "
                         "the block (UNRESOLVED, or RESOLVED-B if S1's rank-3 finding were itself "
                         "overturned in-cell -- not triggered here).",
    "verdict": verdict,
    "reason": reason,
    "s4_required_checks": {
        "names": REQUIRED_S4_NAMES,
        "all_found_and_hold": s4_enforced,
        "missing": required_missing,
        "failed": required_failed,
    },
    "allowlist": {name: reason for name, reason in ALLOWLIST.items()},
    "allowlist_self_consistent": allowlist_sound,
    "allowlist_issues": allowlist_issues,
    "unexpected_violations": [e["name"] for e in unexpected_violations],
    "check_tally": {"total": len(CHECK_LOG), "holds": n_holds, "violated": n_violated},
    "full_check_log": CHECK_LOG,
    "cell_time_s": time.time() - t_start,
}
results_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "results.json")
with open(results_path, "w") as f:
    json.dump(results, f, indent=2, default=str)
print(f"\n  results.json written unconditionally -> {results_path}")

# =====================================================================================
print("=" * 92)
print(f"TOTAL cell time: {time.time() - t_start:.1f}s")
print("=" * 92)
print(f"\nCELL VERDICT: {verdict}   (S4 enforced={s4_enforced}, allowlist_sound={allowlist_sound}, "
      f"unexpected_violations={len(unexpected_violations)})")
sys.exit(0 if verdict == "RESOLVED-A" else 1)
