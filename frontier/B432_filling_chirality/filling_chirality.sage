...
for p in range(0, 8):
    for q in range(1, 8):
        ...
        try:
            ...
            rows.append(dict(p=int(p), q=int(q), vol=float(round(v, 6)), cs=float(round(cs, 9)), chiral=bool(d > 1e-8)))
        except Exception:
            continue
n_ch = sum(1 for r in rows if r["chiral"])
print("cusped CS =", cs0, " fillings:", len(rows), " chiral:", n_ch)
json.dump(dict(cusped_cs=float(cs0), n=int(len(rows)), n_chiral=int(n_ch), rows=rows),
