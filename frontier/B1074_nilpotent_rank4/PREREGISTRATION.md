# PREREGISTRATION — B1074: does the rank ceiling cover nilpotents?
...
`{78×1, 46×27, 38×36}` and nothing else. `frontier/B1071_reality_gate/reality_gate.py` already
...
centralisers admit is `frontier/B1071_reality_gate/`, not this arc.

**Not established here:** that the *object* reaches any particular orbit. The ω-covariant purity
reading that places it on `2A1` is mod-`p` work in `frontier/B1068_j2t_charge_field/` and is
