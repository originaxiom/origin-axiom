...
# --- B778 cleanup CL-W5139: dead RESOLVED-B branch stripped -------------------
# Every clause of genus_is_clean below is ALREADY enforced by a hard assert
# upstream and cannot be False here:
#   controls_ok                          -> assert @ STEP 4 (order estimator controls)
#   len(sup[name]) % 2 == 0 for gens     -> assert @ STEP 5 (divisor degree 0)
#   consistency_ok                       -> assert @ STEP 5b ((Z/2)^3 character check)
#   connected                            -> assert @ STEP 5c (A_3 irreducible 24/40)
#   B >= 0                               -> B is a set cardinality, always >= 0
# The old `else: RESOLVED-B` branch was therefore unreachable. We re-assert the
# conjunction as a final guard (loud failure, never a silent forced RESOLVED-A)
# and emit the single reachable verdict.
genus_is_clean = (controls_ok and consistency_ok and connected
                  and all(len(s) % 2 == 0 for s in supports.values()) and B >= 0)
assert genus_is_clean, ("clean-genus guards regressed at the verdict -- upstream asserts "
                        "(STEP 4/5/5b/5c) should have aborted first; do not emit a verdict")
...
notes = (f"F0 genus 1 (rigorous, squarefree quartic branch = banked disc3). Cover degree 8 "
         ...
         f"only; nothing to CLAIMS.md. [B778 CL-W5139: dead RESOLVED-B verdict branch stripped; "
         f"verdict logic unchanged in outcome, genus=41 reproduced.]")
