...
The dispatching hypothesis ("the factor is the primitivity convention") was
REFUTED by this cell — the primitive cubic transports with t = 1 exactly; the
bridge S is a signed permutation. The factor is real mathematics: two distinct
invariant Hermitian structures (the charge-equivariant H₊ vs the τ-twisted
dual intertwiner M), and 2304/953 = |N_{K/ℚ}(d)|^{−1/2} — the field norm of
the per-line twist. Both seats' numbers are exact and realization-independent.
The cross-seat convention rule is adopted programme-wide: **pin the Hermitian
structure by its equivariance pattern, never by basis-primitivity.** The
