...
| **`philosophy/`** | the **motivation** layer: `P000–P004` + the `P1–P5` argument register + `METALLIC_FOUNDATIONS` | **never** |
| **`story/`** | the **honest narrative** — how the project actually unfolded, with `V`/`B` traceability, no duplicated math | **never** |
| **`knowledge/`** | self-contained explainers `K001–K008` (`K001–K007` stubbed; **`K008` written** — the `(n; trace, det)` determination) | **never** |
